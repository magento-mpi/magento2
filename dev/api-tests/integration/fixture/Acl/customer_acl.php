...
if (!Magento_Test_Webservice::getFixture('customer_acl_is_prepared')) {
    ...
    $count = $rule->getCollection()
        ...
        ->count();
    if (!$count) {
        $rule->setRoleId(Mage_Webapi_Model_Acl_Global_Role::ROLE_CUSTOMER_ID)
    ...
    $count = $attribute->getCollection()
        ->addFieldToFilter('user_type', array(
            'eq' => Mage_Webapi_Model_Auth_User_Customer::USER_TYPE))
        ->addFieldToFilter('resource_id', array(
            'eq' => Mage_Webapi_Model_Acl_Global_Rule::RESOURCE_ALL))
        ->count();
    if (!$count) {
        $attribute->setUserType(Mage_Webapi_Model_Auth_User_Customer::USER_TYPE)
            ->setResourceId(Mage_Webapi_Model_Acl_Global_Rule::RESOURCE_ALL)
            ->save();
