        ...
        for ($i = 0, $c = count($this->tokens); $i < $c; ++$i) {
            ...
            if ('~' === $this->tokens[$i]
                ...
                if ('BINARY' === $matches[1]) {
                    ...
                    $replace = [
                        [is_int(bindec($matches[2])) ? T_LNUMBER : T_DNUMBER, $matches[2], $this->tokens[$i + 1][2]],
                    ];
                } elseif ('NS' === $matches[1]) {
                    ...
                    list($start, $content, $end) = [pack('H*', $start), pack('H*', $content), pack('H*', $end)];

                    $replace = [];
                    $replace[] = [T_START_HEREDOC, $start, $this->tokens[$i + 1][2]];
                    if ('' !== $content) {
                        $replace[] = [T_ENCAPSED_AND_WHITESPACE, $content, -1];
                    }
                    $replace[] = [T_END_HEREDOC, $end, -1];
                } else {
