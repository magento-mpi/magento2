...
\Magento\Autoload\IncludePath::addIncludePath(array(
    $basePath . '/dev/tests/static/framework',
    $basePath . '/lib',
    $basePath . '/dev/tools',
));
...
try{
    ...
    $classFiles = array_filter($classFiles, function($value) {
        return strpos($value, 'data-install') === false &&
            strpos($value, 'data-upgrade') === false &&
            strpos($value, 'install-') === false &&
            strpos($value, 'upgrade-') === false
        ;
    });
