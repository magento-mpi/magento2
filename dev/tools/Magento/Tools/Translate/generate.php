...
if (!isset($args['output'])) {
    exit(USAGE . 'Please indicate output parameter' . "\n");
}
if (!is_writeable(dirname($args['output']))) {
    exit(USAGE . sprintf('Output dir %s isn\'t writeable', realpath(dirname($args['output']))) . "\n");
}
...
function parseFile($fileName, $basicModuleName)
{
    ...
    foreach (file($fileName) as $fileLine => $fileString) {
        ...
        if (preg_match_all(
            '/helper\(([\'|\\\"])([a-z0-9_]+)(?:\/[a-z0-9_]+)?\\1\)-\>__\([\s]*([\'|\\\"])(.*?[^\\\\])\\3.*?\)/',
            $fileString,
            $matches,
            PREG_SET_ORDER
        )
...
function parseXmlFile($fileName, $basicModuleName)
{
    ...
    foreach ($xmlTranslate as $translate) {
        ...
        $moduleName = $translate['module'] ? isset(
            $CONFIG['helpers'][$translate['module']]
        ) ? $CONFIG['helpers'][$translate['module']] : '!' . $translate['module'] : $basicModuleName;
...
function writeToCsv($moduleName, $translationKey, $fileName, $fileLine, $xml = false)
{
    ...
    if (isset($CONFIG['generate']['args']['clean'])) {
        ...
        $csvData[] = array($moduleName, $translationKey, $translationKey, $fileName, $fileLine);
    }
}
