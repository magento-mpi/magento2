...
class Mage_Webhook_Model_UserTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $webapiUserId = Magento_Test_Helper_Bootstrap::getObjectManager()->create('Mage_Webapi_Model_Acl_User')
            ->setSecret('secret')
            ->save()
            ->getId();
        $user = Magento_Test_Helper_Bootstrap::getObjectManager()
            ->create('Mage_Webhook_Model_User', array('webapiUserId' => $webapiUserId));
