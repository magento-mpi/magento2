...
$testsuite = new Magento_Performance_Testsuite($config, new Magento_TestFramework_Application($config, $shell), $scenarioHandler);

$scenarioTotalCount = count($config->getScenarios());
$scenarioCount = 1;
$scenarioFailCount = 0;
$testsuite->onScenarioRun(
    function (Magento_TestFramework_Performance_Scenario $scenario) use ($logger, &$scenarioCount, $scenarioTotalCount) {
        $logger->log("Scenario $scenarioCount of $scenarioTotalCount: '{$scenario->getTitle()}'", Zend_Log::INFO);
        $scenarioCount++;
    }
);
$testsuite->onScenarioFailure(
    function (Magento_TestFramework_Performance_Scenario_FailureException $scenarioFailure) use ($logger, &$scenarioFailCount) {
