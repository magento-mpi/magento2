 ...
 */
class Mage_Core_Model_Config_FileResolver_PrimaryTest extends PHPUnit_Framework_TestCase
{
    /**
     * @var Mage_Core_Model_Config_FileResolver_Primary
     */
    protected $_model;
    ...
    protected function setUp()
    {
        ...
        $applicationDirsMock = $this->getMock('Mage_Core_Model_Dir', array(), array('getDir'), '', false);
        $applicationDirsMock->expects($this->any())
            ->method('getDir')
            ->with(Mage_Core_Model_Dir::CONFIG)
            ->will($this->returnValue($appConfigDir));

        $this->_model = new Mage_Core_Model_Config_FileResolver_Primary($applicationDirsMock);
    }
