...
class Mage_GoogleOptimizer_Model_Observer_Category_LoadTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testAppendToCategoryGoogleExperimentScriptSuccess()
    {
        ...
        $category->expects($this->once())->method('setGoogleExperiment')->with($this->_codeMock);
        $category->expects($this->exactly(2))->method('getStoreId')->will($this->returnValue($values['store_id']));
    ...
    public function testAppendToCategoryGoogleExperimentScriptFail()
    {
        ...
        $category->expects($this->never())->method('setGoogleExperiment');
        $category->expects($this->once())->method('getStoreId');
