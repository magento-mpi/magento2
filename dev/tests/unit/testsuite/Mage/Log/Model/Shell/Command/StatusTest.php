 ...
 */
class Mage_Log_Model_Shell_Command_StatusTest extends PHPUnit_Framework_TestCase
{
    /**
    ...
    /**
     * @var Mage_Log_Model_Shell_Command_Status
     */
    protected $_model;

    protected function setUp()
    {
        $this->_factoryMock = $this->getMock('Mage_Log_Model_Resource_ShellFactory',
            array('create'), array(), '', false
        );
        $this->_shellMock = $this->getMock('Mage_Log_Model_Resource_Shell', array(), array(), '', false);
        $this->_factoryMock->expects($this->once())->method('create')->will($this->returnValue($this->_shellMock));
        $this->_model = new Mage_Log_Model_Shell_Command_Status($this->_factoryMock);
    }
