...
class Magento_Webhook_Model_Subscription_ConfigTest extends PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        ...
        Mage::unregister('_helper/Magento_Webhook_Helper_Data');
        $mockHelper = $this->getMockBuilder('Magento_Webhook_Helper_Data')
            ->disableOriginalConstructor()
            ->getMock();
        $mockHelper->expects($this->any())
            ->method('__')
            ->will($this->returnArgument(0));
        Mage::register('_helper/Magento_Webhook_Helper_Data', $mockHelper);
    }
