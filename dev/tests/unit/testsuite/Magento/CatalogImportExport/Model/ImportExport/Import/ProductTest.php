<?php
/**
 * Test class for \Magento\CatalogImportExport\Model\ImportExport\Import\Product
 *
 * {license_notice}
 *
 * @copyright   {copyright}
...
class ProductTest extends \PHPUnit_Framework_TestCase
{
    ...
    /**
     * @var \Magento\CatalogImportExport\Model\ImportExport\Import\Product\OptionFactory|PHPUnit_Framework_MockObject_MockObject
     */
    protected $_optionFactory;

    /**
     * @var \Magento\CatalogImportExport\Model\ImportExport\Import\Product\Option|PHPUnit_Framework_MockObject_MockObject
     */
    ...
    protected function setUp()
    {
        ...
        $this->_optionFactory = $this->getMock(
            '\Magento\CatalogImportExport\Model\ImportExport\Import\Product\OptionFactory',
            array('create'),
            array(),
            '',
            false
    ...
    {
        $method = new \ReflectionMethod('\Magento\CatalogImportExport\Model\ImportExport\Import\Product', '_isMediaValid');
        $method->setAccessible(true);

        $this->assertEquals($expected['method_return'], $method->invoke($this->_model, $data, 1));

        $errors = new \ReflectionProperty('\Magento\CatalogImportExport\Model\ImportExport\Import\Product', '_errors');
        $errors->setAccessible(true);
        $this->assertEquals($expected['_errors'], $errors->getValue($this->_model));

        $invalidRows = new \ReflectionProperty('\Magento\CatalogImportExport\Model\ImportExport\Import\Product', '_invalidRows');
        $invalidRows->setAccessible(true);
        $this->assertEquals($expected['_invalidRows'], $invalidRows->getValue($this->_model));

        $errorsCount = new \ReflectionProperty('\Magento\CatalogImportExport\Model\ImportExport\Import\Product', '_errorsCount');
