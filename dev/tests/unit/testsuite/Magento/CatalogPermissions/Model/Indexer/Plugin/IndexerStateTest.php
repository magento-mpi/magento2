...
class IndexerStateTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $testableIndex  = \Magento\CatalogPermissions\Model\Indexer\Category::INDEXER_ID;
        $changedIndex = \Magento\CatalogPermissions\Model\Indexer\Product::INDEXER_ID;
        $testableStatus = 'testable_status';

        $testableState = $this->getMockBuilder('Magento\Indexer\Model\Indexer\State')
            ->setMethods(['getIndexerId', 'getStatus', '__wakeup'])
        ...
        $state = $this->getMockBuilder('Magento\Indexer\Model\Indexer\State')
            ->setMethods(['setData', 'loadByIndexer', 'save', '__wakeup'])
            ->disableOriginalConstructor()
            ->getMock();
        ...
        $model = new \Magento\CatalogPermissions\Model\Indexer\Plugin\IndexerState($state);
