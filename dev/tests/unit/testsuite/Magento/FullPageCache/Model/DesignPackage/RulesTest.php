...
class RulesTest extends \PHPUnit_Framework_TestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->_designMock = $this->getMock('Magento\View\DesignInterface');

        $this->_currentDate = date('Y-m-d');

        $this->_cacheId = \Magento\FullPageCache\Model\DesignPackage\Rules::DESIGN_CHANGE_CACHE_SUFFIX . '_' . md5(
            $this->_storeId . $this->_currentDate
        );
    ...
    public function testGetPackageNameWithCachedIdAndWithoutDesignException()
    {
        ...
        $this->_fpcCacheMock->expects($this->exactly(2))->method('load')->will($this->returnValueMap($valueMap));
    ...
    public function testGetPackageNameWithoutCachedIdAndWithoutDesignException()
    {
        ...
        $resourceMock->expects(
            ...
            $this->returnValue($this->_designChange)
        );

        $this->_designChangeMock->expects(
            $this->once()
        )->method(
            'getResource'
        )->will(
            $this->returnValue($resourceMock)
        );
    ...
    {
        $cache = array('design' => 'design_change');
        $designException = array('regexp' => 'some_reg_exp', 'value' => 'some-value');
        ...
        $model = $this->getMock(
            ...
            array($this->_designChangeMock, $this->_designMock, $this->_fpcCacheMock)
        );

        $model->expects(
            $this->once()
        )->method(
            '_getPackageByUserAgent'
        )->with(
            $designException
        )->will(
            $this->returnValue('design_name')
        );
