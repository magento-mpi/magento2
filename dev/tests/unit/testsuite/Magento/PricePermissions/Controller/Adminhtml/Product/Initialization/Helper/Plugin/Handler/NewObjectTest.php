...
class NewObjectTest extends \PHPUnit_Framework_TestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->pricePerDataMock->expects($this->once())
            ->method('getDefaultProductPriceString')->will($this->returnValue('0.00'));
    ...
    public function testHandleWithDynamicProductPrice()
    {
        ...
        $this->productMock->expects($this->once())->method('getPriceType')
        ...
        $this->productMock->expects($this->once())->method('setMsrpDisplayActualPriceType')
            ->with(\Magento\Catalog\Model\Product\Attribute\Source\Msrp\Type\Price::TYPE_USE_CONFIG);
    ...
    public function testHandleWithGiftCardProductType()
    {
        ...
        $this->productMock->expects($this->any())->method('getTypeId')
            ->will($this->returnValue(\Magento\GiftCard\Model\Catalog\Product\Type\Giftcard::TYPE_GIFTCARD));
        ...
        $this->storeManagerMock->expects($this->once())
            ->method('getStore')->with(10)->will($this->returnValue($storeMock));

        $this->productMock->expects($this->once())->method('setGiftcardAmounts')->with(
            array(
                array(
                    'website_id' => 5,
                    'price' => 0.0,
    ...
    public function testHandleWithNonGiftCardProductType()
    {
        $this->productMock->expects($this->once())->method('isObjectNew')->will($this->returnValue(true));
        $this->productMock->expects($this->any())->method('getTypeId')
            ->will($this->returnValue('some product type'));
