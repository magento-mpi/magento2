...
class BundleTest extends \PHPUnit_Framework_TestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->productTypeMock = $this->getMock(
            '\Magento\Bundle\Model\Product\Type', array(), array(), '', false
        );
    ...
    public function testHandleWithoutBundleSelectionData()
    {
        $this->productMock->expects($this->once())
            ->method('getTypeId')->will($this->returnValue(\Magento\Catalog\Model\Product\Type::TYPE_BUNDLE));
    ...
    public function testHandleWithBundleOptions()
    {
        ...
        $expected = array(
            array(
                ...
                array('product_id' => 20),
                array(
                    'product_id' => 40, 'option_id' => 50, 'delete' => true,
                    'selection_price_type' => 0,
                    'selection_price_value' => 0,
                ),
                array(
                    'product_id' => 60, 'option_id' => 70, 'delete' => false,
                    'selection_price_type' => 0,
                    'selection_price_value' => 0,
                ),
                array(
                    'product_id' => 80, 'option_id' => 90, 'delete' => false,
        ...
        $this->productMock->expects($this->once())
            ->method('getTypeId')->will($this->returnValue(\Magento\Catalog\Model\Product\Type::TYPE_BUNDLE));
        $this->productMock->expects($this->once())->method('getBundleSelectionsData')
            ->will($this->returnValue($bundleSelectionsData));
        ...
        $selectionMock = $this->getMock('\Magento\Bundle\Model\Selection',
            array('getProductId', 'getSelectionPriceType', 'getSelectionPriceValue', '__wakeup'), array(), '', false
        );
        ...
        $collectionMock->expects($this->once())->method('appendSelections')
            ->with($selectionsMock)->will($this->returnValue($origBundleOptions));

        /** Configuring product type object mock */
        $this->productTypeMock->expects($this->once())->method('setStoreFilter')->with(1, $this->productMock);
        $this->productTypeMock->expects($this->once())->method('getOptionsIds')
            ->with($this->productMock)->will($this->returnValue(array(1, 2)));
        $this->productTypeMock->expects($this->once())
            ->method('getOptionsCollection')->with($this->productMock)->will($this->returnValue($collectionMock));
        $this->productTypeMock->expects($this->once())
            ->method('getSelectionsCollection')->with(array(1, 2), $this->productMock)
            ->will($this->returnValue($selectionsMock));
