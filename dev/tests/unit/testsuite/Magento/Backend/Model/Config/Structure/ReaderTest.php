...
class Magento_Backend_Model_Config_Structure_ReaderTest extends PHPUnit_Framework_TestCase
{
    /**
     * @var Magento_Backend_Model_Config_Structure_Reader
     */
    protected $_model;
    ...
    {
        $this->_configMock = $this->getMock('Magento_Core_Model_Config_Modules_Reader', array(), array(), '', false);
        $this->_cacheMock = $this->getMock('Magento_Core_Model_Cache_Type_Config', array(), array(), '', false);
        $this->_converterMock = $this->getMock(
            'Magento_Backend_Model_Config_Structure_Converter', array(), array(), '', false
        );
    ...
    public function testGetConfigurationLoadsConfigFromCacheWhenCacheIsEnabled()
    {
        ...
        $this->_cacheMock->expects($this->once())->method('load')
            ->with(Magento_Backend_Model_Config_Structure_Reader::CACHE_SYSTEM_CONFIGURATION_STRUCTURE)
            ->will($this->returnValue($cachedData));

        $model = new Magento_Backend_Model_Config_Structure_Reader(
    ...
    public function testGetConfigurationLoadsConfigFromFilesAndMergeUnknownAttribute()
    {
        ...
        new Magento_Backend_Model_Config_Structure_Reader(
            $this->_cacheMock, $this->_configMock, $this->_converterMock, false
        );
    }
