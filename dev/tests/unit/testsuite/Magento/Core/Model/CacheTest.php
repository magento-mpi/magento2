...
class Magento_Core_Model_CacheTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $helperMock = $this->getMock('Magento_Core_Helper_Data', array('__'), array(), '', false);
        $helperMock
            ->expects($this->any())
            ->method('__')
        ...
        $this->_cacheTypesMock = $this->getMock('Magento_Core_Model_Cache_Types', array(), array(), '', false);

        $configFixture = new Magento_Core_Model_Config_Base(file_get_contents(__DIR__ . '/_files/cache_types.xml'));

        $dirsMock = $this->getMock('Magento_Core_Model_Dir', array(), array(), '', false);

        $this->_model = new Magento_Core_Model_Cache(
    ...
    public function testGetInvalidatedTypes()
    {
        ...
        $this->_cacheFrontendMock
            ->expects($this->once())
            ->method('load')
            ->with(Magento_Core_Model_Cache::INVALIDATED_TYPES)
    ...
    public function testInvalidateType()
    {
        $this->_cacheFrontendMock
            ->expects($this->once())
            ->method('save')
            ->with(serialize(array('test' => 1)), Magento_Core_Model_Cache::INVALIDATED_TYPES)
        ;
