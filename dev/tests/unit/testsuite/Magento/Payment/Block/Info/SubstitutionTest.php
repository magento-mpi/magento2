...
class SubstitutionTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->layout = $this->getMockBuilder('Magento\View\LayoutInterface')
            ->disableOriginalConstructor()
            ->setMethods([])
            ->getMock();

        $eventManager = $this->getMockBuilder('Magento\Event\ManagerInterface')
            ->disableOriginalConstructor()
            ->setMethods([])
            ->getMock();
        ...
        $storeConfig->expects($this->any())
            ->method('getConfig')
            ->with($this->stringContains('advanced/modules_disable_output/'))
            ->will($this->returnValue(false));

        $context = $this->getMockBuilder('Magento\View\Element\Template\Context')
            ->disableOriginalConstructor()
            ->setMethods(['getLayout', 'getEventManager', 'getStoreConfig'])
            ->getMock();
    ...
    {
        $abstractBlock = $this->getMockBuilder('Magento\View\Element\AbstractBlock')
            ->disableOriginalConstructor()
            ->setMethods([])
            ->getMock();
        ...
        $abstractBlock->expects($this->any())
            ...
            ->will($this->returnValue($childAbstractBlock));
        ...
        $this->layout->expects($this->any())
            ->method('getBlock')
            ->will($this->returnValue($abstractBlock));
