...
class CurrentUrlRewritesRegeneratorTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testGenerationForCustom(
        ...
        $this->category->expects($this->any())->method('getId')->will($this->returnValue('product_id'));
        $this->urlRewriteBuilder->expects($this->once())->method('setDescription')->with('')->will($this->returnSelf());
        $this->prepareUrlRewriteMock($storeId, $categoryId, $requestPath, $targetPath, $redirectCode, $metadata);
        $this->assertEquals(
            [$this->urlRewrite],
            $this->currentUrlRewritesRegenerator->generate('store_id', $this->category)
        );
     ...
     */
    protected function prepareUrlRewriteMock($storeId, $categoryId, $requestPath, $targetPath, $redirectType, $metadata)
    {
        ...
        $this->urlRewriteBuilder->expects($this->any())->method('setIsAutogenerated')->with(0)
            ->will($this->returnSelf());
        $this->urlRewriteBuilder->expects($this->any())->method('setRedirectCode')->with($redirectType)
            ->will($this->returnSelf());
