...
class Saas_Apps_Helper_DataTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $locale = $this->getMockBuilder('Magento_Core_Model_Locale')->disableOriginalConstructor()->getMock();
        $locale->expects($this->once())->method('getLocaleCode')->will($this->returnValue('en_GB'));

        $config = $this->getMockBuilder('Magento_Core_Model_Config')->disableOriginalConstructor()->getMock();
        ...
        $context = $this->getMockBuilder('Magento_Core_Helper_Context')->disableOriginalConstructor()->getMock();
