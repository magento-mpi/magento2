...
//         WWW: http://www.e-taller.net/lib3d/
//	   Category: Graphics
//    Requires: GD module
// Description: A PHP class for drawing in 3D space coordinates.
//              Has a Chart() function to draw 3D charts
//****************************************************************************
...
class C3DLib {
	...
	function __construct($dx=0, $dy=0, $dflt=1) {
	...
	function mMatrixMultiply($a, $b) {
	    $res = array(array(0,0,0),array(0,0,0),array(0,0,0),array(0,0,0),array(0,0,0),array(0,0,0),array(0,0,0));
	    for($i=0;$i<4;$i++) {
		    for($j=0;$j<4;$j++) {
                for($k=0;$k<4;$k++) {
                    if (isset($a[$i][$k])) {
