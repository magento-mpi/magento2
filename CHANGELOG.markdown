...
* Refactored ACL for the backend
  * ACL resources
    ...
    * Renaming of ACL resource identifiers according to the format `<namespace>_<module>::<resource>` throughout the system
...
* Implemented ability to extends `app/etc/local.xml` by specifying additional config file via `MAGE_LOCAL_CONFIG` environment variable
* Fixed issues:
  * Loosing qty and visibility information when importing products
...
* Refactoring of methods with high cyclomatic complexity:
