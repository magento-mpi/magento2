...
class Structure
{
     ...
     *
     * @param array $elements
     * @throws \Magento\MagentoException if any format issues identified
     */
    public function importElements(array $elements)
    {
        ...
        foreach ($elements as $elementId => $element) {
            if (is_numeric($elementId)) {
                throw new \Magento\MagentoException("Element ID must not be numeric: '{$elementId}'.");
            }
            ...
            if (isset($element[self::GROUPS])) {
                ...
                foreach ($groups as $groupName => $group) {
                    ...
                    if ($group !== array_flip($group)) {
                        throw new \Magento\MagentoException("Invalid format of group '{$groupName}': " . var_export($group, 1));
                    }
    ...
    protected function _assertParentRelation($elementId)
    {
        ...
        if (isset($element[self::CHILDREN])) {
            ...
            if ($children !== array_flip(array_flip($children))) {
                throw new \Magento\MagentoException('Invalid format of children: ' . var_export($children, 1));
            }
     ...
     * @param string $elementId
     * @param array $data
     * @throws \Magento\MagentoException if an element with this id already exists
     */
    public function createElement($elementId, array $data)
    {
        if (isset($this->_elements[$elementId])) {
            throw new \Magento\MagentoException("Element with ID '{$elementId}' already exists.");
        }
     ...
     * @param string $newId
     * @return \Magento\Data\Structure
     * @throws \Magento\MagentoException if trying to overwrite another element
     */
    public function renameElement($oldId, $newId)
    {
        $this->_assertElementExists($oldId);
        if (!$newId || isset($this->_elements[$newId])) {
            throw new \Magento\MagentoException("Element with ID '{$newId}' is already defined.");
        }
     ...
     * @throws \Magento\MagentoException if attempting to set parent as child to its child (recursively)
     */
    public function setAsChild($elementId, $parentId, $alias = '', $position = null)
    {
        if ($elementId == $parentId) {
            throw new \Magento\MagentoException("The '{$elementId}' cannot be set as child to itself.");
        }
        if ($this->_isParentRecursively($elementId, $parentId)) {
            throw new \Magento\MagentoException("The '{$elementId}' is a parent of '{$parentId}' recursively, "
     ...
     * @param string $childId
     * @return int
     * @throws \Magento\MagentoException if specified elements have no parent-child relation
     */
    protected function _getChildOffset($parentId, $childId)
    {
        $index = array_search($childId, array_keys($this->getChildren($parentId)));
        if (false === $index) {
            throw new \Magento\MagentoException("The '{$childId}' is not a child of '{$parentId}'.");
        }
        return $index;
    }
    ...
    protected function _insertChild($targetParentId, $elementId, $offset, $alias)
    {
        ...
        if (isset($children[$elementId])) {
            throw new \Magento\MagentoException("The element '{$elementId}' already a child of '{$targetParentId}'");
        }
        if (false !== array_search($alias, $children)) {
            throw new \Magento\MagentoException("The element '{$targetParentId}' already has a child with alias '{$alias}'");
        }
     ...
     *
     * @param string $elementId
     * @throws \Magento\MagentoException if doesn't exist
     */
    private function _assertElementExists($elementId)
    {
        if (!isset($this->_elements[$elementId])) {
            throw new \Magento\MagentoException("No element found with ID '{$elementId}'.");
        }
    }
    ...
    {
        if (!is_array($value)) {
            throw new \Magento\MagentoException("An array expected: " . var_export($value, 1));
        }
    }
}
