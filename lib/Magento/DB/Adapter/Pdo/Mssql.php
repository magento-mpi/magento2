 ...
 */
class Magento_DB_Adapter_Pdo_Mssql extends Zend_Db_Adapter_Pdo_Mssql
    implements Magento_DB_Adapter_Interface
{
     ...
     * @var string
     */
    protected $_defaultStmtClass = 'Magento_DB_Statement_Pdo_Mssql';
    ...
    protected function _getDefaultValue($options)
    {
        ...
        if ($ddlType == Magento_DB_Ddl_Table::TYPE_TIMESTAMP) {
            ...
            } else if ($cDefault == Magento_DB_Ddl_Table::TIMESTAMP_UPDATE) {
                $cDefault = new Zend_Db_Expr('/*0 ON UPDATE CURRENT_TIMESTAMP*/');
            } else if ($cDefault == Magento_DB_Ddl_Table::TIMESTAMP_INIT_UPDATE) {
     ...
     * @return Magento_DB_Adapter_Pdo_Mssql
     */
     ...
     * @return Magento_DB_Adapter_Pdo_Mssql
     */
    protected function _createIndexes(Magento_DB_Ddl_Table $table)
    {
        $indexes = $table->getIndexes();

        if (!empty($indexes)) {
            foreach ($indexes as $indexData) {
                if (strtolower($indexData['TYPE']) == Magento_DB_Adapter_Interface::INDEX_TYPE_UNIQUE) {
                    continue;
                }
                $columns = array();
    ...
    protected function _getUniqueConstraintsDefinition(Magento_DB_Ddl_Table $table)
    {
        ...
        if (!empty($constraints)) {
            foreach ($constraints as $constraintData) {
                if (strtolower($constraintData['TYPE']) != Magento_DB_Adapter_Interface::INDEX_TYPE_UNIQUE) {
                    continue;
                }
                $columns = array();
    ...
    public function truncateTable($tableName, $schemaName = null)
    {
        if (!$this->isTableExists($tableName, $schemaName)) {
            throw new Magento_Exception(sprintf('Table "%s" is not exists', $tableName));
        }
    ...
    public function getColumnCreateByDescribe($columnData)
    {
        ...
        if ($columnData['NULLABLE'] === false
            && !($type == Magento_DB_Ddl_Table::TYPE_TEXT && strlen($columnData['DEFAULT']) != 0)
        ) {
        ...
        if (($columnData['DEFAULT'] !== null) && $type != Magento_DB_Ddl_Table::TYPE_TEXT) {
    ...
    public function createTableByDdl($tableName, $newTableName)
    {
        ...
        foreach ($foreignKeys as $keyData) {
            ...
            if ($keyData['ON_DELETE'] == 'CASCADE') {
                $onDelete = Magento_DB_Ddl_Table::ACTION_CASCADE;
            } else if ($keyData['ON_DELETE'] == 'SET NULL') {
                $onDelete = Magento_DB_Ddl_Table::ACTION_SET_NULL;
            } else if ($keyData['ON_DELETE'] == 'RESTRICT') {
                $onDelete = Magento_DB_Ddl_Table::ACTION_RESTRICT;
            } else {
                $onDelete = Magento_DB_Ddl_Table::ACTION_NO_ACTION;
            }

            $onUpdate = '';
            if ($keyData['ON_UPDATE'] == 'CASCADE') {
                $onUpdate = Magento_DB_Ddl_Table::ACTION_CASCADE;
            } else if ($keyData['ON_UPDATE'] == 'SET NULL') {
                $onUpdate = Magento_DB_Ddl_Table::ACTION_SET_NULL;
            } else if ($keyData['ON_UPDATE'] == 'RESTRICT') {
                $onUpdate = Magento_DB_Ddl_Table::ACTION_RESTRICT;
            } else {
                $onUpdate = Magento_DB_Ddl_Table::ACTION_NO_ACTION;
            }
    ...
    protected function _getColumnTypeByDdl($column)
    {
        switch ($column['DATA_TYPE']) {
            case 'bit':
                return Magento_DB_Ddl_Table::TYPE_BOOLEAN;
            case 'int':
            ...
            case 'decimal':
                return Magento_DB_Ddl_Table::TYPE_DECIMAL;
            case 'float':
                return Magento_DB_Ddl_Table::TYPE_FLOAT;
            case 'bigint':
                return Magento_DB_Ddl_Table::TYPE_BIGINT;
            case 'smallint':
                return Magento_DB_Ddl_Table::TYPE_SMALLINT;
            case 'varbinary':
    ...
    protected function _getDdlScriptCreateFullText($tableName, $fields, $schemaName = null)
    {
        ...
        foreach ($this->getIndexList($tableName, $schemaName) as $index) {
            if ($index['INDEX_TYPE'] == Magento_DB_Adapter_Interface::INDEX_TYPE_PRIMARY) {
    ...
    public function purgeOrphanRecords($tableName, $columnName, $refTableName,
        $refColumnName, $onDelete = Magento_DB_Adapter_Interface::FK_ACTION_CASCADE)
    {
        ...
        $sql = '';
        if (strtoupper($onDelete) == Magento_DB_Adapter_Interface::FK_ACTION_CASCADE
            || strtoupper($onDelete) == Magento_DB_Adapter_Interface::FK_ACTION_RESTRICT
        ) {
    ...
    public function addForeignKey($fkName, $tableName, $columnName, $refTableName, $refColumnName,
        $onDelete = Magento_DB_Adapter_Interface::FK_ACTION_CASCADE,
        $onUpdate = Magento_DB_Adapter_Interface::FK_ACTION_CASCADE,
    ...
    protected function _getUniqueIndexColumns($tableName, $schemaName = null)
    {
        ...
        foreach ($this->getIndexList($tableName, $schemaName) as $index) {
            if ($index['INDEX_TYPE'] == Magento_DB_Adapter_Interface::INDEX_TYPE_UNIQUE) {
                foreach ($index['COLUMNS_LIST'] as $value) {
     ...
     *
     * @param string|Magento_DB_Select $sql
     * @param string $tableName
    ...
    protected function _wrapEnableIdentityDataInsert($sql, $tableName, array $updatingColumns)
    {
        ...
        if (count(array_intersect($identityColumns, $updatingColumns))) {
            ...
            $sql = sprintf(Magento_DB_Adapter_Pdo_Mssql::SET_IDENTITY_INSERT_ON, $quotedTableName) . "\n"
                . $sql . "\n;"
                . sprintf(Magento_DB_Adapter_Pdo_Mssql::SET_IDENTITY_INSERT_OFF, $quotedTableName);
        }
    ...
    public function insertForce($table, array $bind)
    {
        ...
        $this->query(sprintf(Magento_DB_Adapter_Pdo_Mssql::SET_IDENTITY_INSERT_ON, $this->quoteIdentifier($table)));
        $result = parent::insert($table, $bind);
        $this->query(sprintf(Magento_DB_Adapter_Pdo_Mssql::SET_IDENTITY_INSERT_OFF, $this->quoteIdentifier($table)));
        return $result;
    }
    ...
    protected function _embedBlobs(&$sql, &$bind = array())
    {
        ...
        if (is_array($bind)) {
            ...
            foreach ($bind as $param) {
                if (($param instanceof Magento_DB_Statement_Parameter) && $param->getIsBlob()) {
                    $hasBlobs = true;
                    break;
                }
            }
        } else {
            $hasBlobs = ($bind instanceof Magento_DB_Statement_Parameter) && $bind->getIsBlob();
        }
        if (!$hasBlobs) {
            return $this;
        }
        ...
        foreach ($bind as $key => $val) {
            ...
            if (($val instanceof Magento_DB_Statement_Parameter) && ($val->getIsBlob())) {
    ...
    protected function _addForeignKeyDeleteAction($tableName, $columnName, $refTableName, $refColumnName, $fkAction)
    {
        ...
        if ($tableName == $refTableName) {
            ...
            if ($fkAction == Magento_DB_Ddl_Table::ACTION_CASCADE) {
    ...
    public function getIndexName($tableName, $fields, $indexType = '')
    {
        ...
        if (strlen($hash) + strlen($prefix) > self::LENGTH_INDEX_NAME) {
            $short = Magento_DB_Helper::shortName($prefix . $hash);
    ...
    public function disableTableKeys($tableName, $schemaName = null)
    {
        ...
        foreach ($indexList as $indexProp) {
            if ($indexProp['INDEX_TYPE'] != Magento_DB_Adapter_Interface::INDEX_TYPE_INDEX) {
                continue;
            }
    ...
    public function insertFromSelect(Magento_DB_Select $select, $table, array $fields = array(), $mode = false)
    {
        ...
        // UPDATE Section
        if ($mode == Magento_DB_Adapter_Interface::INSERT_ON_DUPLICATE && $updateCols) {
            $updateCond = array();
