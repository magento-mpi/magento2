 ...
 */
class Magento_DB_Adapter_Oracle extends Zend_Db_Adapter_Oracle implements Magento_DB_Adapter_Interface
{
     ...
     * @var string
     */
    protected $_defaultStmtClass = 'Magento_DB_Statement_Oracle';
     ...
     *
     * @return Magento_DB_Adapter_Oracle
     */
    public function beginTransaction()
    {
        if ($this->_isRolledBack) {
            throw new Exception(Magento_DB_Adapter_Interface::ERROR_ROLLBACK_INCOMPLETE_MESSAGE);
        }
    ...
    public function commit()
    {
        if ($this->_transactionLevel == 1 && !$this->_isRolledBack) {
            ...
            throw new Exception(Magento_DB_Adapter_Interface::ERROR_ASYMMETRIC_COMMIT_MESSAGE);
    ...
    public function rollBack()
    {
        if ($this->_transactionLevel == 1) {
            ...
            throw new Exception(Magento_DB_Adapter_Interface::ERROR_ASYMMETRIC_ROLLBACK_MESSAGE);
        } else {
    ...
    protected function _checkDdlTransaction($sql)
    {
        if (is_string($sql) && $this->getTransactionLevel() > 0) {
            ...
            if (in_array($startSql, $this->_ddlRoutines)) {
                trigger_error(Magento_DB_Adapter_Interface::ERROR_DDL_MESSAGE, E_USER_ERROR);
            }
        }
    }
    ...
    protected function _prepareQuery(&$sql, &$bind = array())
    {
        ...
        $sql = preg_replace_callback('#(([\'"])((\\2)|((.*?[^\\\\])\\2)))#',
            ...
            $sql);
        Magento_Exception::processPcreError();
    ...
    {
        $table = new Magento_DB_Ddl_Table();
        if ($tableName !== null) {
    ...
    /**
     * Create Magento_DB_Ddl_Table object by data from describe table
     *
     * @param string $tableName
    ...
    public function createTableByDdl($tableName, $newTableName)
    {
        ...
        foreach ($indexes as $indexData) {
            if ($indexData['INDEX_TYPE'] == Magento_DB_Adapter_Interface::INDEX_TYPE_PRIMARY) {
                continue;
            }
            ...
            $options = array();
            $indexType = '';
            if ($indexData['INDEX_TYPE'] == Magento_DB_Adapter_Interface::INDEX_TYPE_UNIQUE) {
                $options = array('type' => Magento_DB_Adapter_Interface::INDEX_TYPE_UNIQUE);
                $indexType = Magento_DB_Adapter_Interface::INDEX_TYPE_UNIQUE;
            }
    ...
    {
        switch ($action) {
            case Magento_DB_Adapter_Interface::FK_ACTION_CASCADE:
                return Magento_DB_Ddl_Table::ACTION_CASCADE;
            case Magento_DB_Adapter_Interface::FK_ACTION_SET_NULL:
                return Magento_DB_Ddl_Table::ACTION_SET_NULL;
            case Magento_DB_Adapter_Interface::FK_ACTION_RESTRICT:
                return Magento_DB_Ddl_Table::ACTION_RESTRICT;
            default:
                return Magento_DB_Ddl_Table::ACTION_NO_ACTION;
        }
    }
    ...
    protected function _getColumnTypeByDdl($column)
    {
        ...
        switch ($columnDataType) {
            case 'INTEGER':
                return Magento_DB_Ddl_Table::TYPE_INTEGER;
            case 'VARCHAR2':
                return Magento_DB_Ddl_Table::TYPE_TEXT;
            case 'CLOB':
                if ($column['LENGTH'] <= 4000) {
                    return Magento_DB_Ddl_Table::TYPE_TEXT; // Just to justify a little from blobs
                }
                return Magento_DB_Ddl_Table::TYPE_BLOB;
            case 'BLOB':
                return Magento_DB_Ddl_Table::TYPE_VARBINARY;
            case 'TIMESTAMP':
                return Magento_DB_Ddl_Table::TYPE_TIMESTAMP;
            case 'NUMBER':
                if(array_key_exists('SCALE', $column) && $column['SCALE'] > 0) {
                    return Magento_DB_Ddl_Table::TYPE_NUMERIC;
                } else {
                    return Magento_DB_Ddl_Table::TYPE_BIGINT;
                }
            case 'DATE':
                return Magento_DB_Ddl_Table::TYPE_DATE;
            case 'FLOAT':
                return Magento_DB_Ddl_Table::TYPE_FLOAT;
        }
    }
    ...
    public function addColumn($tableName, $columnName, $definition, $schemaName = null)
    {
        ...
        if (is_array($definition)) {
            ...
            if (!empty($definition['DEFAULT']) && $definition['TYPE'] == Magento_DB_Ddl_Table::TYPE_TIMESTAMP) {
                if ($definition['DEFAULT']== Magento_DB_Ddl_Table::TIMESTAMP_UPDATE
                    || $definition['DEFAULT'] == Magento_DB_Ddl_Table::TIMESTAMP_INIT_UPDATE
                ) {
    ...
    protected function _addIndex($tableName, $indexName, $fields,
        $indexType = Magento_DB_Adapter_Interface::INDEX_TYPE_INDEX, $schemaName = null, $params = array())
    {
        if ($schemaName === null) {
            $schemaName = $this->_getSchemaName();
        }
        ...
        switch (strtolower($indexType)) {
            case Magento_DB_Adapter_Interface::INDEX_TYPE_PRIMARY:
                $condition = 'ALTER TABLE %1$s ADD CONSTRAINT "%2$s" PRIMARY KEY (%3$s)';
                break;
            case Magento_DB_Adapter_Interface::INDEX_TYPE_UNIQUE:
                $condition = 'ALTER TABLE %1$s ADD CONSTRAINT "%2$s" UNIQUE (%3$s)';
                break;
            case Magento_DB_Adapter_Interface::INDEX_TYPE_FULLTEXT:
    ...
    public function addIndex($tableName, $indexName, $fields,
        $indexType = Magento_DB_Adapter_Interface::INDEX_TYPE_INDEX, $schemaName = null)
    {
    ...
    public function getIndexList($tableName, $schemaName = null)
    {
        ...
        if ($ddl === false) {
            ...
            $casesResults  = array(
                "uc.constraint_type = 'P'" => "'" . Magento_DB_Adapter_Interface::INDEX_TYPE_PRIMARY . "'",
            ...
            $defaultValue  = "'" . Magento_DB_Adapter_Interface::INDEX_TYPE_INDEX . "'";
            ...
            foreach ($rowset as $row) {
                ...
                switch (strtolower($indexType)) {
                    case Magento_DB_Adapter_Interface::INDEX_TYPE_PRIMARY:
                          $upperKeyName = strtoupper(Magento_DB_Adapter_Interface::INDEX_TYPE_PRIMARY);
                          break;
                    default:
...
//        }
     ...
     * @param string $refColumnName
     * @param string $onDelete
     * @return Magento_DB_Adapter_Pdo_Oracle
     */
    public function purgeOrphanRecords($tableName, $columnName, $refTableName, $refColumnName, $onDelete = 'cascade')
    {
        ...
        if (strtoupper($onDelete) == Magento_DB_Ddl_Table::ACTION_CASCADE
            || strtoupper($onDelete) == Magento_DB_Ddl_Table::ACTION_RESTRICT
        ) {
            $sql = " UPDATE {$tableName} t1 SET t1.code = NULL ";
        } else if (strtoupper($onDelete) == Magento_DB_Ddl_Table::ACTION_SET_NULL) {
    ...
    public function dropForeignKey($tableName, $fkName, $schemaName = null)
    {
        ...
        $fkActions = array(Magento_DB_Ddl_Table::ACTION_CASCADE, Magento_DB_Ddl_Table::ACTION_SET_NULL);
        ...
        if (is_array($row)) { // Array of column-value pairs
            ...
            foreach ($data as $row) {
                ...
                if (array_diff($cols, array_keys($row))) {
                    throw new Magento_Exception('Invalid data for insert');
                }
    ...
    public function insertOnDuplicateCompatible($table, array $data, array $fields = array())
    {
        ...
        if ($fields) {
            $query = sprintf("BEGIN\n %s;\n %s;\n END;",
                $query,
                sprintf(
                    'UPDATE %s t1 SET (%s)= (SELECT %s FROM (%s) t2 WHERE %s)'
    ...
    public function insertArray($table, array $columns, array $data)
    {
        ...
        $ddlColumnTypeBlob = $this->_ddlColumnTypes[Magento_DB_Ddl_Table::TYPE_BLOB];
        $ddlColumnTypeVarbinary = $this->_ddlColumnTypes[Magento_DB_Ddl_Table::TYPE_VARBINARY];
        foreach ($data as $row) {
            $row = (array)$row;
    ...
    {
        $date = Magento_Date::formatDate($date, $includeTime);
    ...
    public function prepareColumnValue(array $column, $value)
    {
        ...
        if ($value instanceof Magento_DB_Statement_Parameter) {
            return $value;
        }
        ...
        switch ($column['DATA_TYPE']) {
            ...
            case 'BLOB':
                ...
                if ($column['NULLABLE'] && $value === '') {
                    ...
                    $value = new Magento_DB_Statement_Parameter($value);
    ...
    protected function _getIntervalUnitSql($interval, $unit)
    {
        if (!isset($this->_intervalUnits[$unit])) {
            throw new Magento_DB_Exception(sprintf('Undefined interval unit "%s" specified', $unit));
        }
    ...
    {
        if (is_null($len)) {
            return new Zend_Db_Expr(sprintf('SUBSTR(%s, %s)', $stringExpression, $pos));
        }
        return new Zend_Db_Expr(sprintf('SUBSTR(%s, %s, %s)', $stringExpression, $pos, $len));
    }
    ...
    {
        if (!$this->isTableExists($tableName, $schemaName)) {
            throw new Magento_Exception(sprintf('The table "%s" is not exists', $tableName));
        }
    ...
    protected function _debugStat($type, $sql, $bind = array(), $result = null)
    {
        ...
        if ($this->_logCallStack) {
            $code .= 'TRACE: ' . Magento_Debug::backtrace(true, false) . $nl;
        }
    ...
    protected function _getTriggerName($tableName, $fieldName, $triggerType = self::TRIGGER_IDENTITY)
    {
        ...
        if (strlen($hash) > 30) {
            $short = Magento_DB_Helper::shortName($hash);
            if (strlen($short) > 30) {
    ...
    protected function _debugWriteToFile($str)
    {
        ...
        if (!$this->_debugIoAdapter) {
            $this->_debugIoAdapter = new Magento_Io_File();
    ...
    protected function _getColumnDefinition($options, $ddlType = null)
    {
        ...
        if (empty($ddlType) || !isset($this->_ddlColumnTypes[$ddlType])) {
            throw new Magento_Exception('Invalid column definition data');
        }
        ...
        switch ($ddlType) {
            ...
            case Magento_DB_Ddl_Table::TYPE_NUMERIC:
                $precision  = 10;
                $scale      = 0;
                $match      = array();
            ...
            case Magento_DB_Ddl_Table::TYPE_BLOB:
            case Magento_DB_Ddl_Table::TYPE_VARBINARY:
                if (empty($options['LENGTH'])) {
                    $options['LENGTH'] = Magento_DB_Ddl_Table::DEFAULT_TEXT_SIZE;
                } else {
                    $options['LENGTH'] = $this->_parseTextSize($options['LENGTH']);
                }
                if ($ddlType != Magento_DB_Ddl_Table::TYPE_VARBINARY) {
        ...
        if ($ddlType == Magento_DB_Ddl_Table::TYPE_TIMESTAMP) {
            if ($cDefault === null) {
                $cDefault = new Zend_Db_Expr('NULL');
            } elseif ($cDefault == Magento_DB_Ddl_Table::TIMESTAMP_INIT) {
                $cDefault = new Zend_Db_Expr('sysdate');
            } elseif ($cDefault == Magento_DB_Ddl_Table::TIMESTAMP_UPDATE) {
                $cDefault = new Zend_Db_Expr('/*0 ON UPDATE CURRENT_TIMESTAMP*/');
            } elseif ($cDefault == Magento_DB_Ddl_Table::TIMESTAMP_INIT_UPDATE) {
     ...
     *
     * @param Magento_DB_Ddl_Table $table
     * @return array
     */
    protected function _getColumnsDefinition(Magento_DB_Ddl_Table $table)
    {
        $definition = array();
        $primary    = array();
     ...
     * @return boolean
     */
    protected function _createForeignKeysActions(Magento_DB_Ddl_Table $table)
    {
    ...
    protected function _createSequence($tableName)
    {
        ...
        if (!$this->isSequenceExists($sequenceName)) {
            $query = 'CREATE SEQUENCE "%s" MINVALUE 0 MAXVALUE 4294967296'
     ...
     * @return Magento_DB_Adapter_Oracle
     */
    protected function _createIndexes(Magento_DB_Ddl_Table $table, $params = array())
    {
        // Create indexes
        ...
        foreach ($indexes as $indexData) {
            if (strtoupper($indexData['INDEX_NAME']) == 'PRIMARY') {
                $indexType = Magento_DB_Adapter_Interface::INDEX_TYPE_PRIMARY;
            } else {
                $indexType = $indexData['TYPE'];
            }

            if ($indexType == Magento_DB_Adapter_Interface::INDEX_TYPE_UNIQUE) {
                continue;
            }
     ...
     * @return array
     */
    protected function _getUniqueConstraintsDefinition(Magento_DB_Ddl_Table $table)
    {
        $definition  = array();
        ...
        if (!empty($constraints)) {
            foreach ($constraints as $constraintData) {
                if ($constraintData['TYPE'] != Magento_DB_Adapter_Interface::INDEX_TYPE_UNIQUE) {
                    continue;
                }
                $columns = array();
     ...
     * @return array
     */
    protected function _getForeignKeysDefinition(Magento_DB_Ddl_Table $table)
    {
        $onDeleteAction = '';
        ...
        if (!empty($relations)) {
            foreach ($relations as $fkData) {
                ...
                switch ($fkData['ON_DELETE']) {
                    case Magento_DB_Ddl_Table::ACTION_CASCADE:
                    case Magento_DB_Ddl_Table::ACTION_RESTRICT:
                    case Magento_DB_Ddl_Table::ACTION_SET_NULL:
     ...
     * @return Zend_Db_Statement_Interface
     */
    public function createTable(Magento_DB_Ddl_Table $table)
    {
        $columns = $table->getColumns();
        ...
        foreach ($columns as $columnData) {
            ...
            if (array_key_exists('DEFAULT', $columnData)) {
                ...
                if ($columnData['COLUMN_TYPE'] == Magento_DB_Ddl_Table::TYPE_TIMESTAMP) {
                    if ($cDefault == Magento_DB_Ddl_Table::TIMESTAMP_UPDATE |
                        $cDefault == Magento_DB_Ddl_Table::TIMESTAMP_INIT_UPDATE) {
    ...
    public function getTableName($tableName)
    {
        ...
        if (strlen($tableName) > self::LENGTH_TABLE_NAME) {
            $shortName = Magento_DB_Helper::shortName($tableName);
    ...
    public function getIndexName($tableName, $fields, $indexType = '')
    {
        ...
        switch ($indexType) {
            ...
            case Magento_DB_Adapter_Interface::INDEX_TYPE_FULLTEXT:
                $prefix = 'fti_';
                break;
            case Magento_DB_Adapter_Interface::INDEX_TYPE_INDEX:
            default:
                $prefix = 'idx_';
        }
         ...
         */
        if ($indexType == Magento_DB_Adapter_Interface::INDEX_TYPE_FULLTEXT) {
    ...
    {
        $updateValue = ($fkAction == Magento_DB_Ddl_Table::ACTION_CASCADE ? "s_new(i)" : "NULL");
        /*
         * Create package
         */
    ...
    public function disableTableKeys($tableName, $schemaName = null)
    {
        ...
        foreach ($indexList as $indexProp) {
            if (strtolower($indexProp['INDEX_TYPE']) != Magento_DB_Adapter_Interface::INDEX_TYPE_INDEX) {
                continue;
            }
     ...
     * @return string
     */
    public function insertFromSelect(Magento_DB_Select $select, $table, array $fields = array(), $mode = false)
    {
        if (!$mode) {
        ...
        if (count($colsPart) != count($fields)) {
            throw new Magento_DB_Exception('Wrong columns count in SELECT for INSERT');
        }
        ...
        if (empty($whereCond)) {
            throw new Magento_DB_Exception('Invalid primary or unique columns in merge data');
        }
     ...
     * @throws Zend_Db_Exception
     */
    public function insertFromSelectCompatible(Magento_DB_Select $select, $table, array $fields = array(), $mode = false)
    {
        if (!$mode) {
            return $this->_getInsertFromSelectSql($select, $table, $fields);
        }
     ...
     * @return string
     */
    protected function _getInsertFromSelectSql(Magento_DB_Select $select, $table, array $fields = array())
    {
     ...
     * @return string
     */
    public function updateFromSelect(Magento_DB_Select $select, $table)
    {
        if (!is_array($table)) {
        ...
        if (!$fromConds) {
            throw new Magento_DB_Exception('Invalid SELECT object for update data from SELECT');
        }
     ...
     * @return string|int
     */
    public function deleteFromSelect(Magento_DB_Select $select, $table)
    {
        // clone select
        $select = clone $select;
    ...
    {
        return sprintf('%s %s', $sql, Magento_DB_Adapter_Oracle::SQL_FOR_UPDATE);
    }
    ...
    protected function _parseTextSize($size)
    {
        ...
        if (empty($size)) {
            return Magento_DB_Ddl_Table::DEFAULT_TEXT_SIZE;
        }
        if ($size >= Magento_DB_Ddl_Table::MAX_TEXT_SIZE) {
            return Magento_DB_Ddl_Table::MAX_TEXT_SIZE;
        }
    ...
    /**
     * Get lob (clob/blob) function depending on the field ddl type
     *
    ...
    {
        if ($ddlType == $this->_ddlColumnTypes[Magento_DB_Ddl_Table::TYPE_VARBINARY]) {
            return 'to_blob';
        }
    ...
    /**
     * Quote field to query depending on the field ddl type
     *
     * @param array $ddl
     * @param string $key
     * @param string $col
    ...
    {
        if ($ddlType == $this->_ddlColumnTypes[Magento_DB_Ddl_Table::TYPE_BLOB]
            || $ddlType == $this->_ddlColumnTypes[Magento_DB_Ddl_Table::TYPE_VARBINARY]
        ) {
