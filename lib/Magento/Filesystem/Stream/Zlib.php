 ...
 */
class Magento_Filesystem_Stream_Zlib implements Magento_Filesystem_StreamInterface
{
    /**
    ...
    {
        $fileHandle = @gzopen($this->_path, $mode->getMode());
        if (false === $fileHandle) {
    ...
    public function read($count)
    {
        ...
        $result = @gzread($this->_fileHandle, $count);
        if ($result === false) {
    ...
    {
        throw new Magento_Filesystem_Exception('Zlib does not support csv.');
    }
    ...
    public function write($data)
    {
        ...
        $result = @gzwrite($this->_fileHandle, $data);
        if (false === $result) {
    ...
    {
        $this->_assertOpened();
        $result = @gzclose($this->_fileHandle);
    ...
    {
        $this->_assertOpened();
        $result = gzseek($this->_fileHandle, $offset, $whence);
        if (0 !== $result) {
    ...
    {
        $this->_assertOpened();
        $result = gztell($this->_fileHandle);
        if (false === $result) {
    ...
    {
        $this->_assertOpened();
        return (bool)@gzeof($this->_fileHandle);
    }
