...
class Compiled implements \Magento\Framework\ObjectManager\Factory
{
    /**
     * Object manager
     *
    ...
    public function create($requestedType, array $arguments = array())
    {
        ...
        if (substr($type, -12) == '\Interceptor') {
            $args = array_merge([$this->objectManager, $this->objectManager->get('Magento\Framework\Interception\PluginList'), $this->objectManager->get('Magento\Framework\Interception\Chain')], $args);
        }
        switch (count($args)) {
            case 1:
