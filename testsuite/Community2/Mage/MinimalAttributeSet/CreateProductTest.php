...
class Core_Mage_Product_Create_SimpleTest extends Mage_Selenium_TestCase
{
    ...
    public function basedOnMinimalSetSimple()
    {
        //Data
        ...
        $this->ProductHelper()->fillProductSettings($productData, $productType);
        if ($productType == 'configurable') {
            $this->productHelper()->fillConfigurableSettings($productData);
        }
        $this->productHelper()->fillProductTab($productData, 'general_min');
        $this->saveForm('save');
        //$this->productHelper()->createProduct($productData);
        //Verifying
        $this->assertMessagePresent('success', 'success_saved_product');
