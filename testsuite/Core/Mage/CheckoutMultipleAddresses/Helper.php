...
class Core_Mage_CheckoutMultipleAddresses_Helper extends Mage_Selenium_TestCase
{
     ...
     */
    public function frontMultipleCheckout($checkoutData, $placeOrder = true)
    {
    ...
    public function frontDoMultipleCheckoutSteps(array $checkoutData)
    {
        ...
        if ($generalShippingAddress) {
            ...
            if ($currentPage == 'checkout_multishipping_new_shipping' ||
        ...
        if ($shipping) {
            ...
            foreach ($shipping as $value) {
                ...
                if (isset($value['product_name'])) {
                    $this->addParameter('productName', $value['product_name']);
                    $type = 'exist';
                }
     ...
     * @param bool $proceedToBilling
     */
    public function fillShippingInfo(array $shipInfo, $proceedToBilling = true)
    {
        ...
        switch ($addressChoice) {
            ...
            case 'exist':
                foreach ($addressData as $key => $value) {
                    $shippingData['shipping_' . $key] = $value;
                }
                $addressLine = $this->orderHelper()->defineAddressToChoose($shippingData, 'shipping');
    ...
    public function frontOrderReview(array $checkoutData)
    {
        ...
        if (isset($checkoutData['verify_products_data'])) {
            foreach ($checkoutData['verify_products_data'] as $data) {
                ...
                if (($addressToSearch == null) && $checkTotal) { //Verification of grand total for all addresses
                    $grandTotalXpath = $this->_getControlXpath('pageelement', 'grand_total');
                    $actualGrandTotal = $this->getText($grandTotalXpath);
                    $this->shoppingCartHelper()->compareArrays(
                                                    array('$' . $checkTotal['grand_total']), array($actualGrandTotal));
                    $this->assertEmptyVerificationErrors();
                }
            }
        }
