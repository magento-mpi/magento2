...
class ProductAttribute_Create_PriceTest extends Mage_Selenium_TestCase
{
    ...
    /**
     * Create "Price" type Product Attribute (required fields only)
     *
     * Steps:
     * 1.Click on "Add New Attribute" button
     * 2.Choose "Price" in 'Catalog Input Type for Store Owner' dropdown
     * 3.Fill all required fields
     * 4.Click on "Save Attribute" button
     *
     * Expected result:
     * New attribute ["Price" type] successfully created.
    ...
    {
        //Data
        $attrData = $this->loadData('product_attribute_price', null,
     ...
     *
     * Expected result:
     * New attribute ["Price" type] shouldn't be created.
    ...
    public function test_WithRequiredFieldsEmpty($emptyField)
    {
        ...
        if (!array_key_exists('attribute_code', $emptyField)) {
            $attrData = $this->loadData('product_attribute_price', $emptyField, 'attribute_code');
        } else {
            $attrData = $this->loadData('product_attribute_price', $emptyField);
        }
        //Steps
        $this->productAttributeHelper()->createAttribute($attrData);
        //Verifying
    ...
    {
        //Data
        $attrData = $this->loadData('product_attribute_price', $wrongAttributeCode);
        //Steps
        $this->productAttributeHelper()->createAttribute($attrData);
        //Verifying
    ...
    {
        //Data
        $attrData = $this->loadData('product_attribute_price', $invalidPosition, 'attribute_code');
        //Steps
        $this->productAttributeHelper()->createAttribute($attrData);
        //Verifying
     ...
     * 6.Choose "Price" in 'Catalog Input Type for Store Owner' dropdown
