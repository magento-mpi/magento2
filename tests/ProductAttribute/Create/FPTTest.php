...
class ProductAttribute_Create_FPTTest extends Mage_Selenium_TestCase
{
    ...
    public function withRequiredFieldsEmpty($emptyField)
    {
        ...
        if ($emptyField == 'apply_to') {
            $attrData = $this->loadData('product_attribute_fpt', array($emptyField => 'Selected Product Types'),
                    'attribute_code');
        } else {
            $attrData = $this->loadData('product_attribute_fpt', array($emptyField => '%noValue%'), 'attribute_code');
        }
        //Steps
        $this->productAttributeHelper()->createAttribute($attrData);
        //Verifying
    ...
    {
        //Data
        $attrData = $this->loadData('product_attribute_fpt', array('attribute_code' => $wrongAttributeCode));
        //Steps
        $this->productAttributeHelper()->createAttribute($attrData);
        //Verifying
