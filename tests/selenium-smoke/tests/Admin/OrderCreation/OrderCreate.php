...
class Test_Admin_OrderCreation_OrderCreate extends Test_Admin_OrderCreation_Abstract {
	...
	*/
	function testAdminOrderWorkFlow() {
		...
		$this->doCreateOrder($this->_userEmail, $this->_storeviewName, $this->_productSKU);
		if(!$this->isElementPresent($this->getUiElement("admin/pages/sales/orders/createOrderPage/messages/orderCreated"),10)) {
			return FALSE;
		} else {
		$ordNum = $this->getText($this->getUiElement("admin/pages/sales/orders/editOrderPage/pageElements/orderNumber"));
		...
		$ordNum = substr($ordNum, 8, 10);
		$this->clickAndWait($this->getUiElement("admin/topmenu/sales/orders"));
		$ordStatus = $this->getText($this->getUiElement("admin/pages/sales/orders/manageOrdersPage/pageElements/orderStatus",$ordNum));
		$this->orderStatus($ordNum, $this->orderStatus[1]);
		}
	}
}
