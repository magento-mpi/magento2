...
define('__FILE', BP . DS . 'app' . DS . 'Mage.php');
...
final class Mage {
    ...
    /**
     * Prepare mocking for a factory for specified group (model, block etc)
     *
     * Right now only model group is supported
     *
     * @param string $group
     * @param string $factoryClassName
     * @param PHPUnit_Framework_TestCase $testCaseInstance
     * @param array $methods
     * @param array $arguments
     ...
     */
    public static function mockFactory($group, $factoryClassName, PHPUnit_Framework_TestCase $testCaseInstance, $methods = array(), array $arguments = array(), $mockClassName = '', $callOriginalConstructor = TRUE, $callOriginalClone = TRUE, $callAutoload = TRUE)
    {
        self::$factoryMocks[$group][$factoryClassName] = array(
            $testCaseInstance, $methods, $arguments, $mockClassName, $callOriginalConstructor, $callOriginalClone, $callAutoload
        );
    }

    /**
     * Discard mocking for a factory
     *
     * @param string $group
     * @param string $factoryClassName
     */
    public static function unmockFactory($group, $factoryClassName)
    {
        if (isset(self::$factoryMocks[$group])) {
            unset(self::$factoryMocks[$group][$factoryClassName]);
        }
    }
}
