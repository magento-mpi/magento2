...
/**
 * Credit Memo for Order (Check)
 *
 * @package     selenium
 * @subpackage  tests
 * @license     http://opensource.org/licenses/osl-3.0.php  Open Software License (OSL 3.0)
 */
class OrderCreditMemo_CreateWithCheckTest extends Mage_Selenium_TestCase
{
  ...
  /**
   ...
   public function setUpBeforeTests()
    ...
    {
        $orderData = $this->loadData('order_req_partial_creditmemo_check',
                array('filter_sku' => $productData['general_sku']));
        $orderData['account_data']['customer_email'] = $this->generate('email', 32, 'valid');
        $this->navigate('manage_sales_orders');
        $this->assertTrue($this->checkCurrentPage('manage_sales_orders'), $this->messages);
