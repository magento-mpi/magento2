        ...
        try {
            ...
            if (!file_exists($logPath) || !is_dir($logPath)) {
                if (!mkdir($logPath, 0777, true)) {
                    Mage::throwException(Mage::helper('Enterprise_ImportExport_Helper_Data')->__("We couldn't create directory " . '"%1"', $logPath));
                }
            }

            if (!is_dir($logPath) || !is_writable($logPath)) {
                Mage::throwException(Mage::helper('Enterprise_ImportExport_Helper_Data')->__('The directory "%1" is not writable.', $logPath));
            }
            ...
            foreach ($this->_getDirectoryList($logPath) as $directory) {
                ...
                if ($forceRun || $direcotryDate < $dateCompass) {
                    ...
                    if (!$fs->rmdirRecursive($directory, true)) {
                        ...
                        Mage::throwException(
                            Mage::helper('Enterprise_ImportExport_Helper_Data')->__('We couldn\'t delete "%1" because the directory is not writable.', $directory)
                        );
                    }
                }
            }
