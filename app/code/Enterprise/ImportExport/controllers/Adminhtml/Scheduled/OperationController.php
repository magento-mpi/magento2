    ...
    public function massDeleteAction()
    {
        ...
        if (is_array($ids)) {
            ...
            try {
                ...
                Mage::getSingleton('Mage_Adminhtml_Model_Session')->addSuccess(
                    __('We deleted a total of %s record(s).', count($operations))
                );
            } catch (Mage_Core_Exception $e) {
                Mage::getSingleton('Mage_Adminhtml_Model_Session')->addError($e->getMessage());
            } catch (Exception $e) {
                Mage::logException($e);
                Mage::getSingleton('Mage_Adminhtml_Model_Session')->addError(__('We cannot delete all items.'));
            }
        }
        $this->_redirect('*/scheduled_operation/index');
    }
    ...
    public function massChangeStatusAction()
    {
        ...
        if (is_array($ids)) {
            ...
            try {
                ...
                Mage::getSingleton('Mage_Adminhtml_Model_Session')->addSuccess(
                    __('A total of %s record(s) have been updated.', count($operations))
                );
            } catch (Mage_Core_Exception $e) {
                Mage::getSingleton('Mage_Adminhtml_Model_Session')->addError($e->getMessage());
            } catch (Exception $e) {
