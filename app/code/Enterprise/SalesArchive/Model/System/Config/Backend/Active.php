    ...
    public function getCommentText($currentValue)
    {
        if ($currentValue) {
            $ordersCount = Mage::getResourceSingleton('Enterprise_SalesArchive_Model_Resource_Order_Collection')
                ->getSize();
            if ($ordersCount) {
                return Mage::helper('Enterprise_SalesArchive_Helper_Data')->__('There are %1 orders in this archive. All of them will be moved to the regular table after the archive is disabled.', $ordersCount);
            }
        }
        return '';
    }
