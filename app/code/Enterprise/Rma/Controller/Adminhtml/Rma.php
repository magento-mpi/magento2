...
class Enterprise_Rma_Controller_Adminhtml_Rma extends Magento_Adminhtml_Controller_Action
{
    ...
    public function newAction()
    {
        ...
        if (!$orderId) {
            ...
            try {
                ...
                $this->_initModel();
                if (!$this->_objectManager->get('Enterprise_Rma_Helper_Data')->canCreateRma($orderId, true)) {
    ...
    protected function _getConfigDataJson($model)
    {
        ...
        $carrier = $this->_objectManager->get('Enterprise_Rma_Helper_Data')->getCarrier($code, $model->getStoreId());
        if ($carrier) {
    ...
    protected function _createShippingLabel(Enterprise_Rma_Model_Rma $model)
    {
        ...
        if ($model && isset($data['packages']) && !empty($data['packages'])) {
            ...
            $carrier = $this->_objectManager->get('Enterprise_Rma_Helper_Data')->getCarrier($data['code'], $model->getStoreId());
