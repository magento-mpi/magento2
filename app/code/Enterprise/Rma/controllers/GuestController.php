...
class Enterprise_Rma_GuestController extends Mage_Core_Controller_Front_Action
{
    ...
    public function viewAction()
    {
        ...
        $this->getLayout()
            ->getBlock('head')
            ->setTitle(__('Return #%s', Mage::registry('current_rma')->getIncrementId()));
        $this->renderLayout();
    }
        ...
        if (($post) && !empty($post['items'])) {
            try {
                ...
                Mage::getSingleton('Mage_Core_Model_Session')->addSuccess(
                    __('You submitted Return #%s.', $rmaModel->getIncrementId())
                );
    ...
    protected function _loadOrderItems($orderId)
    {
        ...
        $message = __('We cannot create a return transaction for order #%s.', $incrementId);
