...
class Enterprise_Rma_Adminhtml_RmaController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function saveNewAction()
    {
        ...
        try {
            ...
            Mage::getSingleton('Mage_Adminhtml_Model_Session')->addSuccess(__('You submitted the RMA request.'));
        } catch (Mage_Core_Exception $e) {
            Mage::getSingleton('Mage_Adminhtml_Model_Session')->addError($e->getMessage());
            ...
            Mage::getSingleton('Mage_Adminhtml_Model_Session')->addError(__('We failed to save this RMA.'));
            Mage::logException($e);
        }
        $this->_redirect('*/*/');
    }
    ...
    public function saveAction()
    {
        ...
        try {
            ...
            Mage::getSingleton('Mage_Adminhtml_Model_Session')->addSuccess(__('You saved the RMA request.'));
    ...
    public function closeAction(){
        ...
        if ($countNonCloseRma) {
            if ($countCloseRma) {
                $this->_getSession()->addError(__('%s RMA(s) cannot be closed', $countNonCloseRma));
            } else {
                $this->_getSession()->addError(__('We cannot close the RMA request(s).'));
            }
        }
        if ($countCloseRma) {
            $this->_getSession()->addSuccess(__('%s RMA (s) have been closed.', $countCloseRma));
        }
    ...
    protected function _getConfigDataJson($model)
    {
        ...
        $carrier = Mage::helper('Enterprise_Rma_Helper_Data')->getCarrier($code, $model->getStoreId());
        if ($carrier) {
        ...
        try {
            ...
            if ($labelContent) {
                ...
                if (stripos($labelContent, '%PDF-') !== false) {
                    ...
                    if (!$page) {
                        $this->_getSession()->addError(__("We don't recognize or support the file extension in shipment %s.", $model->getIncrementId()));
                    }
                    $pdf->pages[] = $page;
