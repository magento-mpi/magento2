    ...
    protected function _prepareLayout()
    {
        if ($salesRule = Mage::registry('current_promo_quote_rule')) {
            ...
            if ($count = $resource->getAssignedRulesCount($salesRule->getId())) {
                $confirm = Mage::helper('Enterprise_Reminder_Helper_Data')->__('This rule is assigned to %1 automated reminder rule(s). Deleting this rule will automatically unassign it.', $count);
