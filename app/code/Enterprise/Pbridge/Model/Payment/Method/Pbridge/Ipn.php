...
class Enterprise_Pbridge_Model_Payment_Method_Pbridge_Ipn
{
    ...
    protected function _getOrder()
    {
        if (empty($this->_order)) {
            ...
            if (!$order->getId()) {
                ...
                throw new Exception(__('A wrong Order ID (%s) is specified.', $id));
            }
            $this->_order = $order;
        }
        return $this->_order;
    ...
    protected function _verifyOrder(Mage_Sales_Model_Order $order)
    {
        ...
        if ($merchantEmail) {
            ...
            if ($merchantEmail != $receiverEmail) {
                Mage::throwException(__('Requested %s and configured %s merchant emails do not match.', $receiverEmail, $merchantEmail));
            }
        }
    }
    ...
    protected function _registerPaymentRefund()
    {
        ...
        if ($creditmemo = $payment->getCreatedCreditmemo()) {
            ...
            $comment = $order->addStatusHistoryComment(
                    __('Notified customer about creditmemo #%s.', $creditmemo->getIncrementId())
                )
                ->setIsCustomerNotified(true)
                ->save();
        }
    ...
    protected function _createIpnComment($comment = '', $addToHistory = true)
    {
        ...
        if ($this->getIpnFormData('txn_id')) {
            $message .= ' ' . __('Original gateway transaction id: #%s.', $this->getIpnFormData('txn_id'));
        }
        if ($comment) {
            $message .= ' ' . $comment;
        }
    ...
    private function _explainRefundReason($addToHistory = true)
    {
        ...
        return $this->_createIpnComment(__('Explanation: %s.', $message), $addToHistory);
    }
