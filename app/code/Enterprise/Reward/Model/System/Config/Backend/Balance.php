 ...
 *
 */
class Enterprise_Reward_Model_System_Config_Backend_Balance extends Mage_Core_Model_Config_Value
{
    /**
    ...
    protected function _beforeSave()
    {
        ...
        if ($this->getFieldsetDataValue('min_points_balance') < 0) {
            Mage::throwException(Mage::helper('Enterprise_Reward_Helper_Data')->__('"Minimum Reward Points Balance" should be either a positive number or left empty.'));
        }
        if ($this->getFieldsetDataValue('max_points_balance') < 0) {
            Mage::throwException(Mage::helper('Enterprise_Reward_Helper_Data')->__('"Cap Reward Points Balance" should be either a positive number or left empty.'));
        }
