...
class Enterprise_Pci_Adminhtml_Crypt_KeyController extends Mage_Adminhtml_Controller_Action
{
    ...
    protected function _checkIsLocalXmlWriteable()
    {
        ...
        if (!is_writeable($filename)) {
            Mage::getSingleton('Mage_Adminhtml_Model_Session')->addError(
                __('To enable a key change this file must be writable: %s.', realpath($filename))
            );
            return false;
        }
        return true;
    ...
    public function saveAction()
    {
        try {
            ...
            if (!$key) {
                Mage::getSingleton('Mage_Adminhtml_Model_Session')->addNotice(__('This is your new encryption key: <span style="font-family:monospace;">%s</span>. Be sure to write it down and take good care of it!', $newKey));
            }
            Mage::app()->cleanCache();
        }
        catch (Exception $e) {
