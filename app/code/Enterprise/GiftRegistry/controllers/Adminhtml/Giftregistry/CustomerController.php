...
class Enterprise_GiftRegistry_Adminhtml_Giftregistry_CustomerController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function editAction()
    {
        try {
            ...
            $this->_title(__('Customers'))
                ...
                ->_title(__("Edit '%s' Gift Registry", $model->getTitle()));
    ...
    public function updateAction()
    {
        ...
        if (is_array($items)) {
            try {
                ...
                Mage::getSingleton('Mage_Adminhtml_Model_Session')->addError(__("We couldn't update these gift registry items."));
                Mage::logException($e);
            }
        }
        $this->_redirect('*/*/edit', array('id' => $entity->getId()));
    ...
    public function shareAction()
    {
        ...
        if ($data = $this->getRequest()->getParam('emails')) {
            ...
            if ($sentCount) {
                $this->_getSession()->addSuccess(__('%d email(s) were sent.', $sentCount));
            }
            if ($failedCount) {
                $this->_getSession()->addError(
                    __("We couldn't send '%d of %d emails.", $failedCount, count($emailsForSend))
                );
            }
        }
        $this->_redirect('*/*/edit', array('id' => $model->getId()));
    ...
    public function deleteAction()
    {
        try {
            ...
            Mage::getSingleton('Mage_Adminhtml_Model_Session')->addError(__("We couldn't delete this gift registry entity."));
            Mage::logException($e);
        }
