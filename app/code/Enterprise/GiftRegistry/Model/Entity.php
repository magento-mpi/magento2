...
class Enterprise_GiftRegistry_Model_Entity extends Mage_Core_Model_Abstract
{
    ...
    public function sendShareRegistryEmail($recipient, $storeId, $message, $sender = null)
    {
        ...
        $mail = $this->_config->getModelInstance('Mage_Core_Model_Email_Template');
        ...
        if (count($emails)) {
            ...
            if ($count > 0) {
                $result->setIsSuccess(true)
                    ->setSuccessMessage(
                        __('You shared the gift registry for %d emails.', $count)
                    );
            } else {
            ...
            if ($model->getId() && $model->getEntityId() == $this->getId()) {
                if (!isset($item['delete'])) {
                    ...
                    if ($stockItem->getIsQtyDecimal() == 0 && $item['qty'] != (int)$item['qty']) {
                        throw new Mage_Exception(__('Please correct the  gift registry item quantity.'));
                    }
                }
            } else {
                Mage::throwException(
