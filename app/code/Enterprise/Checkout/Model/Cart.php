...
class Enterprise_Checkout_Model_Cart extends Varien_Object implements Mage_Checkout_Model_Cart_Interface
{
    ...
    public function addProduct($product, $config = 1)
    {
        ...
        if (!($product instanceof Mage_Catalog_Model_Product)) {
            ...
            if (!$product->getId()) {
                Mage::throwException(
                    __('Failed to add a product to cart by id "%s".', $productId)
                );
            }
        }
    ...
    public function getMessages()
    {
        ...
        if ($addedItemsCount) {
            $message = ($addedItemsCount == 1)
                    ? __('You added %s product to your shopping cart.', $addedItemsCount)
                    : __('You added %s products to your shopping cart.', $addedItemsCount);
            $messages[] = Mage::getSingleton('Mage_Core_Model_Message')->success($message);
        }
        if ($failedItemsCount) {
            $warning = ($failedItemsCount == 1)
                    ? __('%s product requires your attention.', $failedItemsCount)
                    : __('%s products require your attention.', $failedItemsCount);
