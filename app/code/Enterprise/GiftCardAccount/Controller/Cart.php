        ...
        if (isset($data['giftcard_code'])) {
            ...
            try {
                ...
                Mage::getSingleton('Mage_Checkout_Model_Session')->addSuccess(
                    __('Gift Card "%1" was added.', Mage::helper('Mage_Core_Helper_Data')->escapeHtml($code))
                );
        ...
        if ($code = $this->getRequest()->getParam('code')) {
            try {
                Mage::getModel('Enterprise_GiftCardAccount_Model_Giftcardaccount')
                    ...
                    ->removeFromCart();
                Mage::getSingleton('Mage_Checkout_Model_Session')->addSuccess(
                    __('Gift Card "%1" was removed.', Mage::helper('Mage_Core_Helper_Data')->escapeHtml($code))
                );
            } catch (Mage_Core_Exception $e) {
                Mage::getSingleton('Mage_Checkout_Model_Session')->addError(
                    $e->getMessage()
