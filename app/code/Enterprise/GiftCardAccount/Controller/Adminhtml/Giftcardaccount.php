...
class Enterprise_GiftCardAccount_Controller_Adminhtml_Giftcardaccount extends Magento_Adminhtml_Controller_Action
{
    ...
    public function editAction()
    {
        ...
        if (!$model->getId() && $id) {
            Mage::getSingleton('Magento_Adminhtml_Model_Session')->addError(Mage::helper('Enterprise_GiftCardAccount_Helper_Data')->__('This gift card account has been deleted.'));
            $this->_redirect('*/*/');
            return;
        }
    ...
    public function saveAction()
    {
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                if (!is_null($sending)) {
                    if ($sending) {
                        Mage::getSingleton('Magento_Adminhtml_Model_Session')->addSuccess(Mage::helper('Enterprise_GiftCardAccount_Helper_Data')->__('You saved the gift card account.'));
                    } else {
                        Mage::getSingleton('Magento_Adminhtml_Model_Session')->addError(Mage::helper('Enterprise_GiftCardAccount_Helper_Data')->__('You saved the gift card account, but an email was not sent.'));
                    }
                } else {
                    Mage::getSingleton('Magento_Adminhtml_Model_Session')->addSuccess(Mage::helper('Enterprise_GiftCardAccount_Helper_Data')->__('You saved the gift card account.'));

                    if ($status) {
                        Mage::getSingleton('Magento_Adminhtml_Model_Session')->addNotice(Mage::helper('Enterprise_GiftCardAccount_Helper_Data')->__('An email was not sent because the gift card account is not active.'));
                    }
                }
    ...
    public function deleteAction()
    {
        ...
        if ($id = $this->getRequest()->getParam('id')) {
            try {
                ...
                Mage::getSingleton('Magento_Adminhtml_Model_Session')->addSuccess(Mage::helper('Enterprise_GiftCardAccount_Helper_Data')->__('This gift card account has been deleted.'));
                // go to grid
                $this->_redirect('*/*/');
                return;
        ...
        // display error message
        Mage::getSingleton('Magento_Adminhtml_Model_Session')->addError(Mage::helper('Enterprise_GiftCardAccount_Helper_Data')->__("We couldn't find a gift card account to delete."));
        // go to grid
        $this->_redirect('*/*/');
    }
