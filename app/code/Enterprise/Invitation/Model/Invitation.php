...
class Enterprise_Invitation_Model_Invitation extends Magento_Core_Model_Abstract
{
    ...
    public function loadByInvitationCode($code)
    {
        $code = explode(':', $code, 2);
        if (count($code) != 2) {
            Mage::throwException(Mage::helper('Enterprise_Invitation_Helper_Data')->__('Please correct the invitation code.'));
        }
    ...
    protected function _beforeSave()
    {
        if (!$this->getId()) {
            ...
            if (Mage::getSingleton('Enterprise_Invitation_Model_Config')->getUseInviterGroup()) {
                ...
                if (!$this->hasGroupId()) {
                    throw new Magento_Core_Exception(Mage::helper('Enterprise_Invitation_Helper_Data')->__('You need to specify a customer ID group.'), self::ERROR_INVALID_DATA);
                }
            }
            else {
            ...
            if (!(int)$this->getStoreId()) {
                throw new Magento_Core_Exception(Mage::helper('Enterprise_Invitation_Helper_Data')->__('The wrong store is specified.'), self::ERROR_INVALID_DATA);
            }
            $this->makeSureCustomerNotExists();
        }
        else {
            if ($this->dataHasChangedFor('message') && !$this->canMessageBeUpdated()) {
                throw new Magento_Core_Exception(Mage::helper('Enterprise_Invitation_Helper_Data')->__("You can't update this message.", self::ERROR_STATUS));
            }
        }
    ...
    {
        if (!$this->getId()) {
            Mage::throwException(Mage::helper('Enterprise_Invitation_Helper_Data')->__("We can't generate encrypted code."));
        }
    ...
    {
        if (!$this->getId()) {
            throw new Magento_Core_Exception(Mage::helper('Enterprise_Invitation_Helper_Data')->__("We couldn't find an ID for this invitation."), self::ERROR_INVALID_DATA);
        }
        if ($this->getStatus() !== self::STATUS_NEW) {
        ...
        if (!$this->getEmail() || !Zend_Validate::is($this->getEmail(), 'EmailAddress')) {
            throw new Magento_Core_Exception(Mage::helper('Enterprise_Invitation_Helper_Data')->__('Please correct the invalid or empty invitation email.'), self::ERROR_INVALID_DATA);
        }
        $this->makeSureCustomerNotExists();
    }
    ...
    public function makeSureCustomerNotExists($email = null, $websiteId = null)
    {
        ...
        if (!$websiteId) {
            throw new Magento_Core_Exception(Mage::helper('Enterprise_Invitation_Helper_Data')->__("We can't identify the proper website."), self::ERROR_INVALID_DATA);
        }
        if (null === $email) {
            $email = $this->getEmail();
        }
        if (!$email) {
            throw new Magento_Core_Exception(Mage::helper('Enterprise_Invitation_Helper_Data')->__('Please specify an email.'), self::ERROR_INVALID_DATA);
        }
    ...
    public function validate()
    {
        ...
        if (!Zend_Validate::is($this->getEmail(), 'EmailAddress')) {
            $errors[] = Mage::helper('Enterprise_Invitation_Helper_Data')->__('Please correct the invitation email.');
        }
