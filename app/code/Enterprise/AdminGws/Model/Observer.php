        ...
        if (empty($websiteIds)) {
            ...
            foreach ($websiteIds as $websiteId) {
                if (!in_array($websiteId, $allWebsiteIds)) {
                    Mage::throwException(Mage::helper('Enterprise_AdminGws_Helper_Data')->__('Incorrect website ID: %1', $websiteId));
                }
                // prevent granting disallowed websites
                if (!$this->_role->getIsAll()) {
                    if (!$this->_role->hasWebsiteAccess($websiteId, true)) {
                        Mage::throwException(
                            Mage::helper('Enterprise_AdminGws_Helper_Data')->__('You need more permissions to access website "%1".', Mage::app()->getWebsite($websiteId)->getName())
                        );
                    }
                }
            }
        ...
        if (empty($storeGroupIds)) {
            ...
            foreach ($storeGroupIds as $storeGroupId) {
                if (!array($storeGroupId, $allStoreGroups)) {
                    Mage::throwException(Mage::helper('Enterprise_AdminGws_Helper_Data')->__('Incorrect store ID: %1', $storeGroupId));
                }
