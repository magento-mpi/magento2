...
class Mage_Catalog_Model_Product_Option_Type_File extends Mage_Catalog_Model_Product_Option_Type_Default
{
    ...
    protected function _validateUploadedFile()
    {
        ...
        try {
            ...
            if (isset($_SERVER['CONTENT_LENGTH']) && $_SERVER['CONTENT_LENGTH'] > $maxFileSize) {
                ...
                Mage::throwException(
                    __("The file you uploaded is larger than %s Megabytes allowed by server", $value)
                );
            } else {
    ...
    protected function _getValidatorErrors($errors, $fileInfo)
    {
        ...
        foreach ($errors as $errorCode) {
            if ($errorCode == Zend_Validate_File_ExcludeExtension::FALSE_EXTENSION) {
                $result[] = __("The file '%s' for '%s' has an invalid extension.", $fileInfo['title'], $option->getTitle());
                ...
                $result[] = __("The file '%s' you uploaded is larger than the %s megabytes allowed by our server.", $fileInfo['title'], $maxFileSize);
            }
        }
        return $result;
    }
    ...
    protected function _createWritableDir($path)
    {
        try {
            ...
            throw new Mage_Core_Exception(__("Cannot create writable directory '%s'.", $path));
        }
    }
