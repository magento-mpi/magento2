...
class Mage_Catalog_Model_Product_Option_Type_Date extends Mage_Catalog_Model_Product_Option_Type_Default
{
    ...
    public function validateUserValue($values)
    {
        ...
        if ($isValid) {
            ...
            if (!$dateValid) {
                Mage::throwException(Mage::helper('Mage_Catalog_Helper_Data')->__('Please specify date required option(s).'));
            } elseif (!$timeValid) {
                Mage::throwException(Mage::helper('Mage_Catalog_Helper_Data')->__('Please specify time required option(s).'));
            } else {
    ...
    public function getFormattedOptionValue($optionValue)
    {
        if ($this->_formattedOptionValue === null) {
            ...
            if ($this->getOption()->getType() == Mage_Catalog_Model_Product_Option::OPTION_TYPE_DATE) {
                $format = Mage::app()->getLocale()->getDateFormat(Mage_Core_Model_LocaleInterface::FORMAT_TYPE_MEDIUM);
                ...
                $format = Mage::app()->getLocale()->getDateTimeFormat(Mage_Core_Model_LocaleInterface::FORMAT_TYPE_SHORT);
