    ...
    implements Mage_Shipping_Model_Carrier_Interface
{
                ...
                if (is_object($xml)) {
                    if (in_array($xml->getName(), array('ErrorResponse', 'ShipmentValidateErrorResponse'))
                        ...
                        if ($this->_isShippingLabelFlag) {
                            ...
                            Mage::throwException(__('Error #%s : %s', trim($code), trim($data)));
                        }
                        ...
                        $this->_errors[$code] = __('Error #%s : %s', trim($code), trim($data));
                    } else {
            ...
            if ($currencyCode != $baseCurrencyCode) {
                ...
                if (!empty($rates) && isset($rates[$baseCurrencyCode])) {
                    ...
                    if (!isset($rates[$currencyCode]) || !$totalEstimate) {
                        $totalEstimate = false;
                        $this->_errors[] = __('We had to skip DHL method %s because we couldn\'t find exchange rate %s (Base Currency).', $currencyCode, $baseCurrencyCode, $dhlProductDescription);
                    }
                }
            }
            if ($totalEstimate) {
                ...
                $this->_errors[] = __("Zero shipping charge for '%s'", $dhlProductDescription);
            }
        } else {
    ...
    protected function _getDimensionUnit()
    {
        ...
        if (empty($measureUnit)) {
            Mage::throwException(__("Cannot identify measure unit for %s", $countryId));
        }
        return $measureUnit;
    }
    ...
    protected function _getWeightUnit()
    {
        ...
        if (empty($weightUnit)) {
            Mage::throwException(__("Cannot identify weight unit for %s", $countryId));
        }
        return $weightUnit;
    }
        ...
        foreach ($packages as &$piece) {
            ...
            if ($params['width'] || $params['length'] || $params['height']) {
                ...
                if ($params['width'] < $minValue || $params['length'] < $minValue || $params['height'] < $minValue) {
                    $message = __('Height, width and length should be equal or greater than %s', $minValue);
    ...
    protected function _parseXmlTrackingResponse($trackings, $response)
    {
        ...
        if (strlen(trim($response)) > 0) {
            ...
            if (is_object($xml) && ((isset($xml->Response->Status->ActionStatus)
                ...
                $code = isset($nodeCondition->ConditionCode) ? (string)$nodeCondition->ConditionCode : 0;
                $data = isset($nodeCondition->ConditionData) ? (string)$nodeCondition->ConditionData : '';
                $this->_errors[$code] = __('Error #%s : %s', $code, $data);
