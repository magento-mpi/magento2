    ...
    public function reindexProcessAction()
    {
        ...
        if ($process) {
            try {
                ...
                $this->_getSession()->addSuccess(
                    __('%s index was rebuilt.', $process->getIndexer()->getName())
                );
            } catch (Mage_Core_Exception $e) {
                $this->_getSession()->addError($e->getMessage());
            } catch (Exception $e) {
    ...
    public function massReindexAction()
    {
        ...
        if (empty($processIds) || !is_array($processIds)) {
            ...
            try {
                ...
                $this->_getSession()->addSuccess(
                    __('Total of %d index(es) have reindexed data.', $counter)
                );
            } catch (Mage_Core_Exception $e) {
                $this->_getSession()->addError($e->getMessage());
            } catch (Exception $e) {
    ...
    public function massChangeModeAction()
    {
        ...
        if (empty($processIds) || !is_array($processIds)) {
            ...
            try {
                ...
                $this->_getSession()->addSuccess(
                    __('Total of %d index(es) have changed index mode.', $counter)
                );
            } catch (Mage_Core_Exception $e) {
                $this->_getSession()->addError($e->getMessage());
            } catch (Exception $e) {
