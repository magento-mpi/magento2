...
class Mage_GoogleShopping_Model_MassOperations
{
    ...
    public function addProducts($productIds, $storeId)
    {
            ...
            foreach ($productIds as $productId) {
                ...
                try {
                    ...
                    if ($e->getCode() == self::ERROR_CODE_SQL_UNIQUE_INDEX) {
                        $message = __("The Google Content item for product '%s' (in '%s' store) already exists.", $product->getName(), Mage::app()->getStore($product->getStoreId())->getName());
                    }
                    $errors[] = $message;
                } catch (Mage_Core_Exception $e) {
                    $errors[] = __('The product "%s" cannot be added to Google Content. %s', $product->getName(), $e->getMessage());
                } catch (Exception $e) {
                    Mage::logException($e);
                    $errors[] = __('The product "%s" hasn\'t been added to Google Content.', $product->getName());
                }
            }
            if (empty($productIds)) {
                return $this;
        ...
        if ($totalAdded > 0) {
            $this->_getNotifier()->addNotice(
                ...
                __('A total of %d product(s) have been added to Google Content.', $totalAdded)
            );
        }
    ...
    public function synchronizeItems($items)
    {
        ...
        if ($itemsCollection) {
            ...
            foreach ($itemsCollection as $item) {
                ...
                try {
                    ...
                    $errors[] = __('The item "%s" cannot be updated at Google Content. %s', $item->getProduct()->getName(), $e->getMessage());
                    $totalFailed++;
                } catch (Exception $e) {
                    Mage::logException($e);
                    $errors[] = __('The item "%s" hasn\'t been updated.', $item->getProduct()->getName());
                    $totalFailed++;
                }
            }
        } else {
        ...
        $this->_getNotifier()->addNotice(
            ...
            __('A total of %d items(s) have been deleted; a total of %d items(s) have been updated.', $totalDeleted, $totalUpdated)
        );
        if ($totalFailed > 0 || count($errors)) {
            array_unshift($errors, __("We cannot update %s items.", $totalFailed));
            $this->_getNotifier()->addMajor(
    ...
    public function deleteItems($items)
    {
        ...
        if ($itemsCollection) {
            ...
            foreach ($itemsCollection as $item) {
                ...
                try {
                    ...
                    Mage::logException($e);
                    $errors[] = __('The item "%s" hasn\'t been deleted.', $item->getProduct()->getName());
                }
            }
        } else {
            return $this;
        ...
        if ($totalDeleted > 0) {
            $this->_getNotifier()->addNotice(
                ...
                __('Total of %d items(s) have been removed from Google Shopping.', $totalDeleted)
            );
        }
        if (count($errors)) {
            $this->_getNotifier()->addMajor(
