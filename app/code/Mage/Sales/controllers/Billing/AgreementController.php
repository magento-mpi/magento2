...
class Mage_Sales_Billing_AgreementController extends Mage_Core_Controller_Front_Action
{
    ...
    public function viewAction()
    {
        ...
        $this->_title(__('Billing Agreements'))
            ->_title(__('Billing Agreement # %s', $agreement->getReferenceId()));
        $this->loadLayout();
        $this->_initLayoutMessages('Mage_Customer_Model_Session');
        ...
        if ($token && $paymentCode) {
            try {
                ...
                $this->_getSession()->addSuccess(
                    __('The billing agreement "%s" has been created.', $agreement->getReferenceId())
                );
    ...
    public function cancelAction()
    {
        ...
        if ($agreement && $agreement->canCancel()) {
            try {
                $agreement->cancel();
                $this->_getSession()->addNotice(__('The billing agreement "%s" has been canceled.', $agreement->getReferenceId()));
            } catch (Mage_Core_Exception $e) {
                $this->_getSession()->addError($e->getMessage());
            } catch (Exception $e) {
                Mage::logException($e);
