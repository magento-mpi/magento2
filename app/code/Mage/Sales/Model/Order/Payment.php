...
class Mage_Sales_Model_Order_Payment extends Mage_Payment_Model_Info
{
    ...
    public function capture($invoice)
    {
        ...
        if (!$invoice->getIsPaid() && !$this->getIsTransactionPending()) {
            ...
            if ($this->getIsTransactionPending()) {
                $message = __('An amount of %s will be captured after being approved at the payment gateway.', $this->_formatPrice($amountToCapture));
                ...
                $message = __('Captured amount of %s online', $this->_formatPrice($amountToCapture));
        ...
        Mage::throwException(
            __('The transaction "%s" cannot be captured yet.', $invoice->getTransactionId())
        );
    }
    ...
    public function registerCaptureNotification($amount)
    {
        ...
        $status = true;
        if ($this->getIsTransactionPending()) {
            $message = __('An amount of %s will be captured after being approved at the payment gateway.', $this->_formatPrice($amount));
            $state = Mage_Sales_Model_Order::STATE_PAYMENT_REVIEW;
            if ($this->getIsFraudDetected()) {
                $message = __('Order is suspended as its capture amount %s is suspected to be fraudulent.', $this->_formatPrice($amount));
                $status = Mage_Sales_Model_Order::STATUS_FRAUD;
            }
        } else {
            $message = __('Registered notification about captured amount of %s.', $this->_formatPrice($amount));
            $state = Mage_Sales_Model_Order::STATE_PROCESSING;
            if ($this->getIsFraudDetected()) {
                $state = Mage_Sales_Model_Order::STATE_PAYMENT_REVIEW;
                $message = __('Order is suspended as its capture amount %s is suspected to be fraudulent.', $this->_formatPrice($amount));
    ...
    public function refund($creditmemo)
    {
        ...
        if ($invoice) {
            $message = __('We refunded %s online.', $this->_formatPrice($baseAmountToRefund));
        } else {
            $message = $this->hasMessage() ? $this->getMessage()
                : __('We refunded %s offline.', $this->_formatPrice($baseAmountToRefund));
        }
    ...
    public function registerRefundNotification($amount)
    {
        ...
        if ($amount <= 0) {
            $order->addStatusHistoryComment(__('IPN "Refunded". Refund issued by merchant. Registered notification about refunded amount of %s. Transaction ID: "%s"', $this->_formatPrice($notificationAmount), $this->getTransactionId()), false);
            return $this;
        }
        ...
        $message = $this->_prependMessage(
            __('Registered notification about refunded amount of %s.', $this->_formatPrice($amount))
        );
        $message = $this->_appendTransactionToMessage($transaction, $message);
        $order->setState(Mage_Sales_Model_Order::STATE_PROCESSING, true, $message);
        return $this;
    ...
    protected function _order($amount)
    {
        ...
        if ($this->getIsTransactionPending()) {
            $message = __('The order amount of %s is pending approval on the payment gateway.', $this->_formatPrice($amount));
            $state = Mage_Sales_Model_Order::STATE_PAYMENT_REVIEW;
            if ($this->getIsFraudDetected()) {
                $status = Mage_Sales_Model_Order::STATUS_FRAUD;
            }
        } else {
            $message = __('Ordered amount of %s', $this->_formatPrice($amount));
        }
    ...
    protected function _authorize($isOnline, $amount)
    {
        ...
        // similar logic of "payment review" order as in capturing
        if ($this->getIsTransactionPending()) {
            $message = __('We will authorize %s after the payment is approved at the payment gateway.', $this->_formatPrice($amount));
            $state = Mage_Sales_Model_Order::STATE_PAYMENT_REVIEW;
            if ($this->getIsFraudDetected()) {
                $status = Mage_Sales_Model_Order::STATUS_FRAUD;
            }
        } else {
            $message = __('Authorized amount of %s', $this->_formatPrice($amount));
        }
    ...
    protected function _void($isOnline, $amount = null, $gatewayCallback = 'void')
    {
        ...
        $message = $this->_prependMessage($message);
        if ($amount) {
            $message .= ' ' . __('Amount: %s.', $this->_formatPrice($amount));
        }
        $message = $this->_appendTransactionToMessage($transaction, $message);
        $order->setState(Mage_Sales_Model_Order::STATE_PROCESSING, true, $message);
