...
class Mage_Sales_Model_Order_Creditmemo_Api extends Mage_Sales_Model_Api_Resource
{
    ...
    public function cancel($creditmemoIncrementId)
    {
        ...
        if (!$creditmemo->canCancel()) {
            $this->_fault('status_not_changed', Mage::helper('Mage_Sales_Helper_Data')->__('We can\'t cancel the credit memo'));
        }
        try {
            $creditmemo->cancel()->save();
        } catch (Exception $e) {
            $this->_fault('status_not_changed', Mage::helper('Mage_Sales_Helper_Data')->__('Something went wrong while canceling the credit memo.'));
        }
