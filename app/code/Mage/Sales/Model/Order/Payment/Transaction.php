...
class Mage_Sales_Model_Order_Payment_Transaction extends Mage_Core_Model_Abstract
{
    ...
    public function close($shouldSave = true)
    {
        ...
        if (1 == $this->getIsClosed() && $this->_isFailsafe) {
            Mage::throwException(__('The transaction "%s" (%s) is already closed.', $this->getTxnId(), $this->getTxnType()));
        }
        $this->setIsClosed(1);
        if ($shouldSave) {
            $this->save();
    ...
    protected function _verifyTxnType($txnType = null)
    {
        ...
        switch ($txnType) {
            ...
            case self::TYPE_REFUND:
                break;
            default:
                Mage::throwException(__('We found an unsupported transaction type "%s".', $txnType));
        }
    }
