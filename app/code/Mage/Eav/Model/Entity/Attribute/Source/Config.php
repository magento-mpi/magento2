...
class Mage_Eav_Model_Entity_Attribute_Source_Config extends Mage_Eav_Model_Entity_Attribute_Source_Abstract
{
    ...
    public function getAllOptions()
    {
        if ($this->_options === null) {
            ...
            if (!$rootNode) {
                throw Mage::exception('Mage_Eav', __('Failed to load node %s from config', $this->_configNodePath));
            }
            $options = $rootNode->children();
            if (empty($options)) {
                throw Mage::exception('Mage_Eav', __('No options found in config node %s', $this->_configNodePath));
            }
