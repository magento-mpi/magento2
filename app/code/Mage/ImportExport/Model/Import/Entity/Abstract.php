...
abstract class Mage_ImportExport_Model_Import_Entity_Abstract
{
    ...
    {
        $this->_errors[(string)$errorCode][] = array($errorRowNum + 1, $colName); // one added for human readability
    ...
    public function validateData()
    {
        if (!$this->_dataValidated) {
            ...
            if ($absentColumns = array_diff($this->_permanentAttributes, $this->getSource()->getColNames())) {
                Mage::throwException(
                    __('Cannot find required columns: %s', implode(', ', $absentColumns))
                );
            }
            ...
            if ($emptyHeaderColumns) {
                Mage::throwException(
                    __('Columns number: "%s" have empty headers', implode('", "', $emptyHeaderColumns))
                );
            }
            if ($invalidColumns) {
                Mage::throwException(
                    __('Column names: "%s" are invalid', implode('", "', $invalidColumns))
                );
            }
