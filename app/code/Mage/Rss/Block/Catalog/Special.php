...
class Mage_Rss_Block_Catalog_Special extends Mage_Rss_Block_Catalog_Abstract
{
    ...
    protected function _toHtml()
    {
        ...
        $title = __('%s - Special Products', Mage::app()->getStore()->getFrontendName());
                ...
                if ($product->getAllowedPriceInRss()) {
                    if (Mage::helper('Mage_Catalog_Helper_Data')->canApplyMsrp($product)) {
                        ...
                        if ($result['use_special']) {
                            $special = '<br />' . __('Special Expires On: %s', $this->formatDate($result['special_to_date'], Mage_Core_Model_LocaleInterface::FORMAT_TYPE_MEDIUM));
                        }
                        $html .= sprintf('<p>%s %s%s</p>',
                            __('Price: %s', Mage::helper('Mage_Core_Helper_Data')->currency($result['price'])),
                            __('Special Price: %s', Mage::helper('Mage_Core_Helper_Data')->currency($result['final_price'])),
                            $special
                        );
                    }
                }
