...
class Mage_Rss_Block_Catalog_Special extends Mage_Rss_Block_Catalog_Abstract
{
    ...
    protected function _toHtml()
    {
        ...
        $title = Mage::helper('Mage_Rss_Helper_Data')->__('%1 - Special Products', Mage::app()->getStore()->getFrontendName());
                ...
                if ($product->getAllowedPriceInRss()) {
                    if (Mage::helper('Mage_Catalog_Helper_Data')->canApplyMsrp($product)) {
                        ...
                        if ($result['use_special']) {
                            $special = '<br />' . Mage::helper('Mage_Catalog_Helper_Data')->__('Special Expires On: %1', $this->formatDate($result['special_to_date'], Mage_Core_Model_LocaleInterface::FORMAT_TYPE_MEDIUM));
                        }
                        $html .= sprintf('<p>%s %s%s</p>',
                            Mage::helper('Mage_Catalog_Helper_Data')->__('Price: %1', Mage::helper('Mage_Core_Helper_Data')->currency($result['price'])),
                            Mage::helper('Mage_Catalog_Helper_Data')->__('Special Price: %1', Mage::helper('Mage_Core_Helper_Data')->currency($result['final_price'])),
                            $special
                        );
                    }
                }
