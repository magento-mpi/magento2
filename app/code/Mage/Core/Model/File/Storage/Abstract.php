...
abstract class Mage_Core_Model_File_Storage_Abstract extends Mage_Core_Model_Abstract
{
    ...
    public function collectFileInfo($path)
    {
        ...
        if (!file_exists($fullPath) || !is_file($fullPath)) {
            Mage::throwException(Mage::helper('Mage_Core_Helper_Data')->__('File %1 does not exist', $fullPath));
        }
        if (!is_readable($fullPath)) {
            Mage::throwException(Mage::helper('Mage_Core_Helper_Data')->__('File %1 is not readable', $fullPath));
        }
