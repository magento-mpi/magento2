...
class Mage_Adminhtml_Sales_OrderController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function massCancelAction()
    {
        ...
        if ($countNonCancelOrder) {
            if ($countCancelOrder) {
                $this->_getSession()->addError(__('%s order(s) cannot be canceled.', $countNonCancelOrder));
            } else {
                $this->_getSession()->addError(__('You cannot cancel the order(s).'));
            }
        }
        if ($countCancelOrder) {
            $this->_getSession()->addSuccess(__('We canceled %s order(s).', $countCancelOrder));
        }
        $this->_redirect('*/*/');
    }
    ...
    public function massHoldAction()
    {
        ...
        if ($countNonHoldOrder) {
            if ($countHoldOrder) {
                $this->_getSession()->addError(__('%s order(s) were not put on hold.', $countNonHoldOrder));
            } else {
                $this->_getSession()->addError(__('No order(s) were put on hold.'));
            }
        }
        if ($countHoldOrder) {
            $this->_getSession()->addSuccess(__('You have put %s order(s) on hold.', $countHoldOrder));
        }
    ...
    public function massUnholdAction()
    {
        ...
        if ($countNonUnHoldOrder) {
            if ($countUnHoldOrder) {
                $this->_getSession()->addError(
                    __('%s order(s) were not released from on hold status.', $countNonUnHoldOrder)
                );
            } else {
        ...
        if ($countUnHoldOrder) {
            $this->_getSession()->addSuccess(
                __('%s order(s) have been released from on hold status.', $countUnHoldOrder)
            );
        }
        $this->_redirect('*/*/');
    }
