...
class Mage_Shipping_Model_Resource_Carrier_Tablerate extends Mage_Core_Model_Resource_Db_Abstract
{
    ...
    public function uploadAndImport(Varien_Object $object)
    {
        ...
        if ($headers === false || count($headers) < 5) {
            $io->streamClose();
            Mage::throwException(Mage::helper('Mage_Shipping_Helper_Data')->__('Please correct Table Rates File Format.'));
        }
        ...
        try {
            ...
            $io->streamClose();
            Mage::logException($e);
            Mage::throwException(Mage::helper('Mage_Shipping_Helper_Data')->__('Something went wrong while importing table rates.'));
        }

        $adapter->commit();

        if ($this->_importErrors) {
            $error = Mage::helper('Mage_Shipping_Helper_Data')->__('We couldn\'t import this file because of these errors: %1', implode(" \n", $this->_importErrors));
    ...
    protected function _getImportRow($row, $rowNumber = 0)
    {
        // validate row
        if (count($row) < 5) {
            $this->_importErrors[] = Mage::helper('Mage_Shipping_Helper_Data')->__('Please correct Table Rates format in the Row #%1.', $rowNumber);
            return false;
        }
        ...
        if (isset($this->_importIso2Countries[$row[0]])) {
            ...
            $this->_importErrors[] = Mage::helper('Mage_Shipping_Helper_Data')->__('Please correct Country "%1" in the Row #%2.', $row[0], $rowNumber);
            return false;
        }
        ...
        if ($countryId != '0' && isset($this->_importRegions[$countryId][$row[1]])) {
            ...
            $this->_importErrors[] = Mage::helper('Mage_Shipping_Helper_Data')->__('Please correct Region/State "%1" in the Row #%2.', $row[1], $rowNumber);
            return false;
        }
        ...
        if ($value === false) {
            $this->_importErrors[] = Mage::helper('Mage_Shipping_Helper_Data')->__('Please correct %1 "%2" in the Row #%3.', $this->_getConditionFullName($this->_importConditionName), $row[3], $rowNumber);
            return false;
        }
        ...
        if ($price === false) {
            $this->_importErrors[] = Mage::helper('Mage_Shipping_Helper_Data')->__('Please correct Shipping Price "%1" in the Row #%2.', $row[4], $rowNumber);
            return false;
        }
        ...
        if (isset($this->_importUniqueHash[$hash])) {
            $this->_importErrors[] = Mage::helper('Mage_Shipping_Helper_Data')->__('Duplicate Row #%1 (Country "%2", Region/State "%3", Zip "%4" and Value "%5")', $rowNumber, $row[0], $row[1], $zipCode, $value);
            return false;
        }
