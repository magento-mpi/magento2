...
class Mage_GoogleCheckout_Model_Api_Xml_Callback extends Mage_GoogleCheckout_Model_Api_Xml_Abstract
{
    ...
    protected function _responseNewOrderNotification()
    {
        ...
        $message = $this->__('Google Order Number: %1', '<strong>' . $this->getGoogleOrderNumber() . '</strong><br />')
                . $this->__('Google Buyer ID: %1', '<strong>' . $this->getData('root/buyer-id/VALUE') . '</strong><br />')
                . $this->__('Is Buyer Willing to Receive Marketing Emails: %1', '<strong>' . $emailStr . '</strong>');
        if ($taxMessage) {
            $message .= $this->__('<br />Warning: <strong>%1</strong><br />', $taxMessage);
        }
    ...
    protected function _responseRiskInformationNotification()
    {
        ...
        $msg .= '<br />' . $this->__('IP Address: %1', '<strong>' . $order->getRemoteIp() . '</strong>');
        $msg .= '<br />' . $this->__('CC Partial: xxxx-%1', '<strong>' . $payment->getCcLast4() . '</strong>');
        $msg .= '<br />' . $this->__('AVS Status: %1', '<strong>' . $payment->getCcAvsStatus() . '</strong>');
        $msg .= '<br />' . $this->__('CID Status: %1', '<strong>' . $payment->getCcCidStatus() . '</strong>');
        $msg .= '<br />' . $this->__('Eligible for Protection: %1', '<strong>' . ($this->getData('root/risk-information/eligible-for-protection/VALUE')=='true' ? 'Yes' : 'No') . '</strong>');
        $msg .= '<br />' . $this->__('Buyer Account Age: %1 days', '<strong>' . $this->getData('root/risk-information/buyer-account-age/VALUE') . '</strong>');
    ...
    protected function _responseAuthorizationAmountNotification()
    {
        ...
        $msg .= '<br />' . $this->__('Amount: %1', '<strong>' . $this->_formatAmount($payment->getAmountAuthorized()) . '</strong>');
        $msg .= '<br />' . $this->__('Expiration: %1', '<strong>' . $expDate->toString() . '</strong>');
    ...
    protected function _responseChargeAmountNotification()
    {
        ...
        $msg .= '<br />' . $this->__('Latest Charge: %1', '<strong>' . $this->_formatAmount($latestCharged) . '</strong>');
        $msg .= '<br />' . $this->__('Total Charged: %1', '<strong>' . $this->_formatAmount($totalCharged) . '</strong>');

        if (!$order->hasInvoices() && abs($order->getBaseGrandTotal() - $latestCharged) < .0001) {
            $invoice = $this->_createInvoice();
            $msg .= '<br />' . $this->__('Invoice Auto-Created: %1', '<strong>' . $invoice->getIncrementId() . '</strong>');
        }
    ...
    protected function _responseChargebackAmountNotification()
    {
        ...
        $msg .= '<br />' . $this->__('Latest Chargeback: %1', '<strong>' . $this->_formatAmount($latestChargeback) . '</strong>');
        $msg .= '<br />' . $this->__('Total Chargeback: %1', '<strong>' . $this->_formatAmount($totalChargeback) . '</strong>');
    ...
    protected function _responseRefundAmountNotification()
    {
        ...
        $msg .= '<br />' . $this->__('Latest Refund: %1', '<strong>' . $this->_formatAmount($latestRefunded) . '</strong>');
        $msg .= '<br />' . $this->__('Total Refunded: %1', '<strong>' . $this->_formatAmount($totalRefunded) . '</strong>');
    ...
    protected function _responseOrderStateChangeNotification()
    {
        ...
        if ($prevFinancial!=$newFinancial) {
            $msg .= "<br />" . $this->__('Financial: %1 -> %2', '<strong>' . $prevFinancial . '</strong>', '<strong>' . $newFinancial . '</strong>');
        }
        if ($prevFulfillment!=$newFulfillment) {
            $msg .= "<br />" . $this->__('Fulfillment: %1 -> %2', '<strong>' . $prevFulfillment . '</strong>', '<strong>' . $newFulfillment . '</strong>');
        }
        $this->getOrder()
