...
class Mage_GoogleCheckout_Model_Api_Xml_Callback extends Mage_GoogleCheckout_Model_Api_Xml_Abstract
{
    ...
    protected function _responseNewOrderNotification()
    {
        ...
        $message = __('Google Order Number: %s', '<strong>' . $this->getGoogleOrderNumber() . '</strong><br />')
                . __('Google Buyer ID: %s', '<strong>' . $this->getData('root/buyer-id/VALUE') . '</strong><br />')
                . __('Is Buyer Willing to Receive Marketing Emails: %s', '<strong>' . $emailStr . '</strong>');
        if ($taxMessage) {
            $message .= __('<br />Warning: <strong>%s</strong><br />', $taxMessage);
        }
    ...
    protected function _responseRiskInformationNotification()
    {
        ...
        $msg .= '<br />' . __('IP Address: %s', '<strong>' . $order->getRemoteIp() . '</strong>');
        $msg .= '<br />' . __('CC Partial: xxxx-%s', '<strong>' . $payment->getCcLast4() . '</strong>');
        $msg .= '<br />' . __('AVS Status: %s', '<strong>' . $payment->getCcAvsStatus() . '</strong>');
        $msg .= '<br />' . __('CID Status: %s', '<strong>' . $payment->getCcCidStatus() . '</strong>');
        $msg .= '<br />' . __('Eligible for Protection: %s', '<strong>' . ($this->getData('root/risk-information/eligible-for-protection/VALUE')=='true' ? 'Yes' : 'No') . '</strong>');
        $msg .= '<br />' . __('Buyer Account Age: %s days', '<strong>' . $this->getData('root/risk-information/buyer-account-age/VALUE') . '</strong>');
    ...
    protected function _responseAuthorizationAmountNotification()
    {
        ...
        $msg .= '<br />' . __('Amount: %s', '<strong>' . $this->_formatAmount($payment->getAmountAuthorized()) . '</strong>');
        $msg .= '<br />' . __('Expiration: %s', '<strong>' . $expDate->toString() . '</strong>');
    ...
    protected function _responseChargeAmountNotification()
    {
        ...
        $msg .= '<br />' . __('Latest Charge: %s', '<strong>' . $this->_formatAmount($latestCharged) . '</strong>');
        $msg .= '<br />' . __('Total Charged: %s', '<strong>' . $this->_formatAmount($totalCharged) . '</strong>');

        if (!$order->hasInvoices() && abs($order->getBaseGrandTotal() - $latestCharged) < .0001) {
            $invoice = $this->_createInvoice();
            $msg .= '<br />' . __('Invoice Auto-Created: %s', '<strong>' . $invoice->getIncrementId() . '</strong>');
        }
    ...
    protected function _responseChargebackAmountNotification()
    {
        ...
        $msg .= '<br />' . __('Latest Chargeback: %s', '<strong>' . $this->_formatAmount($latestChargeback) . '</strong>');
        $msg .= '<br />' . __('Total Chargeback: %s', '<strong>' . $this->_formatAmount($totalChargeback) . '</strong>');
    ...
    protected function _responseRefundAmountNotification()
    {
        ...
        $msg .= '<br />' . __('Latest Refund: %s', '<strong>' . $this->_formatAmount($latestRefunded) . '</strong>');
        $msg .= '<br />' . __('Total Refunded: %s', '<strong>' . $this->_formatAmount($totalRefunded) . '</strong>');
    ...
    protected function _responseOrderStateChangeNotification()
    {
        ...
        if ($prevFinancial!=$newFinancial) {
            $msg .= "<br />" . __('Financial: %s -> %s', '<strong>' . $prevFinancial . '</strong>', '<strong>' . $newFinancial . '</strong>');
        }
        if ($prevFulfillment!=$newFulfillment) {
            $msg .= "<br />" . __('Fulfillment: %s -> %s', '<strong>' . $prevFulfillment . '</strong>', '<strong>' . $newFulfillment . '</strong>');
        }
        $this->getOrder()
