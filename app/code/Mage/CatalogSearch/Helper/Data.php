...
class Mage_CatalogSearch_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function checkNotes($store = null)
    {
        if ($this->_isMaxLength) {
            $this->addNoteMessage(__('Your search query can\'t be longer than %s, so we had to shorten your query.', $this->getMaxQueryLength()));
        }
        ...
        if ($searchType == Mage_CatalogSearch_Model_Fulltext::SEARCH_TYPE_COMBINE
            ...
            if (count($wordsFull) > count($wordsLike)) {
                ...
                $this->addNoteMessage(
                    __('Sorry, but the maximum word count is %1$s. We left out this part of your search: %2$s.', $this->getMaxQueryWords(), join(' ', $wordsCut))
                );
            }
        }
        return $this;
