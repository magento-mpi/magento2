...
class Mage_CatalogSearch_Model_Advanced extends Mage_Core_Model_Abstract
{
    ...
    public function addFilters($values)
    {
        ...
        if ($allConditions) {
            ...
            throw new Mage_Core_Exception(__('Please specify at least one search term.'));
        }
            ...
            if (isset($value['from']) && isset($value['to'])) {
                if (!empty($value['from']) || !empty($value['to'])) {
                    ...
                    if (strlen($value['from']) > 0 && strlen($value['to']) > 0) {
                        ...
                        // and more
                        $value = __('%s and greater', ($currencyModel ? $from : $value['from']));
                    } elseif (strlen($value['to']) > 0) {
                        // to
                        $value = __('up to %s', ($currencyModel ? $to : $value['to']));
                    }
                } else {
                    return $this;
                }
