...
class Mage_CatalogInventory_Model_Stock extends Mage_Core_Model_Abstract
{
    ...
    public function registerProductsSale($items)
    {
        ...
        foreach ($stockInfo as $itemInfo) {
            ...
            if (!$item->checkQty($qtys[$item->getProductId()])) {
                ...
                Mage::throwException(Mage::helper('Mage_CatalogInventory_Helper_Data')->__('Not all of your products are available in the requested quantity.'));
            }
    ...
    public function registerItemSale(Varien_Object $item)
    {
        ...
        if ($productId) {
            ...
            Mage::throwException(Mage::helper('Mage_CatalogInventory_Helper_Data')->__('We cannot specify a product identifier for the order item.'));
        }
        return $this;
    }
