...
class Mage_CatalogInventory_Model_Stock_Item extends Mage_Core_Model_Abstract
{
    ...
    public function checkQuoteItemQty($qty, $summaryQty, $origQty = 0)
    {
        ...
        if ($this->getMinSaleQty() && $qty < $this->getMinSaleQty()) {
            $result->setHasError(true)
                ->setMessage(
                    __('The fewest you may purchase is %s.', $this->getMinSaleQty() * 1)
                )
        ...
        if ($this->getMaxSaleQty() && $qty > $this->getMaxSaleQty()) {
            $result->setHasError(true)
                ->setMessage(
                    __('The most you may purchase is %s.', $this->getMaxSaleQty() * 1)
                )
        ...
        if (!$this->checkQty($summaryQty) || !$this->checkQty($qty)) {
            $message = __('We don\'t have as many "%s" as you requested.', $this->getProductName());
            $result->setHasError(true)
                ->setMessage($message)
            ...
            if (($this->getQty() - $summaryQty) < 0) {
                if ($this->getProductName()) {
                    ...
                    if ($this->getBackorders() == Mage_CatalogInventory_Model_Stock::BACKORDERS_YES_NOTIFY) {
                        if (!$this->getIsChildItem()) {
                            $result->setMessage(
                                __('We don\'t have as many "%s" as you requested, but we\'ll back order the remaining %s.', ($backorderQty * 1))
                            );
                        } else {
                            $result->setMessage(
                                __('We don\'t have "%s" in the requested quantity, so we\'ll back order the remaining %s.', $this->getProductName(), ($backorderQty * 1))
                            );
                        }
                    } elseif (Mage::app()->getStore()->isAdmin()) {
                        $result->setMessage(
                            __('We don\'t have as many "%s" as you requested.', $this->getProductName())
                        );
                    }
                }
            } else {
    ...
    public function checkQtyIncrements($qty)
    {
        ...
        if ($qtyIncrements && (Mage::helper('Mage_Core_Helper_Data')->getExactDivision($qty, $qtyIncrements) != 0)) {
            ...
            if ($this->getIsChildItem()) {
                $result->setMessage(
                    __('You can buy %s only in increments of %s.',$this->getProductName(), $qtyIncrements * 1)
                );
            } else {
                $result->setMessage(
                    __('You can buy this product only in increments of %s.', $qtyIncrements * 1)
                );
            }
        }
