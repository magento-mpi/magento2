...
class Mage_CatalogInventory_Model_Stock_Item extends Mage_Core_Model_Abstract
{
    ...
    public function checkQuoteItemQty($qty, $summaryQty, $origQty = 0)
    {
        ...
        if ($this->getMinSaleQty() && $qty < $this->getMinSaleQty()) {
            $result->setHasError(true)
                ->setMessage(
                    Mage::helper('Mage_CatalogInventory_Helper_Data')->__('The fewest you may purchase is %1.', $this->getMinSaleQty() * 1)
                )
        ...
        if ($this->getMaxSaleQty() && $qty > $this->getMaxSaleQty()) {
            $result->setHasError(true)
                ->setMessage(
                    Mage::helper('Mage_CatalogInventory_Helper_Data')->__('The most you may purchase is %1.', $this->getMaxSaleQty() * 1)
                )
        ...
        if (!$this->checkQty($summaryQty) || !$this->checkQty($qty)) {
            $message = Mage::helper('Mage_CatalogInventory_Helper_Data')->__('We don\'t have as many "%1" as you requested.', $this->getProductName());
            $result->setHasError(true)
                ->setMessage($message)
            ...
            if (($this->getQty() - $summaryQty) < 0) {
                if ($this->getProductName()) {
                    ...
                    if ($this->getBackorders() == Mage_CatalogInventory_Model_Stock::BACKORDERS_YES_NOTIFY) {
                        if (!$this->getIsChildItem()) {
                            $result->setMessage(
                                Mage::helper('Mage_CatalogInventory_Helper_Data')->__('We don\'t have as many "%1" as you requested, but we\'ll back order the remaining %2.', $this->getProductName(), ($backorderQty * 1))
                            );
                        } else {
                            $result->setMessage(
                                Mage::helper('Mage_CatalogInventory_Helper_Data')->__('We don\'t have "%1" in the requested quantity, so we\'ll back order the remaining %2.', $this->getProductName(), ($backorderQty * 1))
                            );
                        }
                    } elseif (Mage::app()->getStore()->isAdmin()) {
                        $result->setMessage(
                            Mage::helper('Mage_CatalogInventory_Helper_Data')->__('We don\'t have as many "%1" as you requested.', $this->getProductName())
                        );
                    }
                }
            } else {
    ...
    public function checkQtyIncrements($qty)
    {
        ...
        if ($qtyIncrements && (Mage::helper('Mage_Core_Helper_Data')->getExactDivision($qty, $qtyIncrements) != 0)) {
            ...
            if ($this->getIsChildItem()) {
                $result->setMessage(
                    Mage::helper('Mage_CatalogInventory_Helper_Data')->__('You can buy %1 only in increments of %2.',$this->getProductName(), $qtyIncrements * 1)
                );
            } else {
                $result->setMessage(
                    Mage::helper('Mage_CatalogInventory_Helper_Data')->__('You can buy this product only in increments of %1.', $qtyIncrements * 1)
                );
            }
        }
