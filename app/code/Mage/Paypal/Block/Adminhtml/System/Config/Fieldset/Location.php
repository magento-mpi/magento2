        ...
        $js = '
            document.observe("dom:loaded", function() {
                    ...
                    checkPaymentConflicts: function(enabler, type) {
                        ...
                        $$(".paypal-enabler").each(function(anotherEnabler) {
                            ...
                            if (conflict && !confirmationShowed && anotherEnabler.value == 1) {
                                if (isEvent) {
                                    confirmationApproved = confirm(\'' .  $this->helper('Mage_Core_Helper_Data')->jsQuoteEscape(__('There is already another PayPal solution enabled. Enable this solution instead?')) . '\');
                                } else {
                ...
                if (paypalConflictsObject.isConflict || paypalConflictsObject.ecMissed) {
                    var notification = \'' .  $this->helper('Mage_Core_Helper_Data')->jsQuoteEscape(__('The following error(s) occured:')) . '\';
                    if (paypalConflictsObject.isConflict) {
                        notification += "\\n  " + \'' .  $this->helper('Mage_Core_Helper_Data')->jsQuoteEscape(__('Some PayPal solutions conflict.')) . '\';
                    }
                    if (paypalConflictsObject.ecMissed) {
                        notification += "\\n  " + \'' .  $this->helper('Mage_Core_Helper_Data')->jsQuoteEscape(__('PayPal Express Checkout is not enabled.')) . '\';
                    }
                    notification += "\\n" + \'' .  $this->helper('Mage_Core_Helper_Data')->jsQuoteEscape(__('Please re-enable the previously enabled payment solutions.')) . '\';
                    setTimeout(function() {
                        alert(notification);
                    }, 1);
                }
