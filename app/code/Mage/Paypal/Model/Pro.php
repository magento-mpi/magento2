...
class Mage_Paypal_Model_Pro
{
    ...
    public function void(Varien_Object $payment)
    {
        if ($authTransactionId = $this->_getParentTransactionId($payment)) {
            ...
            Mage::throwException($this->_factoryHelper->get('Mage_Paypal_Helper_Data')->__('You need an authorization transaction to void.'));
        }
    }
    ...
    public function refund(Varien_Object $payment, $amount)
    {
        ...
        if ($captureTxnId) {
            ...
            Mage::throwException($this->_factoryHelper->get('Mage_Paypal_Helper_Data')->__('We can\'t issue a refund transaction because there is no capture transaction.'));
        }
    }
    ...
    public function validateRecurringProfile(Mage_Payment_Model_Recurring_Profile $profile)
    {
        ...
        if (strlen($profile->getSubscriberName()) > 32) { // up to 32 single-byte chars
            $errors[] = $this->_factoryHelper->get('Mage_Paypal_Helper_Data')->__('The subscriber name is too long.');
        }
        $refId = $profile->getInternalReferenceId(); // up to 127 single-byte alphanumeric
        if (strlen($refId) > 127) { //  || !preg_match('/^[a-z\d\s]+$/i', $refId)
            $errors[] = $this->_factoryHelper->get('Mage_Paypal_Helper_Data')->__('The merchant\'s reference ID format is not supported.');
        }
        ...
        if (strlen($refId) > 127) { //  || !preg_match('/^[a-z\d\s]+$/i', $scheduleDescr)
            $errors[] = $this->_factoryHelper->get('Mage_Paypal_Helper_Data')->__('The schedule description is too long.');
        }
        if ($errors) {
