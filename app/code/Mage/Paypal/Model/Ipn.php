...
class Mage_Paypal_Model_Ipn
{
    ...
    protected function _processOrder()
    {
        ...
        try {
            ...
            $comment = $this->_createIpnComment(__('Note: %s', $e->getMessage()), true);
            $comment->save();
            throw $e;
        }
    }
    ...
    protected function _registerAdjustment()
    {
         ...
         */
        $message = __('IPN "%s". A dispute has been resolved and closed. %s Transaction amount %s.', ucfirst($reasonCode), $notificationAmount, $reasonComment);
    ...
    protected function _registerDispute()
    {
         ...
         *  Add IPN comment about registered dispute
         */
        $message = __('IPN "%s". Case type "%s". Case ID "%s" %s', ucfirst($caseType), $caseTypeLabel, $caseId, $reasonComment);
        $this->_order->addStatusHistoryComment($message)
            ->setIsCustomerNotified(false)
            ->save();
    }
    ...
    protected function _registerPaymentReversal()
    {
         ...
         */
        $message = __('IPN "%s". %s Transaction amount %s. Transaction ID: "%s"', $this->_request['payment_status'], $reasonComment, $notificationAmount, $this->_request['txn_id']);
...
//            $comment->save();
    ...
    protected function _registerRecurringProfilePaymentCapture()
    {
        ...
        if ($invoice = $payment->getCreatedInvoice()) {
            $message = __('You notified customer about invoice #%s.', $invoice->getIncrementId());
    ...
    protected function _registerPaymentCapture()
    {
        ...
        if ($invoice && !$this->_order->getEmailSent()) {
            $this->_order->sendNewOrderEmail()->addStatusHistoryComment(
                __('You notified customer about invoice #%s.', $invoice->getIncrementId())
            )
            ->setIsCustomerNotified(true)
            ->save();
        }
    ...
    protected function _registerPaymentRefund()
    {
        ...
        if ($creditmemo = $payment->getCreatedCreditmemo()) {
            ...
            $comment = $this->_order->addStatusHistoryComment(
                    __('You notified customer about creditmemo #%s.', $creditmemo->getIncrementId())
                )
                ->setIsCustomerNotified(true)
                ->save();
        }
    ...
    protected function _createIpnComment($comment = '', $addToHistory = false)
    {
        ...
        $message = __('IPN "%s"', $paymentStatus);
        if ($comment) {
            $message .= ' ' . $comment;
        }
        if ($addToHistory) {
