...
class Mage_Backup_Model_Backup extends Varien_Object
{
    ...
    public function open($write = false)
    {
        ...
        if (!$write && !$this->_filesystem->isFile($this->_getFilePath())) {
            Mage::exception('Mage_Backup',
                __('The backup file "%s" does not exist.', $this->getFileName()));
        }
        ...
        try {
            ...
            throw new Mage_Backup_Exception_NotEnoughPermissions(
                __('Sorry, but we cannot read from or write to backup file "%s".', $this->getFileName())
            );
        }
    ...
    public function write($string)
    {
        try {
            ...
            Mage::exception('Mage_Backup',
                __('Something went wrong writing to the backup file "%s".', $this->getFileName()));
        }
