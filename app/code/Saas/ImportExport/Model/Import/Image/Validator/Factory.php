...
class Saas_ImportExport_Model_Import_Image_Validator_Factory
{
    ...
    public function createValidator()
    {
        ...
        $messageFilenameWrong = __("File name error (only latin a-z, A-Z, 0-9, '-' and '_' symbols are allowed in files and folders names) in:");
        // @codingStandardsIgnoreEnd
        $messageFilenameLimit = 'File name is too long:';
        $extensionsString = "'" . implode("', '", array_values($allowedExtensions)) . "'";
        $messageWrongImage = __('Unsupported image format (only %1 image file types are allowed) in:',
            $extensionsString);
        $messageFileSizeNotFound = __('File error for:');
        $messageFileSizeTooBig = __('File size is larger than %1 bytes in:', $sizeLimit);
        $dimensions = sprintf('%sx%s', $widthLimit, $heightLimit);
        $messageWrongImageSize = __('Image dimensions are larger than %1 in:', $dimensions);
