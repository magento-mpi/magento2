...
class Mage_Auth_Config
{
    ...
    {
        if (is_null($resource)) {
            $resource = Mage::getConfig()->getXml()->global->admin->acl->resources;
            $resourceName = null;
        } else {
    ...
    {
        $asserts = Mage::getConfig()->getXml()->global->admin->acl->asserts;
        if (''===$name) {
            return $asserts;
        }
    ...
    {
        $sets = Mage::getConfig()->getXml()->global->acl->privilegeSets;
        if (''===$name) {
            return $sets;
        } 
