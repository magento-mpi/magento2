...
class Mage_Sales_Model_Order_Entity extends Varien_Data_Object
{
    ...
    public function importDataObject(Varien_Data_Object $source, $type='')
    {
        ...
        if ($source->hasOrderEntityId()) {
            $this->setOrderEntityId($source->getOrderEntityId());
        }
        
        if ($source->hasOrderId()) {
     ...
     * Set attributes
     *
     * @param string|array|Mage_Sales_Model_Order_Attribute $var
     * @param mixed $value
    ...
    {
        if ($var instanceof Mage_Sales_Model_Order_Attribute) {
        ...
        $this->_order->getAttributes()->addItem($attr);
        return $this;
    }
...
/*
    ...
    {
        if (($var instanceof Mage_Sales_Model_Order_Attribute) || 
            (is_string($var) && !('order_entity_id'==$var || 'order_id'==$var || 'entity_type'==$var))) {
