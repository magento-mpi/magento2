...
class Mage_Sales_Model_Entity_Quote_Item_Collection extends Mage_Eav_Model_Entity_Collection_Abstract
{
    ...
    {
        Magento_Profiler::start('TEST1: '.__METHOD__);
        $productCollection = $this->_getProductCollection();
        Magento_Profiler::stop('TEST1: '.__METHOD__);
        $recollectQuote = false;
        foreach ($this as $item) {
            Magento_Profiler::start('TEST2: '.__METHOD__);
            ...
            $item->checkData();
            Magento_Profiler::stop('TEST2: '.__METHOD__);
        }
