...
class Mage_Sales_Model_Order extends Varien_Data_Object
{
    ...
    {
        $this->_entities = Mage::getModel('sales', 'order_entity_collection');
    ...
    {
        if (empty($this->_entitiesByType['order'])) {
            $this->addEntity('order', Mage::getModel('sales', 'order_entity'));
        }
        foreach ($this->getEntitiesByType('order') as $order) {
            return $order;
        }
    }
    ...
    {
        $entity = Mage::getModel('sales', 'order_entity');
        ...
        if (!$entity->hasOrderEntityId()) {
            $entity->setOrderEntityId($this->getNewEntityId());
        }
    ...
    {
        if ($entityToRemove instanceof Mage_Sales_Model_Order_Entity) {
            $entityId = $entity->getOrderEntityId();
        } else {
        ...
        foreach ($entities as $key=>$entity) {
            if ($entity->getOrderEntityId()===$entityToRemove->getOrderEntityId()) {
    ...
    public function getAddressByType($type)
    {
        foreach ($this->getEntitiesByType('address') as $addr) {
            if ($addr->getAttribute('order_address_type/varchar')==$type) {
                return $addr;
            }
        }
        return false;
    ...
    public function setAddress($addressType, Mage_Customer_Model_Address $address)
    {
        ...
        if (empty($existingAddress)) {
            $address->setOrderAddressType($addressType);
    ...
    {
        $attrLogicClasses = Mage::getConfig()->getGlobalCollection('salesOrderAttributes')->children();
        $totalsArr = array();
    ...
    public function collectShippingMethods()
    {
        ...
        $request->setOrderSubtotal($this->getOrderEntity()->getAttribute('subtotal'));
        $request->setPackageWeight($this->getOrderEntity()->getAttribute('weight'));
    ...
    public function getItemsAsArray($fields=null)
    {
        $arr = array();
        if (is_null($fields)) {
            $fields = Mage::getModel('sales', 'order_entity')->getDefaultType('', 'item');
        }
