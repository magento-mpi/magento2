...
/**
 * Processed quote mysql4 resource model
 *
 * @package    Mage
 * @subpackage Sales
 ...
 */
class Mage_Sales_Model_Mysql4_Quote_Processed extends Mage_Sales_Model_Mysql4_Quote
{
    /**
    ...
    public function __construct()
    {
        parent::__construct();
        $this->initResource();
        $this->setDocType('quote_processed');
    }
     ...
     *
     * @return Mage_Sales_Model_Mysql4_Quote_Processed
     */
    public function initResource()
    {
        $tablesExist = (bool)$this->_read->fetchOne("show tables like '{$this->_documentTable}_processed'");
        if ($tablesExist) {
            return $this;
        }
        ...
        foreach ($origTables as $tableName) {
            ...
            $sql = preg_replace('#^create table `'.$tableName.'#i', 'create table `'.$tableName.'_processed', $sql);
