...
class Mage_Catalog_Model_Resource_Layer_Filter_Price extends Mage_Core_Model_Resource_Db_Abstract
{
     ...
     * @return string
     */
    protected function _getPriceExpression($filter, $replaceAlias = true)
    {
        $priceExpression = $filter->getLayer()->getProductCollection()->getPriceExpression();
     ...
     * @return Zend_Db_Expr
     */
    protected function _getFullPriceExpression($filter)
    {
        return new Zend_Db_Expr('ROUND((' . $this->_getPriceExpression($filter) . ') * '
    ...
    public function loadPrices($filter, $limit, $offset = null, $lowerPrice = null, $upperPrice = null)
    {
        $select = $this->_getSelect($filter);
        $priceExpression = $this->_getPriceExpression($filter);
        $select->columns(array($this->_getFullPriceExpression($filter)));
    ...
    public function loadNextPrices($filter, $price, $rightIndex, $upperPrice = null)
    {
        ...
        $pricesSelect
            ->columns(array($this->_getFullPriceExpression($filter)))
    ...
    public function applyPriceRange($filter)
    {
        ...
        $priceExpr = $this->_getPriceExpression($filter, false);
