    ...
    public function render(Varien_Object $row)
    {
        ...
        if($row->getQueueStatus()==Mage_Newsletter_Model_Queue::STATUS_NEVER) {
           	if(!$row->getQueueStartAt() && $row->getSubscribersTotal()) {
        		$actions[] = array(
		    		'url' => Mage::helper('adminhtml')->getUrl('*/*/start', array('id'=>$row->getId())),
		    		'caption'	=> Mage::helper('newsletter')->__('Start')
		    	);
        	}
        } else if ($row->getQueueStatus()==Mage_Newsletter_Model_Queue::STATUS_SENDING) {
        	$actions[] = array(
		    		'url' => Mage::helper('adminhtml')->getUrl('*/*/pause', array('id'=>$row->getId())),
		    		'caption'	=>	Mage::helper('newsletter')->__('Pause')
		    );

		    $actions[] = array(
    			'url'		=>	Mage::helper('adminhtml')->getUrl('*/*/cancel', array('id'=>$row->getId())),
		    ...
		    $actions[] = array(
    		    'url' => Mage::helper('adminhtml')->getUrl('*/*/resume', array('id'=>$row->getId())),
        ...
        $actions[] = array(
			'url'		=>	Mage::helper('adminhtml')->getUrl('*/newsletter_template/preview',array('id'=>$row->getTemplateId())),
