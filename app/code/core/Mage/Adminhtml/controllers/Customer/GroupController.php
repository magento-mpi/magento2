...
class Mage_Adminhtml_Customer_GroupController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function newAction()
    {
        ...
        $this->_setActiveMenu('customer/group');
        $this->_addBreadcrumb(Mage::helper('customer')->__('Customers'), Mage::helper('customer')->__('Customers'));
        $this->_addBreadcrumb(Mage::helper('customer')->__('Customer Groups'), Mage::helper('customer')->__('Customer Groups'), Mage::helper('adminhtml')->getUrl('*/customer_group'));
    ...
    public function saveAction()
    {
        ...
        if ($code = $this->getRequest()->getParam('code')) {
            try {
                ...
                $this->getResponse()->setRedirect(Mage::helper('adminhtml')->getUrl('*/customer_group'));
                return;
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
                Mage::getSingleton('adminhtml/session')->setCustomerGroupData($customerGroup->getData());
                $this->getResponse()->setRedirect(Mage::helper('adminhtml')->getUrl('*/customer_group/edit', array('id' => $id)));
                return;
            }
        } else {
            $this->_forward('new');
