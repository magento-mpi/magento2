        ...
        if (!$this->checkQty($summaryQty)) {
            ...
            if (($this->getQty() - $summaryQty) < 0) {
                if ($this->getProductName()) {
                    ...
                    $result->setCartBackorderMessage(
                        $this->getProductName() . ' - ' .
                        Mage::helper('cataloginventory')->__('This product is not available in the requested quantity. %s of the items will be backordered.', ($backorderQty * 1), $this->getProductName())
                    );
                }
            }
