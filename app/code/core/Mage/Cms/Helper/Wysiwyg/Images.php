...
class Mage_Cms_Helper_Wysiwyg_Images extends Mage_Core_Helper_Abstract
{
    ...
    {
        return Mage::getBaseDir(Mage_Core_Model_App_Dir::MEDIA) . DS . Mage_Cms_Model_Wysiwyg_Config::IMAGE_DIRECTORY
            . DS;
    }
    ...
    {
        if (!$this->_currentUrl) {
            $path = str_replace(Mage::getBaseDir(Mage_Core_Model_App_Dir::MEDIA), '', $this->getCurrentPath());
            $path = trim($path, DS);
