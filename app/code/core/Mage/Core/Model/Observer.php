        ...
        if (!$adminSession->getSyncProcessStopWatch()) {
            ...
            if ($state == Mage_Core_Model_File_Storage_Flag::STATE_FINISHED) {
                ...
                if (isset($flagData['has_errors']) && $flagData['has_errors']) {
                    ...
                    $title          = Mage::helper('Mage_Adminhtml_Helper_Data')
                        ->__('An error has occured while syncronizing media storages.');
                    $description    = Mage::helper('Mage_Adminhtml_Helper_Data')
                        ->__('Media files failed to be synchronized during the media storages syncronization process.')
                        ->__('Refer to the log file for details.');
                } else {
                    $severity       = Mage_AdminNotification_Model_Inbox::SEVERITY_NOTICE;
                    $title          = Mage::helper('Mage_Adminhtml_Helper_Data')
                        ->__('Media storages synchronization has completed!');
                    $description    = Mage::helper('Mage_Adminhtml_Helper_Data')
                        ->__('Synchronization of media storages has been successfully completed.');
                }
