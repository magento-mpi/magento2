...
class Enterprise_AdminGws_Model_Observer extends Enterprise_AdminGws_Model_Observer_Abstract
{
            ...
            foreach ($websiteIds as $websiteId) {
                ...
                if (!$this->_role->getIsAll()) {
                    if (!$this->_role->hasWebsiteAccess($websiteId, true)) {
                        Mage::throwException(
                            Mage::helper('Enterprise_AdminGws_Helper_Data')->__('Website "%s" is not allowed in your current permission scope.', Mage::app()->getWebsite($websiteId)->getName())
                        );
                    }
                }
            }
        ...
        if (empty($storeGroupIds)) {
            ...
            foreach ($storeGroupIds as $storeGroupId) {
                ...
                if (count(array_diff($storeGroupIds, $this->_role->getStoreGroupIds()))) {
                    Mage::throwException(
                        Mage::helper('Enterprise_AdminGws_Helper_Data')->__('Not enough permissions to save specified Combination of Store Scopes.')
                    );
                }
            }
        }
    ...
    public function copyWebsiteCopyPermissions($observer)
    {
        ...
        $roles = Mage::getResourceSingleton('Mage_User_Model_Resource_Roles_Collection');
