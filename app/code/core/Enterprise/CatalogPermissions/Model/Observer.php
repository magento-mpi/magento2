...
class Enterprise_CatalogPermissions_Model_Observer
{
    ...
    public function applyCategoryPermission(Varien_Event_Observer $observer)
    {
        ...
        if ($observer->getEvent()->getCategory()->getIsHidden()) {
            ...
            Mage::throwException(
                Mage::helper('Enterprise_CatalogPermissions_Helper_Data')->__('You have no permissions to access this category')
            );
        }
        return $this;
    }
            ...
            if ($quoteItem->getDisableAddToCart() && !$quoteItem->isDeleted()) {
                ...
                if ($parentItem) {
                    $quote->setHasError(true)
                            ->addMessage(
                                Mage::helper('Enterprise_CatalogPermissions_Helper_Data')->__('The product "%s" cannot be added to cart.', $parentItem->getName())
                            );
                } else {
                     $quote->setHasError(true)
                            ->addMessage(
                                Mage::helper('Enterprise_CatalogPermissions_Helper_Data')->__('The product "%s" cannot be added to cart.', $quoteItem->getName())
                            );
                }
            }
        }
    ...
    public function applyProductPermission(Varien_Event_Observer $observer)
    {
        ...
        if ($observer->getEvent()->getProduct()->getIsHidden()) {
            ...
            Mage::throwException(
                Mage::helper('Enterprise_CatalogPermissions_Helper_Data')->__('You have no permissions to access this product')
            );
        }
