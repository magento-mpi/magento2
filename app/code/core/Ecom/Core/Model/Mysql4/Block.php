...
{    
    function loadGroup($groupName)
    {
        ...
        foreach ($blocks as $row) {
            $block = Ecom::createBlock($row['block_type'], $row['block_name']);
    ...
    function saveGroup($groupName)
    {
        ...
        $newBlocks = Ecom_Core_Block::getBlocksByGroup($groupName);
    ...
    function saveBlock($block)
    {
        ...
        if (is_string($block)) {
            ...
            if (!$block) {
                Ecom::exception('Non-existing block: '.$block);
            }
        }
        
        if (!$block instanceof Ecom_Core_Block_Abstract) {
            Ecom::exception('Invalid block object received in saveBlock');
        }
        
        $info = $block->getInfo();
        
        if (empty($info['groupName'])) {
            Ecom::exception('Can not save a block without group name set: '.$info['name']);
        }
