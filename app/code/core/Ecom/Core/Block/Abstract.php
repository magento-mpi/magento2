...
abstract class Ecom_Core_Block_Abstract
{
    /**
	...
	{
	    if (is_string($block)) {
	        $block = Ecom::getBlock($block);
	        if (!$block) {
	            Ecom::exception('Invalid block name to set child '.$name.': '.$block);
	        }
	    }
	    ...
	    if ($block->getInfo('anonymous')) {
	        ...
	        Ecom_Core_Block::unsetBlock($block->getInfo('name'));
    	    Ecom_Core_Block::setBlock($blockName, $block);
	...
	public function loadFromArray($arr)
	{
	    ...
	    if (!empty($arr['parent'])) {
	        $arr['parent']['block'] = Ecom::getBlock($arr['parent']['block']);
