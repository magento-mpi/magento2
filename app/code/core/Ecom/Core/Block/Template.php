...
class Ecom_Core_Block_Template extends Ecom_Core_Block_Abstract 
{
    ...
    {       
        $viewModule = Ecom::getModuleInfo($this->getAttribute('viewModule'));
        
        if (!$viewModule instanceof Ecom_Core_Module_Info) {
            Ecom::exception('Invalid view module name specified in block '.$this->getInfo('name').': '.$this->getAttribute('viewModule'));
        }
    ...
    public function toHtml()
    {
        ...
        if (!empty($this->_children)) {
            ...
            foreach ($this->_children as $name=>$block) {
                if ($block instanceof Ecom_Core_Block_Abstract) {
...
}// Class Ecom_Core_Block_Template END