...
{
    const TAGS_PREFIX    = "internal_EacceleratorTag:";
    const TAGS_MASTER_ID = "internal_EacceleratorMasterTags";
...
/**
    ...
    public function load($id, $doNotTestCacheValidity = false)
    {
        ...
        if ($doNotTestCacheValidity) {
            $this->_log("Zend_Cache_Backend_Eaccelerator::load() : \$doNotTestCacheValidity=true is unsupported by the Eaccelerator backend");
        }
    ...
    public function save($data, $id, $tags = array(), $specificLifetime = false)
    {
        ...
        $result1  = eaccelerator_put($id, array($data, time()), $lifetime);
        $result2  = true;
        ...
        if (count($tags) > 0) {
            foreach ($tags as $tag) {
                ...
                $old_tags = eaccelerator_get($this->_prepareId($tagid));
                if ($old_tags === false) {
                    $old_tags = array();
                }
                $old_tags[$id] = $id;
                $this->remove($tagid);
                $result2 = eaccelerator_put($this->_prepareId($tagid), $old_tags);
            }
        }
    ...
    {
        $old_tmaster_wrapper = eaccelerator_get($this->_getTagStorageId());
        ...
        if (in_array($tagId, $old_tmaster)) {
            ...
            return eaccelerator_put($this->_getTagStorageId(), array($old_tmaster, time()));
        }
    }
    ...
    {
        $tmaster_wrapper = eaccelerator_get($this->_getTagStorageId());
    ...
    {
        return eaccelerator_rm($this->_prepareId($id));
    }
