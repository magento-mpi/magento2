...
class Magento_Pci_Block_Adminhtml_Crypt_Key_Form extends Magento_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        $fieldset = $form->addFieldset('main_fieldset', array('legend' => Mage::helper('Magento_Pci_Helper_Data')->__('New Encryption Key')));
        $fieldset->addField('enc_key_note', 'note', array(
            'text' => Mage::helper('Magento_Pci_Helper_Data')->__('The encryption key is used to protect passwords and other sensitive data.')
        ));
        $fieldset->addField('generate_random', 'select', array(
            'name'    => 'generate_random',
            'label'   => Mage::helper('Magento_Pci_Helper_Data')->__('Auto-generate a Key'),
            'options' => array(
            ...
            'note'    => Mage::helper('Magento_Pci_Helper_Data')->__('The generated key will be displayed after changing.'),
        ));
        $fieldset->addField('crypt_key', 'text', array(
            'name'      => 'crypt_key',
            'label'     => Mage::helper('Magento_Pci_Helper_Data')->__('New Key'),
            'style'     => 'width:32em;',
            'maxlength' => 32,
        ));
