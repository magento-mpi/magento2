...
class Magento_CatalogEvent_Model_Event extends Magento_Core_Model_Abstract
{
    ...
    const IMAGE_PATH = 'magento/catalogevent';
    ...
    protected function _beforeSave()
    {
        parent::_beforeSave();
        $dateChanged = false;
        $fieldTitles = array('date_start' => Mage::helper('Magento_CatalogEvent_Helper_Data')->__('Start Date') , 'date_end' => Mage::helper('Magento_CatalogEvent_Helper_Data')->__('End Date'));
        foreach (array('date_start' , 'date_end') as $dateType) {
            $date = $this->getData($dateType);
            if (empty($date)) { // Date fields is required.
                Mage::throwException(Mage::helper('Magento_CatalogEvent_Helper_Data')->__('%s is required.', $fieldTitles[$dateType]));
            }
    ...
    public function validate()
    {
        ...
        if ($dateIsOk) {
            ...
            return array(Mage::helper('Magento_CatalogEvent_Helper_Data')->__('Please make sure the end date follows the start date.'));
        }
    }
