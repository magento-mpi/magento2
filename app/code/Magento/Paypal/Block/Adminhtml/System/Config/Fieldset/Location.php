        ...
        $js = '
            document.observe("dom:loaded", function() {
                    ...
                    checkPaymentConflicts: function(enabler, type) {
                        ...
                        $$(".paypal-enabler").each(function(anotherEnabler) {
                            ...
                            if (conflict && !confirmationShowed && anotherEnabler.value == 1) {
                                if (isEvent) {
                                    confirmationApproved = confirm(\'' .  $this->jsQuoteEscape(__('There is already another PayPal solution enabled. Enable this solution instead?')) . '\');
                                } else {
                ...
                if (paypalConflictsObject.isConflict || paypalConflictsObject.ecMissed) {
                    var notification = \'' .  $this->jsQuoteEscape(__('The following error(s) occured:')) . '\';
                    if (paypalConflictsObject.isConflict) {
                        notification += "\\n  " + \'' .  $this->jsQuoteEscape(__('Some PayPal solutions conflict.')) . '\';
                    }
                    if (paypalConflictsObject.ecMissed) {
                        notification += "\\n  " + \'' .  $this->jsQuoteEscape(__('PayPal Express Checkout is not enabled.')) . '\';
                    }
                    notification += "\\n" + \'' .  $this->jsQuoteEscape(__('Please re-enable the previously enabled payment solutions.')) . '\';
                    setTimeout(function() {
                        alert(notification);
                    }, 1);
                }
