...
class Magento_Paypal_Model_Report_Settlement extends Magento_Core_Model_Abstract
{
        ...
        foreach ($listing as $filename => $attributes) {
            ...
            if ($connection->read($filename, $localCsv)) {
                if (!is_writable($localCsv)) {
                    Mage::throwException(Mage::helper('Magento_Paypal_Helper_Data')->__('We cannot create a target file for reading reports.'));
                }
    ...
    public function getFieldLabel($field)
    {
        switch ($field) {
            case 'report_date':
                return Mage::helper('Magento_Paypal_Helper_Data')->__('Report Date');
            case 'account_id':
                return Mage::helper('Magento_Paypal_Helper_Data')->__('Merchant Account');
            case 'transaction_id':
                return Mage::helper('Magento_Paypal_Helper_Data')->__('Transaction ID');
            case 'invoice_id':
                return Mage::helper('Magento_Paypal_Helper_Data')->__('Invoice ID');
            case 'paypal_reference_id':
                return Mage::helper('Magento_Paypal_Helper_Data')->__('PayPal Reference ID');
            case 'paypal_reference_id_type':
                return Mage::helper('Magento_Paypal_Helper_Data')->__('PayPal Reference ID Type');
            case 'transaction_event_code':
                return Mage::helper('Magento_Paypal_Helper_Data')->__('Event Code');
            case 'transaction_event':
                return Mage::helper('Magento_Paypal_Helper_Data')->__('Event');
            case 'transaction_initiation_date':
                return Mage::helper('Magento_Paypal_Helper_Data')->__('Start Date');
            case 'transaction_completion_date':
                return Mage::helper('Magento_Paypal_Helper_Data')->__('Finish Date');
            case 'transaction_debit_or_credit':
                return Mage::helper('Magento_Paypal_Helper_Data')->__('Debit or Credit');
            case 'gross_transaction_amount':
                return Mage::helper('Magento_Paypal_Helper_Data')->__('Gross Amount');
            case 'fee_debit_or_credit':
                return Mage::helper('Magento_Paypal_Helper_Data')->__('Fee Debit or Credit');
            case 'fee_amount':
                return Mage::helper('Magento_Paypal_Helper_Data')->__('Fee Amount');
            case 'custom_field':
                return Mage::helper('Magento_Paypal_Helper_Data')->__('Custom');
            default:
                return $field;
        }
    }
