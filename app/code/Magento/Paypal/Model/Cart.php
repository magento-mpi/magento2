...
class Magento_Paypal_Model_Cart
{
    ...
    protected function _render()
    {
        ...
        if ($this->_isDiscountAsItem && $this->_totals[self::TOTAL_DISCOUNT]) {
            $this->addItem(Mage::helper('Magento_Paypal_Helper_Data')->__('Discount'), 1, -1.00 * $this->_totals[self::TOTAL_DISCOUNT],
        ...
        if ($this->_isShippingAsItem && (float)$this->_totals[self::TOTAL_SHIPPING]) {
            $this->addItem(Mage::helper('Magento_Paypal_Helper_Data')->__('Shipping'), 1, (float)$this->_totals[self::TOTAL_SHIPPING],
                $shippingItemId
            );
        }
