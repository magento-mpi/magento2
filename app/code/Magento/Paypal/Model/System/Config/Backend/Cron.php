...
class Magento_Paypal_Model_System_Config_Backend_Cron extends Magento_Core_Model_Config_Value
{
    ...
    {
        $cronExprString = '';
        $time = explode(',', Mage::getModel('Magento_Core_Model_Config_Value')->load('paypal/fetch_reports/time', 'path')->getValue());
        if (Mage::getModel('Magento_Core_Model_Config_Value')->load('paypal/fetch_reports/active', 'path')->getValue()) {
            $interval = Mage::getModel('Magento_Core_Model_Config_Value')->load(self::CRON_MODEL_PATH_INTERVAL, 'path')->getValue();
