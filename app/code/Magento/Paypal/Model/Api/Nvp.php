...
//    protected $_manageRecurringPaymentsProfileStatusResponse = array('PROFILEID');
    ...
    public function callManageRecurringPaymentsProfileStatus()
    {
        ...
        try {
            ...
            if ((in_array(11556, $this->_callErrors) && 'Cancel' === $request['ACTION'])
                ...
                Mage::throwException(Mage::helper('Magento_Paypal_Helper_Data')->__('We can\'t change the status because the current status doesn\'t match the real status.'));
            }
            throw $e;
        }
    }
    ...
    public function call($methodName, array $request)
    {
        ...
        if ($http->getErrno()) {
            ...
            Mage::throwException(Mage::helper('Magento_Paypal_Helper_Data')->__('We can\'t contact the PayPal gateway.'));
        }
        ...
        if (!$this->_validateResponse($methodName, $response)) {
            Mage::logException(new Exception(
                Mage::helper('Magento_Paypal_Helper_Data')->__("PayPal response hasn't required fields.")
            ));
            Mage::throwException(Mage::helper('Magento_Paypal_Helper_Data')->__('Something went wrong while processing your order.'));
        }
    ...
    protected function _handleCallErrors($response)
    {
        ...
        if ($errors) {
            ...
            Mage::logException($e);
            $e->setMessage(Mage::helper('Magento_Paypal_Helper_Data')->__('The PayPal gateway has rejected this request. %s', $errors));
            throw $e;
        }
    }
    ...
    protected function _applyStreetAndRegionWorkarounds(Magento_Object $address)
    {
        ...
        if ($address->getCountryId() && $address->getRegion()) {
            $regions = Mage::getModel('Magento_Directory_Model_Country')->loadByCode($address->getCountryId())->getRegionCollection()
    ...
    {
        switch ($value) {
            case 'None': return Magento_Paypal_Model_Info::PAYMENTSTATUS_NONE;
            ...
            case 'In-Progress': return Magento_Paypal_Model_Info::PAYMENTSTATUS_INPROGRESS;
            ...
            case 'Partially-Refunded': return Magento_Paypal_Model_Info::PAYMENTSTATUS_REFUNDEDPART;
            case 'Reversed': return Magento_Paypal_Model_Info::PAYMENTSTATUS_REVERSED;
            case 'Canceled-Reversal': return Magento_Paypal_Model_Info::PAYMENTSTATUS_UNREVERSED;
