...
class Magento_GiftCardAccount_Model_Giftcardaccount extends Magento_Core_Model_Abstract
{
    ...
    protected function _beforeSave()
    {
        ...
        if (is_numeric($this->getLifetime()) && $this->getLifetime() > 0) {
            ...
            if ($this->getDateExpires()) {
                ...
                if ($expirationDate < $currentDate) {
                    Mage::throwException(Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('An expiration date must be in the future.'));
                }
            } else {
        ...
        if ($this->getBalance() < 0) {
            Mage::throwException(Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('The balance cannot be less than zero.'));
        }
    }
        ...
        if ($this->isValid(true, true, $website)) {
            ...
            if (!$cards) {
                ...
                foreach ($cards as $one) {
                    if ($one['i'] == $this->getId()) {
                        Mage::throwException(Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('This gift card account is already in the quote.'));
                    }
                }
            }
            $cards[] = array(
    ...
    {
        if (!$this->getId()) {
            $this->_throwException(Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('Please correct the gift card account code: "%s".', $this->_requestedCode));
        }
        if (is_null($quote)) {
            $quote = $this->_getCheckoutSession()->getQuote();
        }
        ...
        $this->_throwException(Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('This gift card account wasn\'t found in the quote.'));
    }
    ...
    public function isValid($expirationCheck = true, $statusCheck = true, $websiteCheck = false, $balanceCheck = true)
    {
        if (!$this->getId()) {
            $this->_throwException(
                Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('Please correct the gift card account ID. Requested code: "%s"', $this->_requestedCode)
            );
        }
        ...
        if ($websiteCheck) {
            ...
            if ($this->getWebsiteId() != $website) {
                $this->_throwException(
                    Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('Please correct the gift card account website: %s.', $this->getWebsiteId())
                );
            }
        }
        ...
        if ($statusCheck && ($this->getStatus() != self::STATUS_ENABLED)) {
            $this->_throwException(
                Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('Gift card account %s is not enabled.', $this->getId())
            );
        }

        if ($expirationCheck && $this->isExpired()) {
            $this->_throwException(
                Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('Gift card account %s is expired.', $this->getId())
            );
        }
        ...
        if ($balanceCheck) {
            if ($this->getBalance() <= 0) {
                $this->_throwException(
                    Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('Gift card account %s has a zero balance.', $this->getId())
                );
            }
            if ($balanceCheck !== true && is_numeric($balanceCheck)) {
                if ($this->getBalance() < $balanceCheck) {
                    $this->_throwException(
                        Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('Gift card account %s balance is lower than the charged amount.', $this->getId())
                    );
                }
            }
        }
    ...
    public function getStatesAsOptionList()
    {
        ...
        $result[self::STATE_AVAILABLE] = Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('Available');
        $result[self::STATE_USED]      = Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('Used');
        $result[self::STATE_REDEEMED]  = Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('Redeemed');
        $result[self::STATE_EXPIRED]   = Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('Expired');
    ...
    public function redeem($customerId = null)
    {
        if ($this->isValid(true, true, true, true)) {
            ...
            if (!$customerId) {
                Mage::throwException(Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('You supplied an invalid customer ID.'));
            }

            $additionalInfo = Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('Gift Card Redeemed: %s. For customer #%s.', $this->getCode(), $customerId);
    ...
    protected function _throwException($realMessage, $fakeMessage = '')
    {
        ...
        Mage::logException($e);
        if (!$fakeMessage) {
            $fakeMessage = Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('Please correct the gift card code.');
        }
