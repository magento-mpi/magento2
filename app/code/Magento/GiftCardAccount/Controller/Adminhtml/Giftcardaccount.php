...
class Magento_GiftCardAccount_Controller_Adminhtml_Giftcardaccount extends Magento_Adminhtml_Controller_Action
{
    ...
    public function indexAction()
    {
        ...
        if ($this->_showCodePoolStatusMessage) {
            $usage = Mage::getModel('Magento_GiftCardAccount_Model_Pool')->getPoolUsageInfo();
            ...
            Mage::getSingleton('Magento_Adminhtml_Model_Session')->$function(
                Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('Code Pool used: <b>%.2f%%</b> (free <b>%d</b> of <b>%d</b> total). Generate new code pool <a href="%s">here</a>.', $usage->getPercent(), $usage->getFree(), $usage->getTotal(), $url)
            );
        }
    ...
    public function editAction()
    {
        ...
        if (!$model->getId() && $id) {
            Mage::getSingleton('Magento_Adminhtml_Model_Session')->addError(Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('This gift card account has been deleted.'));
            $this->_redirect('*/*/');
            return;
        }
        ...
        $this->loadLayout()
            ->_addBreadcrumb($id ? Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('Edit Gift Card Account') : Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('New Gift Card Account'),
                             $id ? Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('Edit Gift Card Account') : Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('New Gift Card Account'))
            ->_addContent(
    ...
    public function saveAction()
    {
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                if (!is_null($sending)) {
                    if ($sending) {
                        Mage::getSingleton('Magento_Adminhtml_Model_Session')->addSuccess(Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('You saved the gift card account.'));
                    } else {
                        Mage::getSingleton('Magento_Adminhtml_Model_Session')->addError(Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('You saved the gift card account, but an email was not sent.'));
                    }
                } else {
                    Mage::getSingleton('Magento_Adminhtml_Model_Session')->addSuccess(Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('You saved the gift card account.'));

                    if ($status) {
                        Mage::getSingleton('Magento_Adminhtml_Model_Session')->addNotice(Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('An email was not sent because the gift card account is not active.'));
                    }
                }
    ...
    public function deleteAction()
    {
        ...
        if ($id = $this->getRequest()->getParam('id')) {
            try {
                ...
                Mage::getSingleton('Magento_Adminhtml_Model_Session')->addSuccess(Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('This gift card account has been deleted.'));
                // go to grid
                $this->_redirect('*/*/');
                return;
        ...
        // display error message
        Mage::getSingleton('Magento_Adminhtml_Model_Session')->addError(Mage::helper('Magento_GiftCardAccount_Helper_Data')->__("We couldn't find a gift card account to delete."));
        // go to grid
        $this->_redirect('*/*/');
    }
    ...
    {
        try {
            Mage::getModel('Magento_GiftCardAccount_Model_Pool')->generatePool();
            Mage::getSingleton('Magento_Adminhtml_Model_Session')->addSuccess(Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('New code pool was generated.'));
        } catch (Magento_Core_Exception $e) {
            Mage::getSingleton('Magento_Adminhtml_Model_Session')->addError($e->getMessage());
        } catch (Exception $e) {
            Mage::getSingleton('Magento_Adminhtml_Model_Session')->addException($e, Mage::helper('Magento_GiftCardAccount_Helper_Data')->__('We were unable to generate a new code pool.'));
        }
    ...
    public function massDeleteAction()
    {
        ...
        if (!is_array($ids)) {
            ...
            try {
                foreach ($ids as $id) {
                    $model = Mage::getSingleton('Magento_GiftCardAccount_Model_Giftcardaccount')->load($id);
                    $model->delete();
                }
