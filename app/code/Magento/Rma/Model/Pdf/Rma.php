...
class Magento_Rma_Model_Pdf_Rma extends Magento_Sales_Model_Order_Pdf_Abstract
{
    ...
    public function getPdf($rmaArray = array())
    {
        ...
        if (!(is_array($rmaArray) && (count($rmaArray) == 1))){
            Mage::throwException(Mage::helper('Magento_Rma_Helper_Data')->__('Only one RMA is available for printing'));
        }
        $rma = $rmaArray[0];
        ...
        $page->drawText(
            Mage::helper('Magento_Rma_Helper_Data')->__('Return # ') . $rma->getIncrementId() . ' - ' . $rma->getStatusLabel(),
            35,
            $this->y - 40,
            'UTF-8'
        );
        ...
        $page->drawText(
            Mage::helper('Magento_Rma_Helper_Data')->__('Return Date: ') .
                Mage::helper('Magento_Core_Helper_Data')->formatDate($rma->getDateRequested(), 'medium', false),
            35,
            $this->y - 50,
            'UTF-8'
        );
        ...
        $page->drawText(
            Mage::helper('Magento_Rma_Helper_Data')->__('Order # ') . $rma->getOrder()->getIncrementId(),
            35,
            $this->y - 60,
            'UTF-8'
        );

        $text = Mage::helper('Magento_Rma_Helper_Data')->__('Order Date: ');
        $text .= Mage::helper('Magento_Core_Helper_Data')->formatDate(
        ...
        $returnAddress = $this
            ->_formatAddress(Mage::helper('Magento_Rma_Helper_Data')
        ...
        $page->setFillColor(new Zend_Pdf_Color_GrayScale(0));
        $this->_setFontRegular($page);
        $page->drawText(Mage::helper('Magento_Rma_Helper_Data')->__('Shipping Address:'), 35, $this->y - 10, 'UTF-8');

        $page->drawText(Mage::helper('Magento_Rma_Helper_Data')->__('Return Address:'), 315, $this->y - 10, 'UTF-8');
    ...
    protected function _addItemTableHead($page)
    {
        ...
        $page->drawText(
            Mage::helper('Magento_Rma_Helper_Data')->__('Product Name'),
            $this->getProductNameX(),
            $this->y,
            'UTF-8'
        );
        $page->drawText(
            Mage::helper('Magento_Rma_Helper_Data')->__('SKU'),
            $this->getProductSkuX(),
            $this->y,
            'UTF-8'
        );
        $page->drawText(
            Mage::helper('Magento_Rma_Helper_Data')->__('Condition'),
            $this->getConditionX(),
            $this->y,
            'UTF-8'
        );
        $page->drawText(
            Mage::helper('Magento_Rma_Helper_Data')->__('Resolution'),
            $this->getResolutionX(),
            $this->y,
            'UTF-8'
        );
        $page->drawText(
            Mage::helper('Magento_Rma_Helper_Data')->__('Requested Qty'),
            $this->getQtyRequestedX(),
            $this->y,
            'UTF-8'
        );
        $page->drawText(
            Mage::helper('Magento_Rma_Helper_Data')->__('Qty'),
            $this->getQtyX(),
            $this->y,
            'UTF-8'
        );
        $page->drawText(
            Mage::helper('Magento_Rma_Helper_Data')->__('Status'),
            $this->getStatusX(),
            $this->y,
            'UTF-8'
        );
    ...
    protected function _drawRmaItem($item, $page)
    {
        ...
        $page->drawText(
            Mage::helper('Magento_Rma_Helper_Data')->getQty($item),
            $this->getQtyX(),
            $this->y,
            'UTF-8'
        );

        $status = Mage::helper('Magento_Core_Helper_String')->str_split($item->getStatusLabel(), 25);
    ...
    protected function _drawCustomOptions($optionsArray = array(), $page)
    {
        ...
        foreach ($optionsArray as $value) {
            ...
            $productOptions = Mage::helper('Magento_Core_Helper_String')->str_split($optionRowString, 60, true, true);
