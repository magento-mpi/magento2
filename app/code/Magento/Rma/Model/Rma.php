...
class Magento_Rma_Model_Rma extends Magento_Core_Model_Abstract
{
    ...
    protected function _afterSave()
    {
        ...
        $gridModel = Mage::getModel('Magento_Rma_Model_Grid');
        $gridModel->addData($this->getData());
        $gridModel->save();

        Mage::getModel('Magento_Rma_Model_Rma_Status_History')->setRma($this)->saveSystemComment();
    ...
    {
        return Mage::getModel('Magento_Rma_Model_Rma_Source_Status')->getAllOptionsForGrid();
    }
    ...
    public function getStatusLabel()
    {
        if (is_null(parent::getStatusLabel())){
            $this->setStatusLabel(
                Mage::getModel('Magento_Rma_Model_Rma_Source_Status')->getItemLabel($this->getStatus())
            );
        }
    ...
    protected function _preparePost($item)
    {
        ...
        if (!empty($preparePost['status'])) {
            $status = Mage::getModel('Magento_Rma_Model_Item_Attribute_Source_Status');
        ...
        if ($errors) {
            $session = Mage::getSingleton('Magento_Core_Model_Session');
            $session->addError(
                Mage::helper('Magento_Rma_Helper_Data')->__('There is an error in quantities for item %s.', $preparePost['product_name'])
            );
        }
    ...
    protected function _checkPost($itemModels, $orderId)
    {
            ...
            if ($this->getIsUpdate()) {
                ...
                foreach ($validation as $key => $value) {
                    if (isset($previousValue) && $value > $previousValue) {
                        $errors[] = Mage::helper('Magento_Rma_Helper_Data')->__('There is an error in quantities for item %s.', $escapedProductName);
                ...
                $qtyToStatus = array(
                    'qty_authorized' => array(
                            'name' => Mage::helper('Magento_Rma_Helper_Data')->__('Authorized Qty'),
                            'status' => Magento_Rma_Model_Rma_Source_Status::STATE_AUTHORIZED
                        ),
                    'qty_returned' => array(
                            'name' => Mage::helper('Magento_Rma_Helper_Data')->__('Returned Qty'),
                            'status' => Magento_Rma_Model_Rma_Source_Status::STATE_RECEIVED
                        ),
                    'qty_approved' => array(
                            'name' => Mage::helper('Magento_Rma_Helper_Data')->__('Approved Qty'),
                ...
                foreach ($qtyToStatus as $qtyKey => $qtyValue) {
                    if ($item->getStatus() === $qtyValue['status']
                        ...
                        $errors[] = Mage::helper('Magento_Rma_Helper_Data')->__('%s for item %s cannot be empty.', $qtyValue['name'], $escapedProductName);
        ...
        foreach ($itemsArray as $key=>$qty) {
            ...
            if (!array_key_exists($key, $availableItemsArray)) {
                $errors[] = Mage::helper('Magento_Rma_Helper_Data')->__('You cannot return %s.', $escapedProductName);
            }
            if (isset($availableItemsArray[$key]) && $availableItemsArray[$key]['qty'] < $qty) {
                $errors[] = Mage::helper('Magento_Rma_Helper_Data')->__('Quantity of %s is greater than you can return.', $escapedProductName);
    ...
    protected function _validateEmail($value)
    {
        ...
        $validator->setMessage(
            Mage::helper('Magento_Rma_Helper_Data')->__('You entered an invalid type: "%s".', $label),
            Zend_Validate_EmailAddress::INVALID
        );
        $validator->setMessage(
            Mage::helper('Magento_Rma_Helper_Data')->__('You entered an invalid email address: "%s".', $label),
            Zend_Validate_EmailAddress::INVALID_FORMAT
        );
        $validator->setMessage(
            Mage::helper('Magento_Rma_Helper_Data')->__('You entered an invalid hostname: "%s"', $label),
        ...
        $validator->setMessage(
            Mage::helper('Magento_Rma_Helper_Data')->__('"%s" is longer than allowed.', $label),
    ...
    public function getTrackingNumbers()
    {
        if (is_null($this->_trackingNumbers)) {
            $this->_trackingNumbers = Mage::getModel('Magento_Rma_Model_Shipping')
            ->getCollection()
    ...
    public function getShippingLabel()
    {
        if (is_null($this->_shippingLabel)) {
            $this->_shippingLabel = Mage::getModel('Magento_Rma_Model_Shipping')
            ->getCollection()
            ->addFieldToFilter('rma_entity_id', $this->getEntityId())
    ...
    {
        return (bool)(
            Mage::getModel('Magento_Rma_Model_Rma_Source_Status')->getButtonDisabledStatus($this->getStatus())
