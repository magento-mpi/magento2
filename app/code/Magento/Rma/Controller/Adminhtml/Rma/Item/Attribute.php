...
class Magento_Rma_Controller_Adminhtml_Rma_Item_Attribute extends Magento_Adminhtml_Controller_Action
{
    ...
    protected function _initAction()
    {
        $this->loadLayout()
            ...
            ->_addBreadcrumb(
                Mage::helper('Magento_Rma_Helper_Data')->__('RMA'),
                Mage::helper('Magento_Rma_Helper_Data')->__('RMA'))
            ->_addBreadcrumb(
                Mage::helper('Magento_Rma_Helper_Data')->__('Manage RMA Item Attributes'),
                Mage::helper('Magento_Rma_Helper_Data')->__('Manage RMA Item Attributes'));
        return $this;
    }
    ...
    {
        $attribute = Mage::getModel('Magento_Rma_Model_Item_Attribute');
    ...
    public function editAction()
    {
        ...
        if ($attributeId) {
            ...
            if (!$attributeObject->getId()) {
                $this->_getSession()
                    ->addError(Mage::helper('Magento_Rma_Helper_Data')->__('Attribute is no longer exists.'));
                $this->_redirect('*/*/');
                return;
            }
            if ($attributeObject->getEntityTypeId() != $this->_getEntityType()->getId()) {
                $this->_getSession()->addError(Mage::helper('Magento_Rma_Helper_Data')->__('You cannot edit this attribute.'));
                $this->_redirect('*/*/');
                return;
            }
        ...
        $label = $attributeObject->getId()
            ? Mage::helper('Magento_Rma_Helper_Data')->__('Edit Return Item Attribute')
            : Mage::helper('Magento_Rma_Helper_Data')->__('New Return Item Attribute');
        ...
        if (!$attributeId) {
            ...
            if ($attributeObject->getId()) {
                $this->_getSession()->addError(
                    Mage::helper('Magento_Rma_Helper_Data')->__('An attribute with the same code already exists.')
                );
    ...
    public function saveAction()
    {
        ...
        if ($this->getRequest()->isPost() && $data) {
            ...
            if ($attributeId) {
                ...
                if ($attributeObject->getEntityTypeId() != $this->_getEntityType()->getId()) {
                    $this->_getSession()->addError(
                        Mage::helper('Magento_Rma_Helper_Data')->__('You cannot edit this attribute.')
                    );
            ...
            try {
                ...
                $this->_getSession()->addSuccess(
                    Mage::helper('Magento_Rma_Helper_Data')->__('You saved the RMA item attribute.')
                );
                ...
                $this->_getSession()->addException($e,
                    Mage::helper('Magento_Rma_Helper_Data')->__('Something went wrong saving the RMA item attribute.')
                );
                $this->_getSession()->setAttributeData($data);
                $this->_redirect('*/*/edit', array('_current' => true));
                return;
    ...
    public function deleteAction()
    {
        ...
        if ($attributeId) {
            ...
            if ($attributeObject->getEntityTypeId() != $this->_getEntityType()->getId()
                ...
                $this->_getSession()->addError(
                    Mage::helper('Magento_Rma_Helper_Data')->__('You cannot delete this attribute.')
                );
                $this->_redirect('*/*/');
                return;
            }
            try {
                ...
                $this->_getSession()->addSuccess(
                    Mage::helper('Magento_Rma_Helper_Data')->__('You deleted the RMA attribute.')
                );
                $this->_redirect('*/*/');
                return;
            } catch (Magento_Core_Exception $e) {
                ...
                $this->_getSession()->addException($e,
                    Mage::helper('Magento_Rma_Helper_Data')->__('Something went wrong deleting the RMA item attribute.')
                );
                $this->_redirect('*/*/edit', array('attribute_id' => $attributeId, '_current' => true));
                return;
            }
