...
class Magento_Rma_Block_Adminhtml_Rma_Edit extends Magento_Adminhtml_Block_Widget_Form_Container
{
        ...
        if (!$statusIsClosed) {
            ...
            $this->_addButton('close', array(
                ...
                'onclick'   => 'confirmSetLocation(\''
                                . Mage::helper('Magento_Rma_Helper_Data')->__('Are you sure you want to close this returns request?')
                                .'\', \'' . $this->getCloseUrl().'\')'
                )
            );
        } else {
    ...
    public function getHeaderText()
    {
        if ($this->getRma()->getId()) {
            return Mage::helper('Magento_Rma_Helper_Data')->__('Return #%s - %s', intval($this->getRma()->getIncrementId()), $this->getRma()->getStatusLabel());
        }
