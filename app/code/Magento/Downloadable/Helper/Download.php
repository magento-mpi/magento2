 ...
 */
class Magento_Downloadable_Helper_Download extends Magento_Core_Helper_Abstract
{
    ...
    protected function _getHandle()
    {
        if (!$this->_resourceFile) {
            Mage::throwException(Mage::helper('Magento_Downloadable_Helper_Data')->__('Please set resource file and link type.'));
        }
        ...
        if (is_null($this->_handle)) {
            if ($this->_linkType == self::LINK_TYPE_URL) {
                ...
                if (!isset($urlProp['scheme']) || strtolower($urlProp['scheme'] != 'http')) {
                    Mage::throwException(Mage::helper('Magento_Downloadable_Helper_Data')->__('Please correct the download URL scheme.'));
                }
                if (!isset($urlProp['host'])) {
                    Mage::throwException(Mage::helper('Magento_Downloadable_Helper_Data')->__('Please correct the download URL host.'));
                }
                ...
                if ($this->_handle === false) {
                    Mage::throwException(Mage::helper('Magento_Downloadable_Helper_Data')->__('Something went wrong connecting to the host. Error: %s.', $errstr));
                }
                ...
                if (!isset($this->_urlHeaders['code']) || $this->_urlHeaders['code'] != 200) {
                    Mage::throwException(Mage::helper('Magento_Downloadable_Helper_Data')->__('Something went wrong while getting the requested content.'));
                }
            }
            elseif ($this->_linkType == self::LINK_TYPE_FILE) {
                ...
                if (!$this->_handle->fileExists($this->_resourceFile, true)) {
                    Mage::throwException(Mage::helper('Magento_Downloadable_Helper_Data')->__("We can't find this file."));
                }
                $this->_handle->streamOpen($this->_resourceFile, 'r');
            }
            else {
                Mage::throwException(Mage::helper('Magento_Downloadable_Helper_Data')->__('Invalid download link type.'));
            }
        }
        return $this->_handle;
    }
     ...
     * @return Magento_Downloadable_Helper_Download
     */
