...
class CategoryUrlGenerator
{
    ...
    public function generate($category)
    {
        ...
        if ($this->category->getParentId() == \Magento\Catalog\Model\Category::TREE_ROOT_ID) {
            $this->category = null;
            return [];
        }
    ...
    protected function generatePerStore()
    {
        ...
        $urls = array_merge($urls, $this->generateForChildrenPerStore());
        $urls = array_merge($urls, $this->generateCustom());
        $urls = array_merge($urls, $this->generateRewritesHistory());
        return $urls;
    }
