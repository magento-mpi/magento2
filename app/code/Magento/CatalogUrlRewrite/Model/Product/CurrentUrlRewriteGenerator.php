...
class CurrentUrlRewriteGenerator
{
    ...
    protected function generateForAutogenerated($url, $storeId)
    {
        ...
        if ($this->product->getData('save_rewrites_history')) {
            ...
            if ($url->getRequestPath() !== $targetPath) {
                $urls[] = $this->urlRewriteBuilder->setEntityType(ProductUrlRewriteGenerator::ENTITY_TYPE)
                ->setEntityId($this->product->getId())->setStoreId($url->getStoreId())
                ->setRequestPath($url->getRequestPath())->setTargetPath($targetPath)
                ->setIsAutogenerated(0)->setRedirectCode(OptionProvider::PERMANENT)
                ->setMetadata($this->buildMetadata($url, $category))->create();
            }
        }
        return $urls;
    }
    ...
    protected function generateForCustom($url, $storeId)
    {
        ...
        if ($url->getRequestPath() !== $targetPath) {
            $urls[] = $this->urlRewriteBuilder->setEntityType(ProductUrlRewriteGenerator::ENTITY_TYPE)
                ->setEntityId($this->product->getId())->setStoreId($storeId)
                ->setRequestPath($url->getRequestPath())->setTargetPath($targetPath)
                ->setIsAutogenerated(0)->setRedirectCode($url->getRedirectType())
                ->setMetadata($this->buildMetadata($url, $category))->setDescription($url->getDescription())->create();
        }
        return $urls;
    }
