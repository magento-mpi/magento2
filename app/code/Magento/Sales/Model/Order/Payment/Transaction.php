 ...
 *
 * @method Magento_Sales_Model_Resource_Order_Payment_Transaction _getResource()
 * @method Magento_Sales_Model_Resource_Order_Payment_Transaction getResource()
 * @method int getParentId()
 * @method Magento_Sales_Model_Order_Payment_Transaction setParentId(int $value)
 * @method Magento_Sales_Model_Order_Payment_Transaction setOrderId(int $value)
 * @method int getPaymentId()
 * @method Magento_Sales_Model_Order_Payment_Transaction setPaymentId(int $value)
 * @method string getTxnId()
 * @method string getParentTxnId()
 * @method string getTxnType()
 * @method int getIsClosed()
 * @method Magento_Sales_Model_Order_Payment_Transaction setIsClosed(int $value)
 * @method string getCreatedAt()
 * @method Magento_Sales_Model_Order_Payment_Transaction setCreatedAt(string $value)
 *
 * @category    Magento
 * @package     Magento_Sales
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class Magento_Sales_Model_Order_Payment_Transaction extends Magento_Core_Model_Abstract
{
    /**
     ...
     * @var Magento_Sales_Model_Order_Payment
     */
     ...
     */
    public function setOrderPaymentObject(Magento_Sales_Model_Order_Payment $payment)
    {
    ...
    public function setParentTxnId($parentTxnId, $txnId = null)
    {
        ...
        if (empty($txnId)) {
            if ('' == $this->getTxnId()) {
                Mage::throwException(
                    Mage::helper('Magento_Sales_Helper_Data')->__('The parent transaction ID must have a transaction ID.')
                );
            }
        } else {
            $this->setTxnId($txnId);
     ...
     * @return Magento_Sales_Model_Order_Payment_Transaction|array|null
     */
    ...
    {
        if (is_object($value)) {
            Mage::throwException(Mage::helper('Magento_Sales_Helper_Data')->__('Payment transactions disallow storing objects.'));
        }
    ...
    public function close($shouldSave = true)
    {
        ...
        if (1 == $this->getIsClosed() && $this->_isFailsafe) {
            Mage::throwException(Mage::helper('Magento_Sales_Helper_Data')->__('The transaction "%s" (%s) is already closed.', $this->getTxnId(), $this->getTxnType()));
        }
        $this->setIsClosed(1);
        if ($shouldSave) {
            $this->save();
    ...
    public function setOrder($order = null)
    {
        if (null === $order || $order === true) {
            ...
            Mage::throwException(Mage::helper('Magento_Sales_Helper_Data')->__('Set order for existing transactions not allowed'));
        }
    ...
    protected function _loadChildren()
    {
        ...
        if (!$paymentId) {
            Mage::throwException(Mage::helper('Magento_Sales_Helper_Data')->__('At minimum, you need to set a payment ID.'));
        }
    ...
    {
        return array(
            Magento_Sales_Model_Order_Payment_Transaction::TYPE_ORDER    => Mage::helper('Magento_Sales_Helper_Data')->__('Order'),
            Magento_Sales_Model_Order_Payment_Transaction::TYPE_AUTH    => Mage::helper('Magento_Sales_Helper_Data')->__('Authorization'),
            Magento_Sales_Model_Order_Payment_Transaction::TYPE_CAPTURE => Mage::helper('Magento_Sales_Helper_Data')->__('Capture'),
            Magento_Sales_Model_Order_Payment_Transaction::TYPE_VOID    => Mage::helper('Magento_Sales_Helper_Data')->__('Void'),
            Magento_Sales_Model_Order_Payment_Transaction::TYPE_REFUND  => Mage::helper('Magento_Sales_Helper_Data')->__('Refund')
        );
    }
    ...
    protected function _verifyTxnType($txnType = null)
    {
        ...
        switch ($txnType) {
            ...
            case self::TYPE_REFUND:
                break;
            default:
                Mage::throwException(Mage::helper('Magento_Sales_Helper_Data')->__('We found an unsupported transaction type "%s".', $txnType));
        }
    }
     ...
     * @param bool $dryRun
     * @return Magento_Sales_Model_Order_Payment|null|false
     * @throws Magento_Core_Exception
     */
    protected function _verifyPaymentObject($dryRun = false)
    {
        if (!$this->_paymentObject || !$this->getOrderId()) {
            if (!$dryRun) {
                Mage::throwException(Mage::helper('Magento_Sales_Helper_Data')->__('Please set a proper payment object.'));
            }
        }
        return $this->_paymentObject;
    }
    ...
    protected function _verifyTxnId($txnId)
    {
        if (null !== $txnId && 0 == strlen($txnId)) {
            Mage::throwException(Mage::helper('Magento_Sales_Helper_Data')->__('The Transaction ID field cannot be empty.'));
        }
    }
    ...
    {
        if (!$this->getId()) {
            Mage::throwException(Mage::helper('Magento_Sales_Helper_Data')->__('You can\'t do this without a transaction object.'));
        }
        $this->_verifyTxnType();
    }
}
