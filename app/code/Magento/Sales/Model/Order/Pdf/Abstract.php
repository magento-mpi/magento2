...
abstract class Magento_Sales_Model_Order_Pdf_Abstract extends Magento_Object
{
     ...
     *
     * @param Zend_Pdf_Page $page
     * @param Magento_Sales_Model_Order $obj
     * @param bool $putOrderId
     */
    protected function insertOrder(&$page, $obj, $putOrderId = true)
    {
        if ($obj instanceof Magento_Sales_Model_Order) {
            $shipment = null;
            $order = $obj;
        } elseif ($obj instanceof Magento_Sales_Model_Order_Shipment) {
            $shipment = $obj;
        ...
        if ($putOrderId) {
            $page->drawText(
                Mage::helper('Magento_Sales_Helper_Data')->__('Order # ') . $order->getRealOrderId(), 35, ($top -= 30), 'UTF-8'
            );
        }
        $page->drawText(
            Mage::helper('Magento_Sales_Helper_Data')->__('Order Date: ') . Mage::helper('Magento_Core_Helper_Data')->formatDate(
        ...
        $page->drawText(Mage::helper('Magento_Sales_Helper_Data')->__('Sold to:'), 35, ($top - 15), 'UTF-8');

        if (!$order->getIsVirtual()) {
            $page->drawText(Mage::helper('Magento_Sales_Helper_Data')->__('Ship to:'), 285, ($top - 15), 'UTF-8');
        } else {
            $page->drawText(Mage::helper('Magento_Sales_Helper_Data')->__('Payment Method:'), 285, ($top - 15), 'UTF-8');
        }
        ...
        if (!$order->getIsVirtual()) {
            ...
            $this->_setFontBold($page, 12);
            $page->setFillColor(new Zend_Pdf_Color_GrayScale(0));
            $page->drawText(Mage::helper('Magento_Sales_Helper_Data')->__('Payment Method'), 35, $this->y, 'UTF-8');
            $page->drawText(Mage::helper('Magento_Sales_Helper_Data')->__('Shipping Method:'), 285, $this->y , 'UTF-8');
        ...
        if ($order->getIsVirtual()) {
            ...
            $yShipments = $this->y;
            $totalShippingChargesText = "(" . Mage::helper('Magento_Sales_Helper_Data')->__('Total Shipping Charges') . " "
            ...
            if (count($tracks)) {
                ...
                $page->setFillColor(new Zend_Pdf_Color_GrayScale(0));
                //$page->drawText(Mage::helper('Magento_Sales_Helper_Data')->__('Carrier'), 290, $yShipments - 7 , 'UTF-8');
                $page->drawText(Mage::helper('Magento_Sales_Helper_Data')->__('Title'), 290, $yShipments - 7, 'UTF-8');
                $page->drawText(Mage::helper('Magento_Sales_Helper_Data')->__('Number'), 410, $yShipments - 7, 'UTF-8');
        ...
        foreach ($totals as $index => $totalInfo) {
            if (!empty($totalInfo['model'])) {
                ...
                if ($totalModel instanceof Magento_Sales_Model_Order_Pdf_Total_Default) {
                    ...
                    Mage::throwException(
                        Mage::helper('Magento_Sales_Helper_Data')->__('The PDF total model should extend Magento_Sales_Model_Order_Pdf_Total_Default.')
                    );
                }
            } else {
    ...
    protected function _getRenderer($type)
    {
        ...
        if (!isset($this->_renderers[$type])) {
            Mage::throwException(Mage::helper('Magento_Sales_Helper_Data')->__('We found an invalid renderer model.'));
        }
     ...
     * @return Zend_Pdf_Page
     */
    protected function _drawItem(Magento_Object $item, Zend_Pdf_Page $page, Magento_Sales_Model_Order $order)
    {
    ...
    protected function _getPdf()
    {
        if (!$this->_pdf instanceof Zend_Pdf) {
            Mage::throwException(Mage::helper('Magento_Sales_Helper_Data')->__('Please define the PDF object before using.'));
        }
    ...
    public function drawLineBlocks(Zend_Pdf_Page $page, array $draw, array $pageSettings = array())
    {
        foreach ($draw as $itemsProp) {
            if (!isset($itemsProp['lines']) || !is_array($itemsProp['lines'])) {
                Mage::throwException(Mage::helper('Magento_Sales_Helper_Data')->__('We don\'t recognize the draw line data. Please define the "lines" array.'));
            }
