 ...
 */
abstract class Magento_Sales_Model_Quote_Item_Abstract extends Magento_Core_Model_Abstract
    implements Magento_Catalog_Model_Product_Configuration_Item_Interface
{
     ...
     *
     * @return  Magento_Sales_Model_Quote_Item_Abstract
     */
     ...
     * Set parent item
     *
     * @param  Magento_Sales_Model_Quote_Item $parentItem
    ...
    public function checkData()
    {
        ...
        try {
            ...
            $this->setHasError(true);
            $this->setMessage(Mage::helper('Magento_Sales_Helper_Data')->__('Item qty declaration error'));
        }
        ...
        try {
            ...
            $this->getQuote()->setHasError(true)
                ->addMessage(Mage::helper('Magento_Sales_Helper_Data')->__('Some of the products below do not have all the required options.'));
        } catch (Exception $e) {
            $this->setHasError(true)
                ->setMessage(Mage::helper('Magento_Sales_Helper_Data')->__('Something went wrong during the item options declaration.'));
            $this->getQuote()->setHasError(true)
                ->addMessage(Mage::helper('Magento_Sales_Helper_Data')->__('We found an item options declaration error.'));
        }

        if ($this->getProduct()->getHasError()) {
            $this->setHasError(true)
                ->setMessage(Mage::helper('Magento_Sales_Helper_Data')->__('Some of the selected options are not currently available.'));
            $this->getQuote()->setHasError(true)
        ...
        if ($this->getHasConfigurationUnavailableError()) {
            $this->setHasError(true)
                ->setMessage(Mage::helper('Magento_Sales_Helper_Data')->__('Selected option(s) or their combination is not currently available.'));
            $this->getQuote()->setHasError(true)
                ->addMessage(Mage::helper('Magento_Sales_Helper_Data')->__('Some item options or their combination are not currently available.'), 'unavailable-configuration');
