    ...
    protected function _validate()
    {
        if (!$this->getQuote()->isVirtual()) {
            ...
            if ($addressValidation !== true) {
                Mage::throwException(
                    Mage::helper('Magento_Sales_Helper_Data')->__('Please check the shipping address information. %s', implode(' ', $addressValidation))
                );
            }
            $method= $address->getShippingMethod();
            $rate  = $address->getShippingRateByCode($method);
            if (!$this->getQuote()->isVirtual() && (!$method || !$rate)) {
                Mage::throwException(Mage::helper('Magento_Sales_Helper_Data')->__('Please specify a shipping method.'));
            }
        }
        ...
        if ($addressValidation !== true) {
            Mage::throwException(
                Mage::helper('Magento_Sales_Helper_Data')->__('Please check the billing address information. %s', implode(' ', $addressValidation))
            );
        }

        if (!($this->getQuote()->getPayment()->getMethod())) {
            Mage::throwException(Mage::helper('Magento_Sales_Helper_Data')->__('Please select a valid payment method.'));
        }
