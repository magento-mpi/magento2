...
class Magento_VersionsCms_Block_Adminhtml_Cms_Page_Revision_Edit extends Magento_Adminhtml_Block_Cms_Page_Edit
{
    ...
    protected function _construct()
    {
        ...
        if (!empty($objId) && $config->canCurrentUserDeleteRevision()) {
            $this->_addButton('delete_revision', array(
                ...
                'class'     => 'delete',
                'onclick'   => 'deleteConfirm(\''. Mage::helper('Magento_VersionsCms_Helper_Data')->__('Are you sure you want to delete this revision?')
                                .'\', \'' . $this->getDeleteUrl() . '\')',
            ));
        }
        ...
        if ($config->canCurrentUserPublishRevision()) {
            $this->_addButton('publish', array(
                'id'        => 'publish_button',
                'label'     => Mage::helper('Magento_VersionsCms_Helper_Data')->__('Publish'),
            ...
            if ($config->canCurrentUserSaveRevision()) {
                $this->_addButton('save_publish', array(
                    ...
                    'label'     => Mage::helper('Magento_VersionsCms_Helper_Data')->__('Save and publish.'),
        ...
        if ($config->canCurrentUserSaveRevision()) {
            $this->_updateButton('save', 'label', Mage::helper('Magento_VersionsCms_Helper_Data')->__('Save'));
            ...
            $this->_addButton('new_version', array(
                'id'        => 'new_version',
                'label'     => Mage::helper('Magento_VersionsCms_Helper_Data')->__('Save in a new version.'),
                'data_attribute'  => array(
                    'mage-init' => array(
                        'button' => array(
                            'event' => 'save',
            ...
            $this->_formScripts[] = "
                function newVersionAction(e){
                    var versionName = prompt('" . Mage::helper('Magento_VersionsCms_Helper_Data')->__('You must specify a new version name.') . "', '')
                    if (versionName == '') {
                        alert('" . Mage::helper('Magento_VersionsCms_Helper_Data')->__('Please specify a valid name.') . "');
    ...
    public function getHeaderText()
    {
        ...
        if ($revisionNumber) {
            return Mage::helper('Magento_VersionsCms_Helper_Data')->__("Edit Page '%s' Revision #%s", $title, $this->escapeHtml($revisionNumber));
        } else {
            return Mage::helper('Magento_VersionsCms_Helper_Data')->__("Edit Page '%s' New Revision", $title);
        }
    }
