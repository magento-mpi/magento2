        ...
        if (Mage::helper('Magento_VersionsCms_Helper_Hierarchy')->isMetadataEnabled() && $node && $head) {
            ...
            if (is_array($treeMetaData)) {
                ...
                if ($treeMetaData['meta_cs_enabled']) {
                    ...
                    if ($linkNode->getId()) {
                        $head->addLinkRel(Magento_VersionsCms_Model_Hierarchy_Node::META_NODE_TYPE_CHAPTER, $linkNode->getUrl());
                    }

                    $linkNode = $node->getMetaNodeByType(Magento_VersionsCms_Model_Hierarchy_Node::META_NODE_TYPE_SECTION);
                    if ($linkNode->getId()) {
                        $head->addLinkRel(Magento_VersionsCms_Model_Hierarchy_Node::META_NODE_TYPE_SECTION, $linkNode->getUrl());
                    }
                }
                ...
                if ($treeMetaData['meta_next_previous']) {
                    ...
                    if ($linkNode->getId()) {
                        $head->addLinkRel(Magento_VersionsCms_Model_Hierarchy_Node::META_NODE_TYPE_NEXT, $linkNode->getUrl());
                    }

                    $linkNode = $node->getMetaNodeByType(Magento_VersionsCms_Model_Hierarchy_Node::META_NODE_TYPE_PREVIOUS);
                    if ($linkNode->getId()) {
                        $head->addLinkRel(Magento_VersionsCms_Model_Hierarchy_Node::META_NODE_TYPE_PREVIOUS, $linkNode->getUrl());
                    }
                }
                ...
                if ($treeMetaData['meta_first_last']) {
                    ...
                    if ($linkNode->getId()) {
                        $head->addLinkRel(Magento_VersionsCms_Model_Hierarchy_Node::META_NODE_TYPE_FIRST, $linkNode->getUrl());
                    }
                }
            }
        }
