...
class Magento_MultipleWishlist_Controller_Search extends Magento_Core_Controller_Front_Action
{
    ...
    {
        parent::preDispatch();
        if (!Mage::helper('Magento_MultipleWishlist_Helper_Data')->isModuleEnabled()) {
            $this->norouteAction();
    ...
    public function indexAction()
    {
        ...
        if ($headBlock) {
            $headBlock->setTitle(Mage::helper('Magento_MultipleWishlist_Helper_Data')->__('Wish List Search'));
        }
        $this->renderLayout();
    }
    ...
    public function resultsAction()
    {
        ...
        try {
            ...
            if (empty($params) || !is_array($params) || empty($params['search'])) {
                Mage::throwException(
                    Mage::helper('Magento_MultipleWishlist_Helper_Data')->__('Please specify correct search options.')
                );
            };
            ...
            $this->_getSession()->addError(Mage::helper('Magento_MultipleWishlist_Helper_Data')->__('We could not perform the search.'));
        }
        ...
        foreach ($qtys as $itemId => $qty) {
            if ($qty && isset($selected[$itemId])) {
                try {
                    ...
                    Mage::logException($e);
                    $messages[] = Mage::helper('Magento_MultipleWishlist_Helper_Data')->__('We could not add the item to shopping cart.');
                }
            }
        }
