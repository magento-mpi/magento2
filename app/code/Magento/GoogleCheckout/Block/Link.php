...
class Magento_GoogleCheckout_Block_Link extends Magento_Core_Block_Template
{
    ...
    {
        $s = $this->_coreStoreConfig->getConfig('google/checkout/checkout_image');
        if (!$s) {
            $s = '180/46/trans';
        }
        return explode('/', $s);
    ...
    public function getImageUrl()
    {
        ...
        $url .= '?merchant_id='.$this->_coreStoreConfig->getConfig('google/checkout/merchant_id');
        $v = $this->getImageStyle();
        $url .= '&w='.$v[0].'&h='.$v[1].'&style='.$v[2];
        $url .= '&variant='.($this->getIsDisabled() ? 'disabled' : 'text');
        $url .= '&loc='.$this->_coreStoreConfig->getConfig('google/checkout/locale');
        return $url;
    }
