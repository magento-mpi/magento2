...
abstract class AbstractType
{
     ...
     *
     * @var \Magento\CatalogImportExport\Model\ImportExport\Import\Product
     */
    ...
    public function __construct(
        ...
        if ($this->isSuitable()) {
            if (!isset(
                ...
                $params[0]
            ) || !$params[0] instanceof \Magento\CatalogImportExport\Model\ImportExport\Import\Product
            ) {
    ...
    protected function _getProductAttributes($attrSetData)
    {
        if (is_array($attrSetData)) {
            return $this->_attributes[$attrSetData[\Magento\CatalogImportExport\Model\ImportExport\Import\Product::COL_ATTR_SET]];
        } else {
    ...
    public function isRowValid(array $rowData, $rowNum, $isNewProduct = true)
    {
        ...
        if (\Magento\CatalogImportExport\Model\ImportExport\Import\Product::SCOPE_NULL != $rowScope) {
            foreach ($this->_getProductAttributes($rowData) as $attrCode => $attrParams) {
                ...
                if (isset($rowData[$attrCode]) && strlen($rowData[$attrCode])) {
                    ...
                    if (\Magento\CatalogImportExport\Model\ImportExport\Import\Product::SCOPE_DEFAULT == $rowScope &&
                        ($isNewProduct ||
                        array_key_exists(
                            $attrCode,
                            $rowData
                        ...
                        $this->_entityModel->addRowError(
                            \Magento\CatalogImportExport\Model\ImportExport\Import\Product::ERROR_VALUE_IS_REQUIRED,
                            $rowNum,
                            $attrCode
                        );
                        $error = true;
