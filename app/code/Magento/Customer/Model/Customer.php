...
class Magento_Customer_Model_Customer extends Magento_Core_Model_Abstract
{
    ...
    public function authenticate($login, $password)
    {
        ...
        if ($this->getConfirmation() && $this->isConfirmationRequired()) {
            throw Mage::exception('Magento_Core',
                Mage::helper('Magento_Customer_Helper_Data')->__('This account is not confirmed.'),
                self::EXCEPTION_EMAIL_NOT_CONFIRMED
            );
        }
        if (!$this->validatePassword($password)) {
            throw Mage::exception('Magento_Core',
                Mage::helper('Magento_Customer_Helper_Data')->__('Invalid login or password.'),
    ...
    public function sendNewAccountEmail($type = 'registered', $backUrl = '', $storeId = '0')
    {
        ...
        if (!isset($types[$type])) {
            Mage::throwException(
                Mage::helper('Magento_Customer_Helper_Data')->__('Wrong transactional account email type')
            );
        }
    ...
    public function validate()
    {
        ...
        if (!Zend_Validate::is(trim($this->getFirstname()), 'NotEmpty')) {
            $errors[] = Mage::helper('Magento_Customer_Helper_Data')->__('The first name cannot be empty.');
        }

        if (!Zend_Validate::is(trim($this->getLastname()), 'NotEmpty')) {
            $errors[] = Mage::helper('Magento_Customer_Helper_Data')->__('The last name cannot be empty.');
        }

        if (!Zend_Validate::is($this->getEmail(), 'EmailAddress')) {
            $errors[] = Mage::helper('Magento_Customer_Helper_Data')
        ...
        if ($attribute->getIsRequired() && '' == trim($this->getDob())) {
            $errors[] = Mage::helper('Magento_Customer_Helper_Data')->__('The Date of Birth is required.');
        }
        $attribute = Mage::getModel('Magento_Customer_Model_Attribute')->loadByCode($entityType, 'taxvat');
        if ($attribute->getIsRequired() && '' == trim($this->getTaxvat())) {
            $errors[] = Mage::helper('Magento_Customer_Helper_Data')->__('The TAX/VAT number is required.');
        }
        $attribute = Mage::getModel('Magento_Customer_Model_Attribute')->loadByCode($entityType, 'gender');
        if ($attribute->getIsRequired() && '' == trim($this->getGender())) {
            $errors[] = Mage::helper('Magento_Customer_Helper_Data')->__('Gender is required.');
        }
    ...
    public function changeResetPasswordLinkToken($passwordLinkToken)
    {
        if (!is_string($passwordLinkToken) || empty($passwordLinkToken)) {
            throw Mage::exception('Magento_Core',
                Mage::helper('Magento_Customer_Helper_Data')->__('Invalid password reset token.'),
