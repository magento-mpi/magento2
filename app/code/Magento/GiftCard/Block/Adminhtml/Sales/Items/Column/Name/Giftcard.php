    ...
    extends Magento_Adminhtml_Block_Sales_Items_Column_Name
{
    ...
    protected function _getGiftcardOptions()
    {
        ...
        if ($type = $this->getItem()->getProductOptionByCode('giftcard_type')) {
            switch ($type) {
                case Magento_GiftCard_Model_Giftcard::TYPE_VIRTUAL:
                    $type = Mage::helper('Magento_GiftCard_Helper_Data')->__('Virtual');
                    break;
                case Magento_GiftCard_Model_Giftcard::TYPE_PHYSICAL:
                    $type = Mage::helper('Magento_GiftCard_Helper_Data')->__('Physical');
                    break;
                case Magento_GiftCard_Model_Giftcard::TYPE_COMBINED:
                    $type = Mage::helper('Magento_GiftCard_Helper_Data')->__('Combined');
                    break;
            }

            $result[] = array(
                'label'=>Mage::helper('Magento_GiftCard_Helper_Data')->__('Gift Card Type'),
                'value'=>$type,
            );
        }
        ...
        if ($value = $this->_prepareCustomOption('giftcard_lifetime')) {
            $result[] = array(
                'label'=>Mage::helper('Magento_GiftCard_Helper_Data')->__('Gift Card Lifetime'),
                'value'=>sprintf('%s days', $value),
            );
        }

        $yes = Mage::helper('Magento_GiftCard_Helper_Data')->__('Yes');
        $no = Mage::helper('Magento_GiftCard_Helper_Data')->__('No');
        if ($value = $this->_prepareCustomOption('giftcard_is_redeemable')) {
            $result[] = array(
                'label'=>Mage::helper('Magento_GiftCard_Helper_Data')->__('Gift Card Is Redeemable'),
                'value'=>($value ? $yes : $no),
            );
        }
        ...
        if (is_array($codes)) {
            foreach ($codes as &$code) {
                if ($code === null) {
                    $code = Mage::helper('Magento_GiftCard_Helper_Data')->__('We cannot create this gift card.');
                }
            }
        } else {
            $codes = array();
        ...
        for ($i = $createdCodes; $i < $totalCodes; $i++) {
            $codes[] = Mage::helper('Magento_GiftCard_Helper_Data')->__('N/A');
        }

        $result[] = array(
            'label'=>Mage::helper('Magento_GiftCard_Helper_Data')->__('Gift Card Accounts'),
