...
abstract class Magento_ImportExport_Model_Import_EntityAbstract
{
    ...
    public function getErrorMessages()
    {
        ...
        foreach ($this->_errors as $errorCode => $errorRows) {
            if (isset($this->_messageTemplates[$errorCode])) {
                $errorCode = $this->_helper('Magento_ImportExport_Helper_Data')->__($this->_messageTemplates[$errorCode]);
            }
    ...
    {
        if (!$this->_source) {
            Mage::throwException($this->_helper('Magento_ImportExport_Helper_Data')->__('Source is not set'));
        }
        return $this->_source;
    }
    ...
    public function isAttributeValid($attributeCode, array $attributeParams, array $rowData, $rowNumber)
    {
        ...
        if (!$valid) {
            $this->addRowError($this->_helper('Magento_ImportExport_Helper_Data')->__("Please correct the value for '%s'."),
            ...
            if (isset($this->_uniqueAttributes[$attributeCode][$rowData[$attributeCode]])) {
                $this->addRowError(
                    $this->_helper('Magento_ImportExport_Helper_Data')->__("Duplicate Unique Attribute for '%s'"),
                    $rowNumber, $attributeCode
                );
                return false;
            }
    ...
    public function validateData()
    {
        if (!$this->_dataValidated) {
            ...
            if ($absentColumns = array_diff($this->_permanentAttributes, $this->getSource()->getColNames())) {
                Mage::throwException(
                    $this->_helper('Magento_ImportExport_Helper_Data')->__('Cannot find required columns: %s',
            ...
            if ($emptyHeaderColumns) {
                Mage::throwException(
                    $this->_helper('Magento_ImportExport_Helper_Data')->__('Columns number: "%s" have empty headers',
            ...
            if ($invalidColumns) {
                Mage::throwException(
                    $this->_helper('Magento_ImportExport_Helper_Data')->__('Column names: "%s" are invalid',
