 ...
 */
abstract class Magento_ImportExport_Model_Import_Entity_Product_Type_Abstract
{
    /**
    ...
    protected function _getProductAttributes($attrSetData)
    {
        if (is_array($attrSetData)) {
            return $this->_attributes[$attrSetData[Magento_ImportExport_Model_Import_Entity_Product::COL_ATTR_SET]];
        } else {
    ...
    public function isRowValid(array $rowData, $rowNum, $checkRequiredAttributes = true)
    {
        ...
        if (Magento_ImportExport_Model_Import_Entity_Product::SCOPE_NULL != $rowScope) {
            foreach ($this->_getProductAttributes($rowData) as $attrCode => $attrParams) {
                ...
                if (isset($rowData[$attrCode]) && strlen($rowData[$attrCode])) {
                    ...
                    && Magento_ImportExport_Model_Import_Entity_Product::SCOPE_DEFAULT == $rowScope
                    && $attrParams['is_required']
                ) {
                    $this->_entityModel->addRowError(
                        Magento_ImportExport_Model_Import_Entity_Product::ERROR_VALUE_IS_REQUIRED, $rowNum, $attrCode
                    );
                    $error = true;
                }
            }
