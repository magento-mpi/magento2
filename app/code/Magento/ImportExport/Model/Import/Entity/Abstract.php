 ...
 */
abstract class Magento_ImportExport_Model_Import_Entity_Abstract
{
    /**
     * Database constants
     *
    ...
    {
        if (!$this->_source) {
            Mage::throwException(Mage::helper('Magento_ImportExport_Helper_Data')->__('Please specify a source.'));
        }
        return $this->_source;
    }
    ...
    public function getBehavior()
    {
        if (!isset($this->_parameters['behavior'])
            || ($this->_parameters['behavior'] != Magento_ImportExport_Model_Import::BEHAVIOR_APPEND
            && $this->_parameters['behavior'] != Magento_ImportExport_Model_Import::BEHAVIOR_REPLACE
            && $this->_parameters['behavior'] != Magento_ImportExport_Model_Import::BEHAVIOR_DELETE)) {
            return Magento_ImportExport_Model_Import::getDefaultBehavior();
        }
    ...
    {
        if (!$this->_source) {
            Mage::throwException(Mage::helper('Magento_ImportExport_Helper_Data')->__('Source is not set'));
        }
        return $this->_source;
    }
    ...
    public function isAttributeValid($attrCode, array $attrParams, array $rowData, $rowNum)
    {
        ...
        if (!$valid) {
            $this->addRowError(Mage::helper('Magento_ImportExport_Helper_Data')->__("Please correct the value for '%s'."), $rowNum, $attrCode);
        } elseif (!empty($attrParams['is_unique'])) {
            if (isset($this->_uniqueAttributes[$attrCode][$rowData[$attrCode]])) {
                $this->addRowError(Mage::helper('Magento_ImportExport_Helper_Data')->__("Duplicate Unique Attribute for '%s'"), $rowNum, $attrCode);
                return false;
            }
