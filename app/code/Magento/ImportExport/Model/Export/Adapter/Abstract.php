...
abstract class Magento_ImportExport_Model_Export_Adapter_Abstract
{
    ...
    public function __construct($destination = null)
    {
        ...
        if (!is_string($destination)) {
            throw new Magento_Core_Exception(__('Destination file path must be a string'));
        }

        $pathinfo = pathinfo($destination);
        if (empty($pathinfo['dirname']) || !is_writable($pathinfo['dirname'])) {
            throw new Magento_Core_Exception(__('Destination directory is not writable'));
        }
        if (is_file($destination) && !is_writable($destination)) {
            throw new Magento_Core_Exception(__('Destination file is not writable'));
        }
