...
class Collection extends \Magento\Catalog\Model\Resource\Collection\AbstractCollection
{
    ...
    protected function _initSelect()
    {
        if ($this->isEnabledFlat()) {
            $this->getSelect()->from(
                array(self::MAIN_TABLE_ALIAS => $this->getEntity()->getFlatTableName()),
                null
            )->columns(
                array('status' => new \Zend_Db_Expr(ProductStatus::STATUS_ENABLED))
            );
    ...
    public function getAllAttributeValues($attribute)
    {
        ...
        $select->reset()->from(
            ...
            array('entity_id', 'store_id', 'value')
        )->where(
            'attribute_id = ?',
            (int)$attribute->getId()
        );
    ...
    public function getProductCountSelect()
    {
        if ($this->_productCountSelect === null) {
            ...
            $this->_productCountSelect->reset(
                ...
                array('count_table' => $this->getTable('catalog_category_product_index')),
    ...
    {
        /**
         * @TODO: UrlRewrite joinUrlRewrite and addUrlRewrite and _addUrlRewrite
         * @TODO: UrlRewrite is it need to use is_autogenerated flag?
         */
        $this->joinTable(
            'url_rewrite',
            'entity_id = entity_id',
            ['request_path'],
            '{{table}}.entity_type = \'' . ProductUrlRewriteGenerator::ENTITY_TYPE_PRODUCT . '\'',
            'left'
        );
        return $this;
    }
    ...
    public function _addUrlRewrite()
    {
        ...
        $select = $this->getConnection()
            ->select()
            ->from($this->getTable('url_rewrite'), ['entity_id', 'request_path'])
            ->where('store_id = ?', $this->_storeManager->getStore()->getId())
            ->where('is_autogenerated = 1')
            ->where('entity_type = ?', ProductUrlRewriteGenerator::ENTITY_TYPE_PRODUCT)
    ...
    protected function _applyZeroStoreProductLimitations()
    {
        ...
        $this->_joinFields['position'] = array('table' => 'cat_pro', 'field' => 'position');
