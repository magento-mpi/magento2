...
class Flat extends \Magento\Index\Model\Resource\AbstractResource
{
    ...
    protected function _loadNodes($parentNode = null, $recursionLevel = 0, $storeId = 0, $skipMenuFilter = false)
    {
        ...
        $select = $_conn->select()->from(
            ...
            'url_rewrite.entity_id = main_table.entity_id AND url_rewrite.is_autogenerated = 1'
                ...
                . $_conn->quoteInto(' AND url_rewrite.entity_type = ?', CategoryUrlRewriteGenerator::ENTITY_TYPE_CATEGORY),
    ...
    public function getCategories($parent, $recursionLevel = 0, $sorted = false, $asCollection = false, $toLoad = true)
    {
        if ($asCollection) {
            $select = $this->_getReadAdapter()->select()->from(
                array('mt' => $this->getMainStoreTable($this->getStoreId())),
                array('path')
            )->where(
                'mt.entity_id = ?',
                $parent
    ...
    public function getParentCategories($category, $isActive = true)
    {
        ...
        $select = $read->select()->from(
            ...
            'url_rewrite.entity_id = main_table.entity_id AND url_rewrite.is_autogenerated = 1'
                ...
                . $read->quoteInto(' AND url_rewrite.entity_type = ?', CategoryUrlRewriteGenerator::ENTITY_TYPE_CATEGORY),
            ['request_path' => 'url_rewrite.request_path']
        )->where(
    ...
    {
        $select = $this->_getReadAdapter()->select()->from(
            array('e' => $this->getMainStoreTable($storeId)),
            'entity_id'
        )->where(
            'is_anchor = ?',
            1
