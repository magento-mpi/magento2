 ...
 */
abstract class Magento_Catalog_Model_Product_Type_Abstract
{
    /**
     ...
     * @param  Magento_Catalog_Model_Product $product
     * @return Magento_Catalog_Model_Resource_Eav_Attribute|null
     */
    ...
    {
        $salable = $product->getStatus() == Magento_Catalog_Model_Product_Status::STATUS_ENABLED;
                ...
                switch ($operation) {
                    case 'receive_uploaded_file':
                        ...
                        try {
                            ...
                            Mage::throwException(
                                $this->_helper('Magento_Catalog_Helper_Data')
                        ...
                        if (empty($src) || empty($dst) || !$uploader->receive($src)) {
                            ...
                            Mage::throwException($this->_helper('Magento_Catalog_Helper_Data')->__("The file upload failed."));
                        }
    ...
    {
        return $this->_helper('Magento_Catalog_Helper_Data')->__('Please specify the product\'s required option(s).');
    }
            ...
            foreach ($product->getOptions() as $option) {
                if ($option->getIsRequire()) {
                    ...
                    if (!$customOption || strlen($customOption->getValue()) == 0) {
                        ...
                        Mage::throwException(
                            $this->_helper('Magento_Catalog_Helper_Data')->__('The product has required options.')
                        );
                    }
                }
            }
     ...
     *
     * @param Magento_Catalog_Model_Product $product Product with Custom Options
    ...
    public function checkProductConfiguration($product, $buyRequest)
    {
        ...
        try {
            ...
            Mage::logException($e);
            $errors[] = $this->_helper('Magento_Catalog_Helper_Data')->__('Something went wrong while processing the request.');
        }
