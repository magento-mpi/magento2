...
class Magento_CatalogPermissions_Model_Observer
{
    ...
    public function applyCategoryPermission(Magento_Event_Observer $observer)
    {
        ...
        if ($observer->getEvent()->getCategory()->getIsHidden()) {
            ...
            Mage::throwException(
                Mage::helper('Magento_CatalogPermissions_Helper_Data')->__('You may need more permissions to access this category.')
            );
        }
        return $this;
    }
            ...
            if ($quoteItem->getDisableAddToCart() && !$quoteItem->isDeleted()) {
                ...
                if ($parentItem) {
                    $quote->setHasError(true)
                            ->addMessage(
                                Mage::helper('Magento_CatalogPermissions_Helper_Data')->__('You cannot add "%s" to the cart.', $parentItem->getName())
                            );
                } else {
                     $quote->setHasError(true)
                            ->addMessage(
                                Mage::helper('Magento_CatalogPermissions_Helper_Data')->__('You cannot add "%s" to the cart.', $quoteItem->getName())
                            );
                }
            }
        }
    ...
    public function applyProductPermission(Magento_Event_Observer $observer)
    {
        ...
        if ($observer->getEvent()->getProduct()->getIsHidden()) {
            ...
            Mage::throwException(
                Mage::helper('Magento_CatalogPermissions_Helper_Data')->__('You may need more permissions to access this product.')
            );
        }
