    ...
    implements Magento_Catalog_Controller_Product_View_Interface
{
    ...
    protected function _getWishlist($wishlistId = null)
    {
        ...
        try {
            ...
            if (!$wishlist->getId() || $wishlist->getCustomerId() != $customerId) {
                $wishlist = null;
                Mage::throwException(
                    Mage::helper('Magento_Wishlist_Helper_Data')->__("The requested wish list doesn\'t exist.")
                );
            }
            ...
            Mage::getSingleton('Magento_Wishlist_Model_Session')->addException($e,
                Mage::helper('Magento_Wishlist_Helper_Data')->__('Wish List could not be created.')
            );
            return false;
        }
    ...
    public function configureAction()
    {
        ...
        try {
            ...
            Mage::getSingleton('Magento_Customer_Model_Session')->addError($this->__('We can\'t configure the product.'));
            Mage::logException($e);
            $this->_redirect('*');
            return;
        }
        ...
        if($post && isset($post['description']) && is_array($post['description'])) {
            ...
            if ($updatedItems) {
                try {
                    ...
                    Mage::getSingleton('Magento_Customer_Model_Session')->addError($this->__('Can\'t update wish list'));
                }
            }
    ...
    public function fromcartAction()
    {
        ...
        try{
            ...
            if (!$item) {
                Mage::throwException(
                    Mage::helper('Magento_Wishlist_Helper_Data')->__("The requested cart item doesn\'t exist.")
                );
            }
            ...
            $session->addSuccess(
                Mage::helper('Magento_Wishlist_Helper_Data')->__("%s has been moved to wish list %s", $productName, $wishlistName)
            );
            $wishlist->save();
        } catch (Magento_Core_Exception $e) {
            $session->addError($e->getMessage());
