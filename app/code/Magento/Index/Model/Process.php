...
class Magento_Index_Model_Process extends Magento_Core_Model_Abstract
{
    ...
    {
        if ($this->isLocked()) {
            Mage::throwException(Mage::helper('Magento_Index_Helper_Data')->__('%s Index process is not working now. Please try running this process later.', $this->getIndexer()->getName()));
        }
    ...
    public function getIndexer()
    {
        if ($this->_indexer === null) {
            ...
            if (!$code) {
                Mage::throwException(Mage::helper('Magento_Index_Helper_Data')->__('Indexer code is not defined.'));
            }
            $xmlPath = self::XML_PATH_INDEXER_DATA . '/' . $code;
            $config = Mage::getConfig()->getNode($xmlPath);
            if (!$config || empty($config->model)) {
                Mage::throwException(Mage::helper('Magento_Index_Helper_Data')->__('Indexer model is not defined.'));
            }
            $model = Mage::getModel((string)$config->model);
            if ($model instanceof Magento_Index_Model_Indexer_Abstract) {
                $this->_indexer = $model;
            } else {
                Mage::throwException(Mage::helper('Magento_Index_Helper_Data')->__('Indexer model should extend Magento_Index_Model_Indexer_Abstract.'));
            }
        }
    ...
    {
        return array(
            self::MODE_REAL_TIME => Mage::helper('Magento_Index_Helper_Data')->__('Update on Save'),
            self::MODE_MANUAL => Mage::helper('Magento_Index_Helper_Data')->__('Manual Update')
        );
    }
    ...
    {
        return array(
            self::STATUS_PENDING            => Mage::helper('Magento_Index_Helper_Data')->__('Ready'),
            self::STATUS_RUNNING            => Mage::helper('Magento_Index_Helper_Data')->__('Processing'),
            self::STATUS_REQUIRE_REINDEX    => Mage::helper('Magento_Index_Helper_Data')->__('Reindex Required'),
        );
    }
    ...
    {
        return array(
            0 => Mage::helper('Magento_Index_Helper_Data')->__('No'),
            1 => Mage::helper('Magento_Index_Helper_Data')->__('Yes'),
        );
    }
