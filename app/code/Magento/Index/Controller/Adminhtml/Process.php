...
class Magento_Index_Controller_Adminhtml_Process extends Magento_Adminhtml_Controller_Action
{
    ...
    public function editAction()
    {
        ...
        if ($process) {
            ...
            $this->_getSession()->addError(
                Mage::helper('Magento_Index_Helper_Data')->__('Cannot initialize the indexer process.')
            );
            $this->_redirect('*/*/list');
        }
    }
        ...
        if ($process) {
            ...
            try {
                ...
                $this->_getSession()->addSuccess(
                    Mage::helper('Magento_Index_Helper_Data')->__('The index has been saved.')
                );
            } catch (Magento_Core_Exception $e) {
                $this->_getSession()->addError($e->getMessage());
            } catch (Exception $e) {
                $this->_getSession()->addException($e,
                     Mage::helper('Magento_Index_Helper_Data')->__('There was a problem with saving process.')
                );
            }
            $this->_redirect('*/*/list');
        } else {
    ...
    public function reindexProcessAction()
    {
        ...
        if ($process) {
            try {
                ...
                $this->_getSession()->addSuccess(
                    Mage::helper('Magento_Index_Helper_Data')->__('%s index was rebuilt.', $process->getIndexer()->getName())
                );
            } catch (Magento_Core_Exception $e) {
                $this->_getSession()->addError($e->getMessage());
            } catch (Exception $e) {
                $this->_getSession()->addException($e,
                     Mage::helper('Magento_Index_Helper_Data')->__('There was a problem with reindexing process.')
                );
            }
        } else {
            $this->_getSession()->addError(
    ...
    public function massReindexAction()
    {
        ...
        if (empty($processIds) || !is_array($processIds)) {
            $this->_getSession()->addError(Mage::helper('Magento_Index_Helper_Data')->__('Please select Indexes'));
        } else {
            try {
                $counter = 0;
                ...
                $this->_getSession()->addSuccess(
                    Mage::helper('Magento_Index_Helper_Data')->__('Total of %d index(es) have reindexed data.', $counter)
                );
            } catch (Magento_Core_Exception $e) {
                $this->_getSession()->addError($e->getMessage());
            } catch (Exception $e) {
                $this->_getSession()->addException($e, Mage::helper('Magento_Index_Helper_Data')->__('Cannot initialize the indexer process.'));
            }
        }
    ...
    public function massChangeModeAction()
    {
        $processIds = $this->getRequest()->getParam('process');
        if (empty($processIds) || !is_array($processIds)) {
            $this->_getSession()->addError(Mage::helper('Magento_Index_Helper_Data')->__('Please select Index(es)'));
        } else {
            try {
                $counter = 0;
                ...
                $this->_getSession()->addSuccess(
                    Mage::helper('Magento_Index_Helper_Data')->__('Total of %d index(es) have changed index mode.', $counter)
                );
            } catch (Magento_Core_Exception $e) {
                $this->_getSession()->addError($e->getMessage());
            } catch (Exception $e) {
