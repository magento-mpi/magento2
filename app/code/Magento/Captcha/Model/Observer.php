...
class Magento_Captcha_Model_Observer
{
    ...
    public function checkForgotpassword($observer)
    {
        ...
        if ($captchaModel->isRequired()) {
            ...
            if (!$captchaModel->isCorrect($this->_getCaptchaString($controller->getRequest(), $formId))) {
                Mage::getSingleton('Magento_Customer_Model_Session')->addError(Mage::helper('Magento_Captcha_Helper_Data')->__('Incorrect CAPTCHA'));
    ...
    public function checkGuestCheckout($observer)
    {
        ...
        if ($checkoutMethod == Magento_Checkout_Model_Type_Onepage::METHOD_GUEST) {
            if ($captchaModel->isRequired()) {
                ...
                if (!$captchaModel->isCorrect($this->_getCaptchaString($controller->getRequest(), $formId))) {
                    $controller->setFlag('', Magento_Core_Controller_Varien_Action::FLAG_NO_DISPATCH, true);
                    $result = array('error' => 1, 'message' => Mage::helper('Magento_Captcha_Helper_Data')->__('Incorrect CAPTCHA'));
        ...
        if ($captchaModel->isRequired($login)) {
            if (!$captchaModel->isCorrect($this->_getCaptchaString(Mage::app()->getRequest(), $formId))) {
                ...
                throw new Magento_Backend_Model_Auth_Plugin_Exception(
                    Mage::helper('Magento_Captcha_Helper_Data')->__('Incorrect CAPTCHA.')
                );
            }
        }
        $captchaModel->logAttempt($login);
        ...
        if (!empty($email) && !empty($params)){
            if ($captchaModel->isRequired()){
                if (!$captchaModel->isCorrect($this->_getCaptchaString($controller->getRequest(), $formId))) {
                    ...
                    $controller->setFlag('', Magento_Core_Controller_Varien_Action::FLAG_NO_DISPATCH, true);
                    $this->_getBackendSession()->addError(Mage::helper('Magento_Captcha_Helper_Data')->__('Incorrect CAPTCHA'));
    ...
    public function deleteExpiredImages()
    {
        foreach (Mage::app()->getWebsites(true) as $website) {
            $expire = time() - Mage::helper('Magento_Captcha_Helper_Data')
                ->getConfigNode('timeout', $website->getDefaultStore()) * 60;
            $imageDirectory = Mage::helper('Magento_Captcha_Helper_Data')->getImgDir($website);
