    ...
    protected function _prepareLayout()
    {
        if ($salesRule = Mage::registry('current_promo_quote_rule')) {
            ...
            if ($count = $resource->getAssignedRulesCount($salesRule->getId())) {
                $confirm = Mage::helper('Magento_Reminder_Helper_Data')->__('This rule is assigned to %s automated reminder rule(s). Deleting this rule will automatically unassign it.', $count);
