    ...
    extends Magento_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        $fieldset = $form->addFieldset('email_fieldset', array(
            'legend' => Mage::helper('Magento_Reminder_Helper_Data')->__('Email Templates'),
            'class'  => 'tree-store-scope',
            'comment' => Mage::helper('Magento_Reminder_Helper_Data')->__('Only customers who registered on a store view will receive emails related to that store view.'),
        ));
        ...
        $fieldset = $form->addFieldset('default_label_fieldset', array(
            'legend' => Mage::helper('Magento_Reminder_Helper_Data')->__('Default Titles and Description'),
            'comment' => Mage::helper('Magento_Reminder_Helper_Data')->__('You can find and edit rule label and descriptions, per store view, in email templates.'),
        ));

        $fieldset->addField('default_label', 'text', array(
            'name'      => 'default_label',
            'required'  => false,
            'label'     => Mage::helper('Magento_Reminder_Helper_Data')->__('Rule Title for All Store Views')
        ));

        $fieldset->addField('default_description', 'textarea', array(
            'name'     => 'default_description',
            'required' => false,
            'label'    => Mage::helper('Magento_Reminder_Helper_Data')->__('Rule Description for All Store Views'),
            'style'    => 'height: 50px;'
        ));

        $fieldset = $form->addFieldset('labels_fieldset', array(
            'legend' => Mage::helper('Magento_Reminder_Helper_Data')->__('Titles and Descriptions Per Store View'),
            'comment' => Mage::helper('Magento_Reminder_Helper_Data')->__('Overrides default titles and descriptions. Note that if email an template is not specified for this store view, the respective variable values will be deleted.'),
            'class'  => 'tree-store-scope'
        ));
    ...
    public function getTemplatesOptionsArray()
    {
        ...
        array_unshift($options, array('value'=>'',
            'label' => Mage::helper('Magento_Reminder_Helper_Data')->__('-- Not Selected --'))
        );
        return $options;
    }
}
