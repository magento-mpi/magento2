...
class Magento_Reminder_Block_Adminhtml_Reminder_Edit extends Magento_Adminhtml_Block_Widget_Form_Container
{
    ...
    protected function _construct()
    {
        ...
        if ($rule && $rule->getId()) {
            $confirm = Mage::helper('Magento_Reminder_Helper_Data')->__('Are you sure you want to match this rule now?');
            if ($limit = Mage::helper('Magento_Reminder_Helper_Data')->getOneRunLimit()) {
                $confirm .= ' ' . Mage::helper('Magento_Reminder_Helper_Data')->__('No more than %s customers may receive the reminder email after this action.', $limit);
            }
            $this->_addButton('run_now', array(
                'label'   => Mage::helper('Magento_Reminder_Helper_Data')->__('Run Now'),
        ...
        $this->_addButton('save_and_continue_edit', array(
            'class'   => 'save',
            'label'   => Mage::helper('Magento_Reminder_Helper_Data')->__('Save and Continue Edit'),
    ...
    public function getHeaderText()
    {
        $rule = Mage::registry('current_reminder_rule');
        if ($rule->getRuleId()) {
            return Mage::helper('Magento_Reminder_Helper_Data')->__("Edit Rule '%s'", $this->escapeHtml($rule->getName()));
        }
        else {
            return Mage::helper('Magento_Reminder_Helper_Data')->__('New Rule');
        }
    }
