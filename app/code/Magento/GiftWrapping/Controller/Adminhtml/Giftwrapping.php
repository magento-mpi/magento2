...
class Magento_GiftWrapping_Controller_Adminhtml_Giftwrapping extends Magento_Adminhtml_Controller_Action
{
    ...
    protected function _initAction()
    {
        ...
        $this->_title(Mage::helper('Magento_GiftWrapping_Helper_Data')->__('Gift Wrapping'));
        return $this;
    }
    ...
    protected function _initModel($requestParam = 'id')
    {
        ...
        $model = Mage::getModel('Magento_GiftWrapping_Model_Wrapping');
        ...
        if ($wrappingId) {
            ...
            if (!$model->getId()) {
                Mage::throwException(Mage::helper('Magento_GiftWrapping_Helper_Data')->__('Please request the correct gift wrapping.'));
            }
        }
    ...
    public function newAction()
    {
        ...
        $this->_initAction();
        $this->_title(Mage::helper('Magento_GiftWrapping_Helper_Data')->__('New Gift Wrapping'));
        $this->renderLayout();
    }
    ...
    public function editAction()
    {
        ...
        $this->_title(Mage::helper('Magento_GiftWrapping_Helper_Data')->__('%s', $model->getDesign()));
        $this->renderLayout();
    }
            ...
            try {
                ...
                if ($data->getData('image_name/delete')) {
                    ...
                    try {
                        ...
                        Mage::throwException(Mage::helper('Magento_GiftWrapping_Helper_Data')->__('You have not uploaded the image.'));
                    }
                }

                $model->save();
                Mage::getSingleton('Magento_Adminhtml_Model_Session')->addSuccess(Mage::helper('Magento_GiftWrapping_Helper_Data')->__('You saved the gift wrapping.'));
                ...
                Mage::getSingleton('Magento_Adminhtml_Model_Session')->addError(Mage::helper('Magento_GiftWrapping_Helper_Data')->__("We couldn't save the gift wrapping."));
                Mage::logException($e);
            }
        }
        $this->_redirect('*/*/');
        ...
        if ($wrappingRawData) {
            try {
                ...
                try {
                    ...
                    Mage::throwException(Mage::helper('Magento_GiftWrapping_Helper_Data')->__('You have not updated the image.'));
                }
            } catch (Magento_Core_Exception $e) {
                Mage::getSingleton('Magento_Adminhtml_Model_Session')->addError($e->getMessage());
    ...
    public function changeStatusAction()
    {
        ...
        try {
            $wrappingCollection = Mage::getModel('Magento_GiftWrapping_Model_Wrapping')->getCollection();
            ...
            $this->_getSession()->addSuccess(
                Mage::helper('Magento_GiftWrapping_Helper_Data')->__('You updated a total of %d records.', count($wrappingIds))
            );
        } catch (Magento_Core_Exception $e) {
            $this->_getSession()->addError($e->getMessage());
        } catch (Exception $e) {
            $this->_getSession()->addException($e, Mage::helper('Magento_GiftWrapping_Helper_Data')->__('Something went wrong while updating the wrapping(s) status.'));
        }
    ...
    public function massDeleteAction()
    {
        ...
        if (!is_array($wrappingIds)) {
            $this->_getSession()->addError(Mage::helper('Magento_GiftWrapping_Helper_Data')->__('Please select items.'));
        } else {
            try {
                $wrappingCollection = Mage::getModel('Magento_GiftWrapping_Model_Wrapping')->getCollection();
                $wrappingCollection->addFieldToFilter('wrapping_id', array('in' => $wrappingIds));
                ...
                $this->_getSession()->addSuccess(
                    Mage::helper('Magento_GiftWrapping_Helper_Data')->__('You deleted a total of %d records.', count($wrappingIds))
                );
            } catch (Exception $e) {
                $this->_getSession()->addError($e->getMessage());
            }
    ...
    public function deleteAction()
    {
        ...
        if ($wrapping->getId()) {
            try {
                $wrapping->delete();
                $this->_getSession()->addSuccess(Mage::helper('Magento_GiftWrapping_Helper_Data')->__('You deleted the gift wrapping.'));
            } catch (Exception $e) {
                $this->_getSession()->addError($e->getMessage());
