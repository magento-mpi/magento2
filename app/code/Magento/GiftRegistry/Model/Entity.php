...
class Magento_GiftRegistry_Model_Entity extends Magento_Core_Model_Abstract
{
    ...
    public function sendShareRegistryEmails()
    {
        ...
        if (empty($senderName) || empty($senderMessage) || empty($senderEmail)) {
            return $result->setErrorMessage(
                $this->_helper('Magento_GiftRegistry_Helper_Data')->__('You need to enter sender data.')
            );
        }

        if (!Zend_Validate::is($senderEmail, 'EmailAddress')) {
            return $result->setErrorMessage(
                $this->_helper('Magento_GiftRegistry_Helper_Data')->__('Please enter a valid sender email address.')
            );
        }
        ...
        foreach ($this->getRecipients() as $recipient) {
            ...
            if (!Zend_Validate::is($recipientEmail, 'EmailAddress')) {
                return $result->setErrorMessage(
                    $this->_helper('Magento_GiftRegistry_Helper_Data')->__('Please enter a valid recipient email address.')
                );
            }
            ...
            if (empty($recipient['name'])) {
                return $result->setErrorMessage(
                    $this->_helper('Magento_GiftRegistry_Helper_Data')->__('Please enter a recipient name.')
                );
            }
            $emails[] = $recipient;
        }
        ...
        if (count($emails)) {
            ...
            if ($count > 0) {
                $result->setIsSuccess(true)
                    ->setSuccessMessage(
                        $this->_helper('Magento_GiftRegistry_Helper_Data')->__('You shared the gift registry for %d emails.', $count)
                    );
            } else {
                $result->setErrorMessage(
                    $this->_helper('Magento_GiftRegistry_Helper_Data')->__("We couldn't share the registry.")
                );
            }
        }
    ...
    public function getOptionsIsPublic()
    {
        if (!isset($this->_optionsIsPublic)) {
            $this->_optionsIsPublic = array(
                '0' => Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Private'),
                '1' => Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Public'));
        }
    ...
    public function getOptionsStatus()
    {
        if (!isset($this->_optionsStatus)) {
            $this->_optionsStatus = array(
                '0' => Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Inactive'),
                '1' => Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Active'));
        }
    ...
    public function validate()
    {
        ...
        if (!Zend_Validate::is($this->getTitle(), 'NotEmpty')) {
            $errors[] = Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Please enter the title.');
        }

        if (!Zend_Validate::is($this->getMessage(), 'NotEmpty')) {
            $errors[] = Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Please enter the message.');
        }

        if (!Zend_Validate::is($this->getIsPublic(), 'NotEmpty')) {
            $errors[] = Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Please enter correct the Privacy setting.');
    ...
    public function getProduct($productId)
    {
        ...
        if (is_null($product)) {
            if (!$productId) {
                Mage::throwException(Mage::helper('Magento_GiftRegistry_Helper_Data')->__('We cannot specify the product.'));
            }
            ...
            if ($model->getId() && $model->getEntityId() == $this->getId()) {
                if (!isset($item['delete'])) {
                    ...
                    if ($stockItem->getIsQtyDecimal() == 0 && $item['qty'] != (int)$item['qty']) {
                        throw new Magento_Exception(Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Please correct the  gift registry item quantity.'));
                    }
                }
            } else {
                Mage::throwException(
                    Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Please correct the gift registry item ID.')
                );
            }
        }
    }
