...
class Giftregistry extends \Magento\Backend\App\Action
{
    ...
    public function saveAction()
    {
        ...
        if ($data) {
            ...
            try {
                ...
                $this->_objectManager->get('Magento\Backend\Model\Session')->addError(__("We couldn't save this gift registry type."));
    ...
    public function deleteAction()
    {
        try {
            $model = $this->_initType();
            $model->delete();
            $this->_objectManager->get('Magento\Backend\Model\Session')->addSuccess(__('You deleted the gift registry type.'));
        } catch (\Magento\Core\Exception $e) {
            $this->_objectManager->get('Magento\Backend\Model\Session')->addError($e->getMessage());
            $this->_redirect('adminhtml/*/edit', array('id' => $model->getId()));
            return;
        } catch (\Exception $e) {
            $this->_objectManager->get('Magento\Backend\Model\Session')->addError(__("We couldn't delete this gift registry type."));
            $this->_objectManager->get('Magento\Logger')->logException($e);
        }
        $this->_redirect('adminhtml/*/');
    }
