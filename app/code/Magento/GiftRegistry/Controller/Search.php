...
class Magento_GiftRegistry_Controller_Search extends Magento_Core_Controller_Front_Action
{
    ...
    protected function _validateSearchParams($params)
    {
        if (empty($params) || !is_array($params) || empty($params['search'])) {
            $this->_getSession()->addNotice(
                Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Please enter correct search options.')
            );
            return false;
        }
        ...
        switch ($params['search']) {
            case 'type':
                if (empty($params['firstname']) || strlen($params['firstname']) < 2) {
                    $this->_getSession()->addNotice(
                        Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Please enter at least 2 letters of the first name.')
                    );
                    return false;
                }
                if (empty($params['lastname']) || strlen($params['lastname']) < 2) {
                    $this->_getSession()->addNotice(
                        Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Please enter at least 2 letters of the last name.')
                    );
                    return false;
                }
                break;
            ...
            case 'email':
                if (empty($params['email']) || !Zend_Validate::is($params['email'], 'EmailAddress')) {
                    $this->_getSession()->addNotice(
                        Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Please enter a valid email address.')
                    );
                    return false;
                }
                break;
            ...
            case 'id':
                if (empty($params['id'])) {
                    $this->_getSession()->addNotice(
                        Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Please enter a gift registry ID.')
                    );
                    return false;
                }
                break;
    ...
    public function indexAction()
    {
        ...
        if ($headBlock) {
            $headBlock->setTitle(Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Gift Registry Search'));
        }
        $this->renderLayout();
    }
