 ...
 */
class Magento_GiftRegistry_Controller_Index extends Magento_Core_Controller_Front_Action
{
    /**
     * Only logged in users can use this functionality,
     * this function checks if user is logged in before all other actions
     *
     * @return Magento_GiftRegistry_Controller_Index
     */
    ...
    public function indexAction()
    {
        ...
        if ($headBlock) {
            $headBlock->setTitle(Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Gift Registry'));
        }
        $this->renderLayout();
    }
            ...
            if ($entity && $entity->getId()) {
                ...
                if ($count > 0) {
                    Mage::getSingleton('Magento_Checkout_Model_Session')->addSuccess(
                        Mage::helper('Magento_GiftRegistry_Helper_Data')->__('%d item(s) have been added to the gift registry.', $count)
                    );
                } else {
                    Mage::getSingleton('Magento_Checkout_Model_Session')->addNotice(
                        Mage::helper('Magento_GiftRegistry_Helper_Data')->__('We have nothing to add to this gift registry.')
                    );
                }
                if (!empty($skippedItems)) {
                    Mage::getSingleton('Magento_Checkout_Model_Session')->addNotice(
                        Mage::helper('Magento_GiftRegistry_Helper_Data')->__("You can't add virtual products, digital products or gift cards to gift registries.")
                    );
                }
            }
        } catch (Magento_Core_Exception $e) {
            if ($e->getCode() == Magento_GiftRegistry_Model_Entity::EXCEPTION_CODE_HAS_REQUIRED_OPTIONS) {
        ...
        if ($itemId) {
            try {
                ...
                $this->_getSession()->addSuccess(
                    Mage::helper('Magento_GiftRegistry_Helper_Data')->__('The wish list item has been added to this gift registry.')
                );
                ...
                if ($e->getCode() == Magento_GiftRegistry_Model_Entity::EXCEPTION_CODE_HAS_REQUIRED_OPTIONS) {
                    ...
                    $query['options'] = Magento_GiftRegistry_Block_Product_View::FLAG;
    ...
    public function deleteAction()
    {
        try {
            ...
            if ($entity->getId()) {
                $entity->delete();
                $this->_getSession()->addSuccess(
                    Mage::helper('Magento_GiftRegistry_Helper_Data')->__('You deleted this gift registry.')
                );
            }
        } catch (Magento_Core_Exception $e) {
            $this->_getSession()->addError($e->getMessage());
        } catch (Exception $e) {
            $message = Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Something went wrong while deleting the gift registry.');
    ...
    public function shareAction()
    {
        try {
            ...
            $headBlock = $this->getLayout()->getBlock('head');
            if ($headBlock) {
                $headBlock->setTitle(Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Share Gift Registry'));
            }
            ...
            $message = Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Something went wrong while sharing the gift registry.');
            $this->_getSession()->addException($e, $message);
        }
        $this->_redirect('*/*/');
    }
    ...
    public function itemsAction()
    {
        try {
            ...
            $headBlock = $this->getLayout()->getBlock('head');
            if ($headBlock) {
                $headBlock->setTitle(Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Gift Registry Items'));
            }
            $this->renderLayout();
            return;
        } catch (Magento_Core_Exception $e) {
    ...
    public function updateItemsAction()
    {
        ...
        try {
            ...
            if ($entity->getId()) {
                ...
                $this->_getSession()->addSuccess(
                    Mage::helper('Magento_GiftRegistry_Helper_Data')->__('You updated the gift registry items.')
                );
            }
        } catch (Magento_Core_Exception $e) {
            $this->_getSession()->addError($e->getMessage());
    ...
    public function sendAction()
    {
        ...
        try {
            /** @var $entity Magento_GiftRegistry_Model_Entity */
            ...
            $message = Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Something went wrong while sending email(s).');
            $this->_getSession()->addException($e, $message);
        }
        $this->_redirect('*/*/');
    }
    ...
    public function addSelectAction()
    {
        ...
        $headBlock = $this->getLayout()->getBlock('head');
        if ($headBlock) {
            $headBlock->setTitle(Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Create Gift Registry'));
        }
        $this->renderLayout();
    }
        ...
        try {
            if (!$typeId) {
                if (!$entityId) {
                    ...
                    /* @var $model Magento_GiftRegistry_Model_Entity */
            ...
            if ($typeId && !$entityId) {
                // creating new entity
                /* @var $model Magento_GiftRegistry_Model_Entity */
                $model = Mage::getSingleton('Magento_GiftRegistry_Model_Entity');
                if ($model->setTypeById($typeId) === false) {
                    Mage::throwException(Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Please correct the gift registry.'));
                }
            }
            ...
            if ($model->getId()) {
                $pageTitle = Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Edit Gift Registry');
            } else {
                $pageTitle = Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Create Gift Registry');
            }
            $headBlock = $this->getLayout()->getBlock('head');
            if ($headBlock) {
        ...
        if ($this->getRequest()->isPost() && ($data = $this->getRequest()->getPost())) {
            ...
            try {
                ...
                if ($isAddAction) {
                    ...
                    if ($model->setTypeById($typeId) === false) {
                        Mage::throwException(Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Incorrect Type'));
                    }
                }

                $data = Mage::helper('Magento_GiftRegistry_Helper_Data')->filterDatesByFormat(
                    $data,
                ...
                if (is_array($registrantsPost)) {
                    foreach  ($registrantsPost as $index => $registrant) {
                        if (is_array($registrant)) {
                            /* @var $person Magento_GiftRegistry_Model_Person */
                            $person = Mage::getModel('Magento_GiftRegistry_Model_Person');
                            $idField = $person->getIdFieldName();
                            if (!empty($registrant[$idField])) {
                                $person->load($registrant[$idField]);
                                if (!$person->getId()) {
                                    Mage::throwException(Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Please correct the recipient data.'));
                                }
                            } else {
                ...
                if (!$addressTypeOrId || $addressTypeOrId == Magento_GiftRegistry_Helper_Data::ADDRESS_NEW) {
                    ...
                    if (!empty($data['address'])) {
                        ...
                        Mage::throwException(Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Address is empty.'));
                    }
                    if ($errors !== true) {
                        foreach ($errors as $err) {
                            $this->_getSession()->addError($err);
                ...
                } else if ($addressTypeOrId != Magento_GiftRegistry_Helper_Data::ADDRESS_NONE) {
                    // using one of existing Customer adressess
                    $addressId = $addressTypeOrId;
                    if (!$addressId) {
                        Mage::throwException(Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Please select an address.'));
                    }
                    ...
                    if (!$address) {
                        Mage::throwException(Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Please correct the address.'));
                    }
                    $model->importAddress($address);
                }
                ...
                if (!$isError) {
                    ...
                    if (!$isAddAction) {
                        Mage::getModel('Magento_GiftRegistry_Model_Person')
                            ->getResource()
                            ->deleteOrphan($entityId, $personLeft);
                    }
                    $this->_getSession()->addSuccess(
                        Mage::helper('Magento_GiftRegistry_Helper_Data')->__('You saved this gift registry.')
                    );
                    if ($isAddAction) {
                ...
                $this->_getSession()->addError(
                    Mage::helper('Magento_GiftRegistry_Helper_Data')->__("We couldn't save this gift registry.")
                );
                Mage::logException($e);
                $isError = true;
            }
    ...
    protected function _initEntity($requestParam = 'id')
    {
        ...
        if ($entityId) {
            ...
            if (!$entity->getId() || $entity->getCustomerId() != $customerId) {
                Mage::throwException(Mage::helper('Magento_GiftRegistry_Helper_Data')->__('Please correct the gift registry ID.'));
            }
        }
        return $entity;
    }
