...
class Magento_Oauth_Controller_Adminhtml_Oauth_Consumer extends Magento_Adminhtml_Controller_Action
{
    ...
    public function editAction()
    {
        ...
        if (!$id) {
            $this->_getSession()->addError(Mage::helper('Magento_Oauth_Helper_Data')->__('Invalid ID parameter.'));
        ...
        if (!$model->getId()) {
            $this->_getSession()->addError(Mage::helper('Magento_Oauth_Helper_Data')->__('Entry with ID #%s not found.', $id));
            $this->_redirect('*/*/index');
            return;
        }
    ...
    public function deleteAction()
    {
        ...
        if ($consumerId) {
            try {
                ...
                if (!$consumer->getId()) {
                    Mage::throwException(Mage::helper('Magento_Oauth_Helper_Data')->__('Unable to find a consumer.'));
                }

                $consumer->delete();

                $this->_getSession()->addSuccess(Mage::helper('Magento_Oauth_Helper_Data')->__('The consumer has been deleted.'));
            } catch (Magento_Core_Exception $e) {
                $this->_getSession()->addError($e->getMessage());
            } catch (Exception $e) {
                $this->_getSession()->addException(
                    $e, Mage::helper('Magento_Oauth_Helper_Data')->__('An error occurred while deleting the consumer.')
                );
            }
        }
        $this->_redirect('*/*/index');
