...
class Magento_Pbridge_Model_Payment_Method_Pbridge_Ipn
{
    ...
    public function processIpnRequest()
    {
        ...
        if ($error = $http->getError()) {
            $this->_notifyAdmin(Mage::helper('Magento_Pbridge_Helper_Data')->__('IPN postback HTTP error: %s', $error));
            return;
        }
        ...
        if (false !== preg_match('~VERIFIED~si', $response)) {
            ...
            $this->_notifyAdmin(Mage::helper('Magento_Pbridge_Helper_Data')->__('IPN postback Validation error: %s', $sReq));
        }
    }
    ...
    protected function _createIpnComment($comment = '', $addToHistory = true)
    {
        ...
        if ($this->getIpnFormData('txn_id')) {
            $message .= ' ' . Mage::helper('Magento_Pbridge_Helper_Data')->__('Original gateway transaction id: #%s.', $this->getIpnFormData('txn_id'));
        }
        if ($comment) {
            $message .= ' ' . $comment;
        }
