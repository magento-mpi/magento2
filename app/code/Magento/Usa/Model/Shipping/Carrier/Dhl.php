    ...
    implements Magento_Shipping_Model_Carrier_Interface
{
    ...
    protected function _doRequest()
    {
        ...
        if (!$hasShipCode) {
            $this->_errors[] = Mage::helper('Magento_Usa_Helper_Data')->__('We don\'t have a way to ship to the selected shipping address. Please choose another address or edit the current address.');
            return;
        }
            ...
            if (strpos(trim($response), '<?xml') === 0) {
                ...
                if (is_object($xml)) {
                    if (
                        ...
                        $this->_errors[$code] = Mage::helper('Magento_Usa_Helper_Data')->__('Error #%s : %s (%s)', $code, $description, $context);
                    } else {
                        if ($r->getDestCountryId() == self::USA_COUNTRY_ID) {
                            if ($xml->Shipment) {
                                ...
                                $this->_errors[] = Mage::helper('Magento_Usa_Helper_Data')->__('Shipment is not available.');
                            }
                        } else {
                ...
                $this->_errors[] = Mage::helper('Magento_Usa_Helper_Data')->__('Please format your response correctly.');
            }
        }
    ...
    protected function _parseXmlObject($shipXml)
    {
        if (
            ...
            $this->_errors[$code] = Mage::helper('Magento_Usa_Helper_Data')->__('Error #%s: %s', $code, $description);
        } elseif (
            is_object($shipXml->Faults)
    ...
    public function getCode($type, $code = '')
    {
        ...
        $codes = array(
            'service' => array(
                'IE' => Mage::helper('Magento_Usa_Helper_Data')->__('International Express'),
                'E SAT' => Mage::helper('Magento_Usa_Helper_Data')->__('Express Saturday'),
                'E 10:30AM' => Mage::helper('Magento_Usa_Helper_Data')->__('Express 10:30 AM'),
                'E' => Mage::helper('Magento_Usa_Helper_Data')->__('Express'),
                'N' => Mage::helper('Magento_Usa_Helper_Data')->__('Next Afternoon'),
                'S' => Mage::helper('Magento_Usa_Helper_Data')->__('Second Day Service'),
                'G' => Mage::helper('Magento_Usa_Helper_Data')->__('Ground'),
            ),
            'shipment_type' => array(
                'L' => Mage::helper('Magento_Usa_Helper_Data')->__('Letter'),
                'P' => Mage::helper('Magento_Usa_Helper_Data')->__('Package'),
            ),
            'international_searvice' => 'IE',
            'dutypayment_type' => array(
                'S' => Mage::helper('Magento_Usa_Helper_Data')->__('Sender'),
                'R' => Mage::helper('Magento_Usa_Helper_Data')->__('Receiver'),
                '3' => Mage::helper('Magento_Usa_Helper_Data')->__('Third Party'),
            ),
            ...
            if (strpos(trim($response), '<?xml') === 0) {
                ...
                if (is_object($xml)) {
                    ...
                    if (
                        ...
                        $description = $xml->Faults->Fault->Description;
                        $context = $xml->Faults->Fault->Context;
                        $errorTitle = Mage::helper('Magento_Usa_Helper_Data')->__('Error #%s : %s (%s)', $code, $description, $context);
                        ...
                        foreach ($trackxml->Shipment as $txml) {
                            ...
                            if (is_object($txml)) {
                                ...
                                if ($txml->Fault) {
                                    ...
                                    $errorArr[$tracknum] = Mage::helper('Magento_Usa_Helper_Data')->__('Error #%s: %s', $code, $description);
                                } elseif ($txml->Result) {
                                    ...
                                    if ($code === 0) {
                                        ...
                                        */
                                        ...
                                        if (isset($txml->Delivery)) {
                                            ...
                                            $rArr['status'] = Mage::helper('Magento_Usa_Helper_Data')->__('Delivered');
                                            ...
                                            $rArr['status'] = Mage::helper('Magento_Usa_Helper_Data')->__('Shipment picked up');
                                        } else {
                                            $rArr['status'] = (string)$txml->ShipmentType->Desc
                                                  . Mage::helper('Magento_Usa_Helper_Data')->__(' was not delivered nor scanned');
                                        }
                                        ...
                                        if ($description)
                                            ...
                                            $errorArr[$tracknum] = Mage::helper('Magento_Usa_Helper_Data')->__('Unable to retrieve tracking');
                                    }
                                } else {
                                    $errorArr[$tracknum] = Mage::helper('Magento_Usa_Helper_Data')->__('Unable to retrieve tracking');
                                }
                ...
                $errorTitle = Mage::helper('Magento_Usa_Helper_Data')->__('Response is in the wrong format');
            }
        }
                ...
                foreach ($trackings as $tracking) {
                    if ($data = $tracking->getAllData()) {
                        if (isset($data['status'])) {
                            ...
                            $statuses .= Mage::helper('Magento_Usa_Helper_Data')->__($data['error_message']) . "\n<br/>";
                        }
                    }
                }
            }
    ...
    {
        return array(
            self::ADDITIONAL_PROTECTION_VALUE_CONFIG => Mage::helper('Magento_Usa_Helper_Data')->__('Configuration'),
            self::ADDITIONAL_PROTECTION_VALUE_SUBTOTAL => Mage::helper('Magento_Usa_Helper_Data')->__('Subtotal'),
            self::ADDITIONAL_PROTECTION_VALUE_SUBTOTAL_WITH_DISCOUNT => Mage::helper('Magento_Usa_Helper_Data')->__('Subtotal With Discount'),
        );
    }
    ...
    {
        return array(
            self::ADDITIONAL_PROTECTION_ROUNDING_FLOOR => Mage::helper('Magento_Usa_Helper_Data')->__('To Lower'),
            self::ADDITIONAL_PROTECTION_ROUNDING_CEIL => Mage::helper('Magento_Usa_Helper_Data')->__('To Upper'),
            self::ADDITIONAL_PROTECTION_ROUNDING_ROUND => Mage::helper('Magento_Usa_Helper_Data')->__('Round'),
        );
    }
