 ...
 */
class Magento_Usa_Model_Shipping_Carrier_Dhl_Label_Pdf_PageBuilder
{
    /**
     ...
     * @return Magento_Usa_Model_Shipping_Carrier_Dhl_Label_Pdf_PageBuilder
     */
    ...
    public function addProductName($name)
    {
        ...
        if (!strlen($name)) {
            throw new InvalidArgumentException(Mage::helper('Magento_Usa_Helper_Data')->__('Product name is missing'));
        }
    ...
    public function addProductContentCode($code)
    {
        ...
        if (!key_exists($code, $codes)) {
            throw new InvalidArgumentException(Mage::helper('Magento_Usa_Helper_Data')->__('Product content code is invalid'));
        }
        $font = null;
        if ($codes[$code]) {
    ...
    public function addSenderInfo(SimpleXMLElement $sender)
    {
        ...
        if (!$contactName) {
            throw new InvalidArgumentException(Mage::helper('Magento_Usa_Helper_Data')->__('Sender contact name is missing'));
        }
        ...
        if (!strlen($cityInfo)) {
            throw new InvalidArgumentException(Mage::helper('Magento_Usa_Helper_Data')->__('Sender city info is missing'));
        }
        ...
        if (!strlen($countryInfo)) {
            throw new InvalidArgumentException(Mage::helper('Magento_Usa_Helper_Data')->__('Sender country info is missing'));
        }
    ...
    public function addOriginInfo($serviceAreaCode)
    {
        if (strlen(!$serviceAreaCode)) {
            throw new InvalidArgumentException(Mage::helper('Magento_Usa_Helper_Data')->__('Origin serviceAreaCode is missing'));
        }
        $this->_page->saveGS();
        $this->_page->setFont($this->_fontNormal, 6);
    ...
    public function addReceiveInfo(SimpleXMLElement $consignee)
    {
        ...
        $y = $this->_page->drawLines(array($consignee->Contact->PersonName), $this->_x(283), $this->_y(98), 25,
            Magento_Usa_Model_Shipping_Carrier_Dhl_Label_Pdf_Page::ALIGN_RIGHT
        );
    ...
    public function addDestinationFacilityCode($countryCode, $serviceAreaCode, $facilityCode)
    {
        ...
        if (!strlen($code)) {
            throw new InvalidArgumentException(Mage::helper('Magento_Usa_Helper_Data')->__('Destination facility code is empty'));
        }
        $this->_page->drawText($code, $this->_x(144), $this->_y(186), null,
            Magento_Usa_Model_Shipping_Carrier_Dhl_Label_Pdf_Page::ALIGN_CENTER
        );
    ...
    public function addShipmentInformation($data)
    {
        ...
        if (!$refCode) {
            throw new InvalidArgumentException(Mage::helper('Magento_Usa_Helper_Data')->__('Reference code is missing'));
        }
        $this->_page->drawText('Ref Code: ' . Mage::helper('Magento_Usa_Helper_Data')->__('Order #%s', $refCode), $this->_x(8),
            $this->_y(224)
        );
        $this->_page->restoreGS();
        return $this;
    ...
    public function addWeightInfo($weight, $unit)
    {
        ...
        if (!isset($units[$unit])) {
            throw new InvalidArgumentException(Mage::helper('Magento_Usa_Helper_Data')->__('Weight unit is invalid'));
        }
        $unit = $units[$unit];
    ...
    public function addContentInfo($package)
    {
        ...
        $this->_page->setFont($this->_fontNormal, 6);
        if (empty($package)) {
            throw new InvalidArgumentException(Mage::helper('Magento_Usa_Helper_Data')->__('Package content is missing'));
        }
    ...
    public function addWaybillBarcode($number, $barCode)
    {
        ...
        if(!strlen($number) || !strlen($barCode)) {
            throw new InvalidArgumentException(Mage::helper('Magento_Usa_Helper_Data')->__('Waybill barcode information is missing'));
        }
    ...
    public function addRoutingBarcode($routingCode, $id, $barCode)
    {
        ...
        if(!$barCode) {
            throw new InvalidArgumentException(Mage::helper('Magento_Usa_Helper_Data')->__('Routing barcode is missing'));
        }
    ...
    public function addPieceIdBarcode($dataIdentifier, $licensePlate, $barCode)
    {
        ...
        if (!strlen($barCode)) {
            throw new InvalidArgumentException(Mage::helper('Magento_Usa_Helper_Data')->__('Piece Id barcode is missing'));
        }
    ...
    public function addPieceNumber($pieceNumber, $piecesTotal)
    {
        ...
        if (!$pieceNumber || !$piecesTotal) {
            throw new InvalidArgumentException(Mage::helper('Magento_Usa_Helper_Data')->__('Piece number information is missing'));
        }
