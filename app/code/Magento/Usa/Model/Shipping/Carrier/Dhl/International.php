    ...
    implements Magento_Shipping_Model_Carrier_Interface
{
        ...
        if ($this->_isDomestic) {
            ...
            switch ($contentType) {
                ...
                case self::DHL_CONTENT_TYPE_NON_DOC:
                    ...
                    break;
                default:
                    Mage::throwException(Mage::helper('Magento_Usa_Helper_Data')->__('Wrong Content Type'));
            }
        }
        $methods = array();
    ...
    public function getCode($type, $code = '')
    {
        $codes = array(
            'unit_of_measure'   => array(
                'L' => Mage::helper('Magento_Usa_Helper_Data')->__('Pounds'),
                'K' => Mage::helper('Magento_Usa_Helper_Data')->__('Kilograms'),
            ),
            'unit_of_dimension' => array(
                'I' => Mage::helper('Magento_Usa_Helper_Data')->__('Inches'),
                'C' => Mage::helper('Magento_Usa_Helper_Data')->__('Centimeters'),
            ),
            'unit_of_dimension_cut' => array(
                'I' => Mage::helper('Magento_Usa_Helper_Data')->__('inch'),
                'C' => Mage::helper('Magento_Usa_Helper_Data')->__('cm'),
            ),
            'dimensions' => array(
                'HEIGHT'    => Mage::helper('Magento_Usa_Helper_Data')->__('Height'),
                'DEPTH'     => Mage::helper('Magento_Usa_Helper_Data')->__('Depth'),
                'WIDTH'     => Mage::helper('Magento_Usa_Helper_Data')->__('Width'),
            ),
            'size'              => array(
                '0' => Mage::helper('Magento_Usa_Helper_Data')->__('Regular'),
                '1' => Mage::helper('Magento_Usa_Helper_Data')->__('Specific'),
            ),
    ...
    {
        $docType = array(
            '2' => Mage::helper('Magento_Usa_Helper_Data')->__('Easy shop'),
            '5' => Mage::helper('Magento_Usa_Helper_Data')->__('Sprintline'),
            '6' => Mage::helper('Magento_Usa_Helper_Data')->__('Secureline'),
            '7' => Mage::helper('Magento_Usa_Helper_Data')->__('Express easy'),
            '9' => Mage::helper('Magento_Usa_Helper_Data')->__('Europack'),
            'B' => Mage::helper('Magento_Usa_Helper_Data')->__('Break bulk express'),
            'C' => Mage::helper('Magento_Usa_Helper_Data')->__('Medical express'),
            'D' => Mage::helper('Magento_Usa_Helper_Data')->__('Express worldwide'), // product content code: DOX
            'U' => Mage::helper('Magento_Usa_Helper_Data')->__('Express worldwide'), // product content code: ECX
            'K' => Mage::helper('Magento_Usa_Helper_Data')->__('Express 9:00'),
            'L' => Mage::helper('Magento_Usa_Helper_Data')->__('Express 10:30'),
            'G' => Mage::helper('Magento_Usa_Helper_Data')->__('Domestic economy select'),
            'W' => Mage::helper('Magento_Usa_Helper_Data')->__('Economy select'),
            'I' => Mage::helper('Magento_Usa_Helper_Data')->__('Break bulk economy'),
            'N' => Mage::helper('Magento_Usa_Helper_Data')->__('Domestic express'),
            'O' => Mage::helper('Magento_Usa_Helper_Data')->__('Others'),
            'R' => Mage::helper('Magento_Usa_Helper_Data')->__('Globalmail business'),
            'S' => Mage::helper('Magento_Usa_Helper_Data')->__('Same day'),
            'T' => Mage::helper('Magento_Usa_Helper_Data')->__('Express 12:00'),
            'X' => Mage::helper('Magento_Usa_Helper_Data')->__('Express envelope'),
        );

        $nonDocType = array(
            '1' => Mage::helper('Magento_Usa_Helper_Data')->__('Customer services'),
            '3' => Mage::helper('Magento_Usa_Helper_Data')->__('Easy shop'),
            '4' => Mage::helper('Magento_Usa_Helper_Data')->__('Jetline'),
            '8' => Mage::helper('Magento_Usa_Helper_Data')->__('Express easy'),
            'P' => Mage::helper('Magento_Usa_Helper_Data')->__('Express worldwide'),
            'Q' => Mage::helper('Magento_Usa_Helper_Data')->__('Medical express'),
            'E' => Mage::helper('Magento_Usa_Helper_Data')->__('Express 9:00'),
            'F' => Mage::helper('Magento_Usa_Helper_Data')->__('Freight worldwide'),
            'H' => Mage::helper('Magento_Usa_Helper_Data')->__('Economy select'),
            'J' => Mage::helper('Magento_Usa_Helper_Data')->__('Jumbo box'),
            'M' => Mage::helper('Magento_Usa_Helper_Data')->__('Express 10:30'),
            'V' => Mage::helper('Magento_Usa_Helper_Data')->__('Europack'),
            'Y' => Mage::helper('Magento_Usa_Helper_Data')->__('Express 12:00'),
        );
    ...
    protected function _parseResponse($response)
    {
        ...
        $responseError =  Mage::helper('Magento_Usa_Helper_Data')->__('The response is in wrong format.');
                ...
                if (is_object($xml)) {
                    if (in_array($xml->getName(), array('ErrorResponse', 'ShipmentValidateErrorResponse'))
                        ...
                        if ($this->_isShippingLabelFlag) {
                            ...
                            Mage::throwException(Mage::helper('Magento_Usa_Helper_Data')->__('Error #%s : %s', trim($code), trim($data)));
                        }
                        ...
                        $this->_errors[$code] = Mage::helper('Magento_Usa_Helper_Data')->__('Error #%s : %s', trim($code), trim($data));
                    } else {
                        if (isset($xml->GetQuoteResponse->BkgDetails->QtdShp)) {
                            ...
                            try {
                                ...
                                Mage::throwException(Mage::helper('Magento_Usa_Helper_Data')->__($e->getMessage()));
                            }
                            return $result;
                        } else {
    ...
    protected function _addRate(Magento_Usa_Model_Simplexml_Element $shipmentDetails)
    {
        if (isset($shipmentDetails->ProductShortName)
            ...
            if ($currencyCode != $baseCurrencyCode) {
                ...
                if (!empty($rates) && isset($rates[$baseCurrencyCode])) {
                    ...
                    if (!isset($rates[$currencyCode]) || !$totalEstimate) {
                        $totalEstimate = false;
                        $this->_errors[] = Mage::helper('Magento_Usa_Helper_Data')->__('We had to skip DHL method %s because we couldn\'t find exchange rate %s (Base Currency).', $currencyCode, $baseCurrencyCode, $dhlProductDescription);
                    }
                }
            }
            if ($totalEstimate) {
                ...
                $this->_errors[] = Mage::helper('Magento_Usa_Helper_Data')->__("Zero shipping charge for '%s'", $dhlProductDescription);
            }
        } else {
            ...
            $dhlProductDescription = $dhlProductDescription ? $dhlProductDescription : Mage::helper('Magento_Usa_Helper_Data')->__("DHL");
            $this->_errors[] = Mage::helper('Magento_Usa_Helper_Data')->__("Zero shipping charge for '%s'", $dhlProductDescription);
        }
        return $this;
    }
    ...
    protected function _getDimensionUnit()
    {
        ...
        if (empty($measureUnit)) {
            Mage::throwException(Mage::helper('Magento_Usa_Helper_Data')->__("Cannot identify measure unit for %s", $countryId));
        }
        return $measureUnit;
    }
    ...
    protected function _getWeightUnit()
    {
        ...
        if (empty($weightUnit)) {
            Mage::throwException(Mage::helper('Magento_Usa_Helper_Data')->__("Cannot identify weight unit for %s", $countryId));
        }
        return $weightUnit;
    }
    ...
    public function proccessAdditionalValidation(Magento_Shipping_Model_Rate_Request $request)
    {
        ...
        if (!count($this->getAllItems($request))) {
            $this->_errors[] = Mage::helper('Magento_Usa_Helper_Data')->__('There is no items in this order');
        }
    ...
    {
        return array(
            self::DHL_CONTENT_TYPE_DOC      => Mage::helper('Magento_Usa_Helper_Data')->__('Documents'),
            self::DHL_CONTENT_TYPE_NON_DOC  => Mage::helper('Magento_Usa_Helper_Data')->__('Non Documents')
        );
    }
        ...
        foreach ($packages as &$piece) {
            ...
            if ($params['width'] || $params['length'] || $params['height']) {
                ...
                if ($params['width'] < $minValue || $params['length'] < $minValue || $params['height'] < $minValue) {
                    $message = Mage::helper('Magento_Usa_Helper_Data')->__('Height, width and length should be equal or greater than %s', $minValue);
    ...
    protected function _doRequest()
    {
        ...
        if (!$originRegion) {
            Mage::throwException(Mage::helper('Magento_Usa_Helper_Data')->__('Wrong Region'));
        }
    ...
    protected function _parseXmlTrackingResponse($trackings, $response)
    {
        ...
        if (strlen(trim($response)) > 0) {
            ...
            if (is_object($xml) && ((isset($xml->Response->Status->ActionStatus)
                ...
                $code = isset($nodeCondition->ConditionCode) ? (string)$nodeCondition->ConditionCode : 0;
                $data = isset($nodeCondition->ConditionData) ? (string)$nodeCondition->ConditionData : '';
                $this->_errors[$code] = Mage::helper('Magento_Usa_Helper_Data')->__('Error #%s : %s', $code, $data);
                ...
                foreach ($xml->AWBInfo as $awbinfo) {
                    ...
                    if (!is_object($awbinfo) || !$awbinfo->ShipmentInfo) {
                        $this->_errors[$trackNum] = Mage::helper('Magento_Usa_Helper_Data')->__('Unable to retrieve tracking');
                        continue;
                    }
    ...
    public function requestToShipment(Magento_Shipping_Model_Shipment_Request $request)
    {
        ...
        if (!is_array($packages) || !$packages) {
            Mage::throwException(Mage::helper('Magento_Usa_Helper_Data')->__('No packages for request'));
        }
