...
class Magento_Eav_Model_Entity_Attribute_Source_Config extends Magento_Eav_Model_Entity_Attribute_Source_Abstract
{
    ...
    public function getAllOptions()
    {
        if ($this->_options === null) {
            ...
            if (!$rootNode) {
                throw Mage::exception('Magento_Eav', Mage::helper('Magento_Eav_Helper_Data')->__('Failed to load node %s from config', $this->_configNodePath));
            }
            $options = $rootNode->children();
            if (empty($options)) {
                throw Mage::exception('Magento_Eav', Mage::helper('Magento_Eav_Helper_Data')->__('No options found in config node %s', $this->_configNodePath));
            }
            foreach ($options as $option) {
                $this->_options[] = array(
                    'value' => (string)$option->value,
                    'label' => Mage::helper('Magento_Eav_Helper_Data')->__((string)$option->label)
                );
            }
        }
