...
abstract class Magento_Eav_Model_Entity_Collection_Abstract extends Magento_Data_Collection_Db
{
    ...
    public function setEntity($entity)
    {
        if ($entity instanceof Magento_Eav_Model_Entity_Abstract) {
            ...
            throw Mage::exception(
                'Magento_Eav',
                Mage::helper('Magento_Eav_Helper_Data')->__('Invalid entity supplied: %s', print_r($entity, 1))
            );
        }
        return $this;
    }
    ...
    public function addItem(Magento_Object $object)
    {
        if (get_class($object) !== $this->_itemObjectClass) {
            throw Mage::exception(
                'Magento_Eav',
                Mage::helper('Magento_Eav_Helper_Data')->__('Attempt to add an invalid object')
            );
        }
    ...
    public function addAttributeToFilter($attribute, $condition = null, $joinType = 'inner')
    {
        ...
        } else if ($attribute instanceof Magento_Eav_Model_Entity_Attribute_Interface) {
    ...
    public function addAttributeToSelect($attribute, $joinType = false)
    {
        ...
        if ($joinType !== false && !$this->getEntity()->getAttribute($attribute)->isStatic()) {
            ...
            if (empty($attrInstance)) {
                throw Mage::exception(
                    'Magento_Eav',
                    Mage::helper('Magento_Eav_Helper_Data')->__('Invalid attribute requested: %s', (string)$attribute)
                );
            }
    ...
    public function addExpressionAttributeToSelect($alias, $expression, $attribute)
    {
        ...
        if (isset($this->_joinFields[$alias])) {
            throw Mage::exception(
                'Magento_Eav',
                Mage::helper('Magento_Eav_Helper_Data')
     ...
     * @param string|Magento_Eav_Model_Entity_Attribute_Abstract $attribute
    ...
    public function joinAttribute($alias, $attribute, $bind, $filter = null, $joinType = 'inner', $storeId = null)
    {
        ...
        if (isset($this->_joinAttributes[$alias])) {
            throw Mage::exception(
                'Magento_Eav',
                Mage::helper('Magento_Eav_Helper_Data')->__('Invalid alias, already exists in joint attributes')
            );
        }
        ...
        if (!$bindAttribute || (!$bindAttribute->isStatic() && !$bindAttribute->getId())) {
            throw Mage::exception('Magento_Eav', Mage::helper('Magento_Eav_Helper_Data')->__('Invalid foreign key'));
        }
        ...
        // validate entity
        if (empty($entity) && $attribute instanceof Magento_Eav_Model_Entity_Attribute_Abstract) {
        ...
        if (!$entity || !$entity->getTypeId()) {
            throw Mage::exception('Magento_Eav', Mage::helper('Magento_Eav_Helper_Data')->__('Invalid entity type'));
        }
        ...
        if (!$attribute) {
            throw Mage::exception('Magento_Eav', Mage::helper('Magento_Eav_Helper_Data')->__('Invalid attribute type'));
        }
    ...
    public function joinField($alias, $table, $field, $bind, $cond = null, $joinType = 'inner')
    {
        ...
        if (isset($this->_joinFields[$alias])) {
            throw Mage::exception(
                'Magento_Eav',
                Mage::helper('Magento_Eav_Helper_Data')->__('A joined field with this alias is already declared.')
            );
        }
    ...
    public function joinTable($table, $bind, $fields = null, $cond = null, $joinType = 'inner')
    {
        ...
        if (!$fields) {
            throw Mage::exception('Magento_Eav', Mage::helper('Magento_Eav_Helper_Data')->__('Invalid joint fields'));
        }
        foreach ($fields as $alias=>$field) {
            if (isset($this->_joinFields[$alias])) {
                throw Mage::exception(
                    'Magento_Eav',
                    Mage::helper('Magento_Eav_Helper_Data')
                        ->__('A joint field with this alias (%s) is already declared.', $alias)
                );
            }
            $this->_joinFields[$alias] = array(
                'table' => $tableAlias,
    ...
    {
        $select->columns(array('value' => $table . '.value'));
        return $select;
    }
    ...
    protected function _setItemAttributeValue($valueInfo)
    {
        ...
        if (!isset($this->_itemsById[$entityId])) {
            throw Mage::exception('Magento_Eav',
                Mage::helper('Magento_Eav_Helper_Data')->__('Data integrity: No header row found for attribute')
            );
        }
    ...
    protected function _getAttributeFieldName($attributeCode)
    {
        ...
        if (!$attribute) {
            throw Mage::exception(
                'Magento_Eav',
                Mage::helper('Magento_Eav_Helper_Data')->__('Invalid attribute name: %s', $attributeCode)
            );
        }
