...
class Magento_Eav_Model_Entity_Setup extends Magento_Core_Model_Resource_Setup
{
    ...
    {
        $this->_cache->clean(array(Magento_Eav_Model_Cache_Type::CACHE_TAG));
        return $this;
    }
...
/******************* ENTITY TYPES *****************/
    ...
    public function getEntityTypeId($entityTypeId)
    {
        ...
        if (!is_numeric($entityTypeId)) {
            throw Mage::exception('Magento_Eav', Mage::helper('Magento_Eav_Helper_Data')->__('Wrong entity ID'));
        }
...
/******************* ATTRIBUTE SETS *****************/
    ...
    public function getAttributeSetId($entityTypeId, $setId)
    {
        ...
        if (!is_numeric($setId)) {
            throw Mage::exception('Magento_Eav', Mage::helper('Magento_Eav_Helper_Data')->__('Wrong attribute set ID'));
        }
...
/******************* ATTRIBUTE GROUPS *****************/
    ...
    public function getAttributeGroupId($entityTypeId, $setId, $groupId)
    {
        ...
        if (!is_numeric($groupId)) {
            throw Mage::exception('Magento_Eav', Mage::helper('Magento_Eav_Helper_Data')->__('Wrong attribute group ID'));
        }
        return $groupId;
    }
...
/******************* ATTRIBUTES *****************/
    ...
    {
        $attributeCodeMaxLength = Magento_Eav_Model_Entity_Attribute::ATTRIBUTE_CODE_MAX_LENGTH;
    ...
    public function addAttributeOption($option)
    {
        ...
        if (isset($option['value'])) {
            foreach ($option['value'] as $optionId => $values) {
                ...
                // Default value
                if (!isset($values[0])) {
                    Mage::throwException(Mage::helper('Magento_Eav_Helper_Data')->__('Default option value is not defined'));
                }
                $condition = array('option_id =?' => $intOptionId);
...
/****************************** CREATE ENTITY TABLES ***********************************/
    ...
    public function createEntityTables($baseTableName, array $options = array())
    {
        ...
        if (!empty($customTypes)) {
            foreach ($customTypes as $type => $fieldType) {
                if (count($fieldType) != 2) {
                    throw Mage::exception('Magento_Eav', Mage::helper('Magento_Eav_Helper_Data')->__('Wrong type definition for %s', $type));
                }
        ...
        try {
           ...
           $connection->rollBack();
           throw Mage::exception('Magento_Eav', Mage::helper('Magento_Eav_Helper_Data')->__('Can\'t create table: %s', $tableName));
        }
