...
class Magento_Eav_Model_Attribute_Data_Image extends Magento_Eav_Model_Attribute_Data_File
{
    ...
    {
        $label  = Mage::helper('Magento_Eav_Helper_Data')->__($this->getAttribute()->getStoreLabel());
        ...
        if (!is_uploaded_file($value['tmp_name']) || !$imageProp) {
            return array(
                Mage::helper('Magento_Eav_Helper_Data')->__('"%s" is not a valid file', $label)
            );
        }
        ...
        if (!isset($allowImageTypes[$imageProp[2]])) {
            return array(
                Mage::helper('Magento_Eav_Helper_Data')->__('"%s" is not a valid image format', $label)
            );
        }
        ...
        if (!empty($rules['max_file_size'])) {
            ...
            if ($rules['max_file_size'] < $size) {
                $errors[] = Mage::helper('Magento_Eav_Helper_Data')->__('"%s" exceeds the allowed file size.', $label);
            };
        }
        ...
        if (!empty($rules['max_image_width'])) {
            if ($rules['max_image_width'] < $imageProp[0]) {
                ...
                $errors[] = Mage::helper('Magento_Eav_Helper_Data')->__('"%s" width exceeds allowed value of %s px.', $label, $r);
            };
        }
        if (!empty($rules['max_image_heght'])) {
            if ($rules['max_image_heght'] < $imageProp[1]) {
                $r = $rules['max_image_heght'];
                $errors[] = Mage::helper('Magento_Eav_Helper_Data')->__('"%s" height exceeds allowed value of %s px.', $label, $r);
            };
        }
