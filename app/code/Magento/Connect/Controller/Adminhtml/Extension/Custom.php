...
class Magento_Connect_Controller_Adminhtml_Extension_Custom extends Magento_Adminhtml_Controller_Action
{
    ...
    public function loadAction()
    {
        ...
        if ($packageName) {
            ...
            try {
                ...
                if (!$data) {
                    Mage::throwException(Mage::helper('Magento_Connect_Helper_Data')->__('Something went wrong loading the package data.'));
                }
                $data = array_merge($data, array('file_name' => $packageName));
                $session->setCustomExtensionPackageFormData($data);
                $session->addSuccess(
                    Mage::helper('Magento_Connect_Helper_Data')->__('The package %s data has been loaded.', $packageName)
                );
            } catch (Exception $e) {
    ...
    public function saveAction()
    {
        ...
        try {
            ...
            $ext->setData($p);
            if ($ext->savePackage()) {
                $session->addSuccess(Mage::helper('Magento_Connect_Helper_Data')->__('The package data has been saved.'));
            } else {
                $session->addError(Mage::helper('Magento_Connect_Helper_Data')->__('Something went wrong saving the package data.'));
                $this->_redirect('*/*/edit');
            }
            if (empty($create)) {
                $this->_redirect('*/*/edit');
            ...
            $session->addException($e, Mage::helper('Magento_Connect_Helper_Data')->__('Something went wrong saving the package.'));
            $this->_redirect('*/*');
        }
    }
    ...
    public function createAction()
    {
        ...
        try {
            ...
            $session->addException($e, Mage::helper('Magento_Connect_Helper_Data')->__('Something went wrong creating the package.'));
            $this->_redirect('*/*');
        }
    }
