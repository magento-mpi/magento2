...
class Bundle extends \Magento\Catalog\Block\Product\View\AbstractView
{
    ...
    public function getJsonConfig()
    {
        ...
        foreach ($optionsArray as $optionItem) {
            ...
            foreach ($optionItem->getSelections() as $selectionItem) {
                ...
                $tierPrices = $selectionItem->getPriceInfo()
                    ->getPrice(\Magento\Catalog\Pricing\Price\TierPriceInterface::PRICE_TYPE_TIER)
                    ->getTierPriceList();
        ...
        $productAmount = $currentProduct
            ->getPriceInfo()
            ->getPrice(\Magento\Catalog\Pricing\Price\FinalPriceInterface::PRICE_TYPE_FINAL)
            ->getAmount();

        $baseProductAmount = $currentProduct
            ->getPriceInfo()
            ->getPrice(\Magento\Catalog\Pricing\Price\RegularPrice::PRICE_TYPE_PRICE_DEFAULT)
            ->getAmount();
        ...
        $config = array(
            ...
            'specialPrice' => $currentProduct
                ->getPriceInfo()
                ->getPrice(\Magento\Catalog\Pricing\Price\SpecialPriceInterface::PRICE_TYPE_SPECIAL)
                ->getValue(),
