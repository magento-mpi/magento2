...
class Magento_Backend_Model_Auth
{
    ...
    protected function _initCredentialStorage()
    {
        ...
        self::throwException(
            Mage::helper('Magento_Backend_Helper_Data')->__('There are no authentication credential storage.')
        );
    }
    ...
    public function login($username, $password)
    {
        if (empty($username) || empty($password)) {
            self::throwException(
                Mage::helper('Magento_Backend_Helper_Data')->__('Please correct the user name or password.')
            );
        }
    ...
    {
        if (is_null($msg)) {
            $msg = Mage::helper('Magento_Backend_Helper_Data')->__('Authentication error occurred.');
        }
