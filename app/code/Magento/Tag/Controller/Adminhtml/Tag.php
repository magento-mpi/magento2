...
class Magento_Tag_Controller_Adminhtml_Tag extends Magento_Adminhtml_Controller_Action
{
    ...
    protected function _initAction()
    {
        $this->loadLayout()
            ->_addBreadcrumb(
                Mage::helper('Magento_Tag_Helper_Data')->__('Catalog'), Mage::helper('Magento_Tag_Helper_Data')->__('Catalog')
            )
            ->_addBreadcrumb(
                Mage::helper('Magento_Tag_Helper_Data')->__('Tags'), Mage::helper('Magento_Tag_Helper_Data')->__('Tags')
            );
    ...
    public function indexAction()
    {
        ...
        $this->_initAction()
            ->_addBreadcrumb(
                Mage::helper('Magento_Tag_Helper_Data')->__('All Tags'),
                Mage::helper('Magento_Tag_Helper_Data')->__('All Tags')
            )
    ...
    public function editAction()
    {
        ...
        if (! ($model = $this->_initTag())) {
            Mage::getSingleton('Magento_Adminhtml_Model_Session')->addError(
                Mage::helper('Magento_Tag_Helper_Data')->__('Please correct the tag and try again.')
            );
        ...
        if ($postData = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                Mage::getSingleton('Magento_Adminhtml_Model_Session')->addSuccess(
                    Mage::helper('Magento_Tag_Helper_Data')->__('You saved the tag.')
                );
    ...
    public function deleteAction()
    {
        ...
        if ($model && $model->getId()) {
            try {
                $model->delete();
                $session->addSuccess(Mage::helper('Magento_Tag_Helper_Data')->__('The tag has been deleted.'));
            } catch (Exception $e) {
                $session->addError($e->getMessage());
            }
        } else {
            $session->addError(Mage::helper('Magento_Tag_Helper_Data')->__('We can\'t find a tag to delete.'));
        }
    ...
    public function pendingAction()
    {
        ...
        $this->_initAction()
            ->_addBreadcrumb(
                Mage::helper('Magento_Tag_Helper_Data')->__('Pending Tags'),
                Mage::helper('Magento_Tag_Helper_Data')->__('Pending Tags')
            )
            ->renderLayout();
    }
    ...
    public function massDeleteAction()
    {
        ...
        if (!is_array($tagIds)) {
             Mage::getSingleton('Magento_Adminhtml_Model_Session')->addError($this->__('Please select tag(s).'));
        } else {
            try {
