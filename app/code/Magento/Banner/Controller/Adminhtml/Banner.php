    ...
    public function editAction()
    {
        ...
        if (!$model->getId() && $bannerId) {
            Mage::getSingleton('Magento_Adminhtml_Model_Session')
                ->addError(Mage::helper('Magento_Banner_Helper_Data')->__('This banner no longer exists.'));
            $this->_redirect('*/*/');
            return;
        }
        ...
        $this->_setActiveMenu('Magento_Banner::cms_magento_banner');
        $this->_addBreadcrumb(
            $bannerId ? Mage::helper('Magento_Banner_Helper_Data')->__('Edit Banner')
                : Mage::helper('Magento_Banner_Helper_Data')->__('New Banner'),
            $bannerId ? Mage::helper('Magento_Banner_Helper_Data')->__('Edit Banner')
                : Mage::helper('Magento_Banner_Helper_Data')->__('New Banner')
        )
        ->renderLayout();
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            if (!$model->getId() && $bannerId) {
                Mage::getSingleton('Magento_Adminhtml_Model_Session')->addError(
                    Mage::helper('Magento_Banner_Helper_Data')->__('This banner does not exist.')
                );
                $this->_redirect('*/*/');
                return;
            }
            ...
            try {
                ...
                Mage::getSingleton('Magento_Adminhtml_Model_Session')->addSuccess(
                    Mage::helper('Magento_Banner_Helper_Data')->__('You saved the banner.')
                );
                ...
                $this->_getSession()->addError(
                    Mage::helper('Magento_Banner_Helper_Data')->__('We cannot save the banner.')
                );
                $redirectBack = true;
                Mage::logException($e);
            }
    ...
    public function deleteAction()
    {
        ...
        if ($bannerId = $this->getRequest()->getParam('id')) {
            try {
                ...
                Mage::getSingleton('Magento_Adminhtml_Model_Session')->addSuccess(
                    Mage::helper('Magento_Banner_Helper_Data')->__('The banner has been deleted.')
                );
                // go to grid
                $this->_redirect('*/*/');
                return;
                ...
                // @codingStandardsIgnoreStart
                    Mage::helper('Magento_Banner_Helper_Data')->__('Something went wrong deleting banner data. Please review the action log and try again.')
        ...
        Mage::getSingleton('Magento_Adminhtml_Model_Session')->addError(
            Mage::helper('Magento_Banner_Helper_Data')->__('We cannot find a banner to delete.')
        );
        // go to grid
        $this->_redirect('*/*/');
    }
    ...
    public function massDeleteAction()
    {
        ...
        if (!is_array($ids)) {
            ...
            try {
                ...
                $this->_getSession()->addError(
                // @codingStandardsIgnoreStart
                    Mage::helper('Magento_Banner_Helper_Data')->__('Something went wrong mass-deleting banners. Please review the action log and try again.')
                // @codingStandardsIgnoreEnd
                );
                Mage::logException($e);
                return;
