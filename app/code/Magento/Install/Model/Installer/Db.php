...
class Magento_Install_Model_Installer_Db extends Magento_Install_Model_Installer_Abstract
{
    ...
    public function checkDbConnectionData($data)
    {
        ...
        try {
            ...
            if (!$resource = $this->_getDbResource($dbModel)) {
                Mage::throwException(
                    Mage::helper('Magento_Install_Helper_Data')->__('There is no resource for %s DB model.', $dbModel)
                );
            }
            ...
            if (!empty($absenteeExtensions)) {
                Mage::throwException(
                    Mage::helper('Magento_Install_Helper_Data')
            ...
            if (version_compare($version, $requiredVersion) == -1) {
                Mage::throwException(
                    Mage::helper('Magento_Install_Helper_Data')->__('The database server version doesn\'t match system requirements (required: %s, actual: %s).', $requiredVersion, $version)
                );
            }
            ...
            Mage::logException($e);
            Mage::throwException(
                Mage::helper('Magento_Install_Helper_Data')->__('Something went wrong while connecting to the database.')
            );
        }
    ...
    protected function _getCheckedData($data)
    {
        if (!isset($data['db_name']) || empty($data['db_name'])) {
            Mage::throwException(
                Mage::helper('Magento_Install_Helper_Data')->__('The Database Name field cannot be empty.')
            );
        }
        ...
        if ($data['db_prefix'] != '') {
            if (!preg_match('/^[a-z]+[a-z0-9_]*$/', $data['db_prefix'])) {
                Mage::throwException(
                    Mage::helper('Magento_Install_Helper_Data')->__('The table prefix should contain only letters (a-z), numbers (0-9) or underscores (_); the first character should be a letter.')
                );
            }
        }
