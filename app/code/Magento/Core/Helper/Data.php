...
class Magento_Core_Helper_Data extends Magento_Core_Helper_Abstract
{
    ...
    {
        $types = array();
        $config = Mage::getConfig()->getNode(Magento_Core_Model_Cache::XML_PATH_TYPES);
...
XML;
    ...
    {
        if (preg_match('#\.\.[\\\/]#', $name)) {
            throw new Magento_Core_Exception($this->__('Requested file may not include parent directory traversal ("../", "..\\" notation)'));
        }
        return true;
    }
