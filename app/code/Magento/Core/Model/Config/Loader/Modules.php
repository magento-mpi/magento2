...
class Magento_Core_Model_Config_Loader_Modules implements Magento_Core_Model_Config_LoaderInterface
{
    ...
    /**
     * @var Magento_Core_Model_Config_Modules_SortedFactory
     */
     ...
     * @param Magento_Core_Model_Config_Modules_SortedFactory $sortedFactory
    ...
    public function __construct(
        ...
        Magento_Core_Model_Config_Modules_SortedFactory $sortedFactory,
     ...
     *
     * @param Magento_Core_Model_Config_Base $mergeToConfig
     */
    protected function _loadDeclaredModules(Magento_Core_Model_Config_Base $mergeToConfig)
    {
        ...
        $unsortedConfig = new Magento_Core_Model_Config_Base('<config/>');
        $emptyConfig = new Magento_Core_Model_Config_Element('<config><modules/></config>');
        $declaredModules = array();
        foreach ($moduleFiles as $oneConfigFile) {
            $path = explode(DIRECTORY_SEPARATOR, $oneConfigFile);
            $moduleConfig = new Magento_Core_Model_Config_Base($oneConfigFile);
        ...
        foreach ($declaredModules as $moduleName => $module) {
            if ($module['active'] == 'true') {
                ...
                $unsortedConfig->extend(new Magento_Core_Model_Config_Base($module['module']));
            }
        }
        $params = array(
    ...
    {
        $codeDir = $this->_dirs->getDir(Magento_Core_Model_Dir::MODULES);
        ...
        $etcDir = $this->_dirs->getDir(Magento_Core_Model_Dir::CONFIG);
