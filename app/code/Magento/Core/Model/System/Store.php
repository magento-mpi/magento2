...
class Magento_Core_Model_System_Store extends Magento_Object
{
    ...
    public function getStoreValuesForForm($empty = false, $all = false)
    {
        ...
        if ($all && $this->_isAdminScopeAllowed) {
            $options[] = array(
                'label' => Mage::helper('Magento_Core_Helper_Data')->__('All Store Views'),
                'value' => 0
            );
        }
    ...
    public function getWebsiteValuesForForm($empty = false, $all = false)
    {
        ...
        if ($empty) {
            $options[] = array(
                'label' => Mage::helper('Magento_Core_Helper_Data')->__('-- Please Select --'),
                'value' => ''
            );
        }
        if ($all && $this->_isAdminScopeAllowed) {
            $options[] = array(
                'label' => Mage::helper('Magento_Core_Helper_Data')->__('Admin'),
                'value' => 0
            );
        }
