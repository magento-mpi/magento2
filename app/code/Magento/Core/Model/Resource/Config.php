...
class Magento_Core_Model_Resource_Config extends Magento_Core_Model_Resource_Db_Abstract
{
     ...
     *
     * @param Magento_Core_Model_Config_Base $xmlConfig
     * @param string $condition
     * @return Magento_Core_Model_Resource_Config
     */
    public function loadToXml(Magento_Core_Model_Config_Base $xmlConfig, $condition = null)
    {
        ...
        foreach ($rowset as $s) {
            ...
            $websites[$s['website_id']][Magento_Core_Model_Config::SCOPE_STORES][$s['store_id']] = $s['code'];
        }
        ...
        foreach ($rowset as $r) {
            if ($r['scope'] !== Magento_Core_Model_Store::DEFAULT_CODE) {
                continue;
            }
        ...
        foreach ($rowset as $r) {
            if ($r['scope'] !== Magento_Core_Model_Config::SCOPE_WEBSITES) {
                continue;
            }
            $value = str_replace($substFrom, $substTo, $r['value']);
        ...
        foreach ($websites as $website) {
            ...
            if (isset($website[Magento_Core_Model_Config::SCOPE_STORES])) {
                foreach ($website[Magento_Core_Model_Config::SCOPE_STORES] as $sCode) {
        ...
        foreach ($rowset as $r) {
            if ($r['scope'] !== Magento_Core_Model_Config::SCOPE_STORES) {
                continue;
            }
            $value = str_replace($substFrom, $substTo, $r['value']);
