 ...
 */
abstract class Magento_Core_Model_Resource_Db_Abstract extends Magento_Core_Model_Resource_Abstract
{
    /**
    ...
    public function getIdFieldName()
    {
        if (empty($this->_idFieldName)) {
            Mage::throwException(Mage::helper('Magento_Core_Helper_Data')->__('Empty identifier field name'));
        }
    ...
    public function getMainTable()
    {
        if (empty($this->_mainTable)) {
            Mage::throwException(Mage::helper('Magento_Core_Helper_Data')->__('Empty main table name'));
        }
     ...
     * @return Magento_Core_Model_Resource_Db_Abstract
     */
    public function delete(Magento_Core_Model_Abstract $object)
    {
     ...
     * @param Magento_Core_Model_Abstract $object
     */
    public function unserializeFields(Magento_Core_Model_Abstract $object)
    {
    ...
    protected function _checkUnique(Magento_Core_Model_Abstract $object)
    {
        ...
        if (!empty($existent)) {
            if (count($existent) == 1 ) {
                $error = Mage::helper('Magento_Core_Helper_Data')->__('%s already exists.', $existent[0]);
            } else {
                $error = Mage::helper('Magento_Core_Helper_Data')->__('%s already exist.', implode(', ', $existent));
            }
     ...
     * @param Magento_Core_Model_Abstract $object
     */
    public function afterLoad(Magento_Core_Model_Abstract $object)
    {
        $this->_afterLoad($object);
    }
     ...
     * @param Magento_Core_Model_Abstract $object
     */
    protected function _serializeFields(Magento_Core_Model_Abstract $object)
    {
        foreach ($this->_serializableFields as $field => $parameters) {
            list($serializeDefault, $unserializeDefault) = $parameters;
