...
abstract class Magento_Core_Model_Abstract extends Magento_Object
{
    ...
    protected function _getResource()
    {
        if (empty($this->_resourceName) && empty($this->_resource)) {
            Mage::throwException(Mage::helper('Magento_Core_Helper_Data')->__('Resource is not set.'));
        }
    ...
    public function getResourceCollection()
    {
        if (empty($this->_resourceCollection) && empty($this->_collectionName)) {
            Mage::throwException(
                Mage::helper('Magento_Core_Helper_Data')->__('Model collection resource name is not defined.')
            );
        }
    ...
    protected function _validateBeforeSave()
    {
        ...
        if ($validator && !$validator->isValid($this)) {
            ...
            $exception = new Magento_Core_Exception(implode(PHP_EOL, $errors));
            foreach ($errors as $errorMessage) {
                $exception->addMessage(new Magento_Core_Model_Message_Error($errorMessage));
            }
            throw $exception;
        }
        return $this;
