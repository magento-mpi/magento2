<?php
/**
 *
 * {license_notice}
 *
 * @copyright   {copyright}
 * @license     {license_link}
 */
namespace Magento\Cms\Controller\Adminhtml\Page;

/**
 * Class MassDelete
 */
class MassDelete extends \Magento\Backend\App\Action
{
    /**
     * Execute action
     *
     * @return void
     */
    public function execute()
    {
        $data = $this->getRequest()->getParam('massaction', '[]');
        $data = json_decode($data, true);

        try {
            if (isset($data['all_selected']) && $data['all_selected'] === true) {
                if (!empty($data['excluded'])) {
                    $this->excludedDelete($data['excluded']);
                } else {
                    $this->deleteAll();
                }
            } elseif (!empty($data['selected'])) {
                $this->selectedDelete($data['selected']);
            } else {
                $this->setErrorResponses();
            }
        } catch (\Exception $e) {
            $this->messageManager->addError($e->getMessage());
        }

        $this->_redirect('cms/*/index');
    }

    /**
     * Delete all
     *
     * @return void
     * @throws \Exception
     */
    protected function deleteAll()
    {
        /** @var \Magento\Cms\Model\Resource\Page\Collection $collection */
        $collection = $this->_objectManager->get('Magento\Cms\Model\Resource\Page\Collection');
        $count = 0;
        foreach ($collection->getAllIds() as $id) {
            /** @var \Magento\Cms\Model\Page $model */
            $model = $this->_objectManager->get('Magento\Cms\Model\Page');
            $model->load($id);
            $model->delete();
            ++$count;
        }
        $this->setSuccessResponses($count);
    }

    /**
     * Delete all but the not selected
     *
     * @param array $excluded
     * @throws \Exception
     */
    protected function excludedDelete(array $excluded)
    {
        /** @var \Magento\Cms\Model\Resource\Page\Collection $collection */
        $collection = $this->_objectManager->get('Magento\Cms\Model\Resource\Page\Collection');
        $collection->addFieldToFilter('page_id', ['nin' => $excluded]);
        $count = 0;
        foreach ($collection->getAllIds() as $id) {
            /** @var \Magento\Cms\Model\Page $model */
            $model = $this->_objectManager->get('Magento\Cms\Model\Page');
            $model->load($id);
            $model->delete();
            ++$count;
        }
        $this->setSuccessResponses($count);
    }

    /**
     * Delete selected items
     *
     * @param array $selected
     * @throws \Exception
     */
    protected function selectedDelete(array $selected)
    {
        /** @var \Magento\Cms\Model\Resource\Page\Collection $collection */
        $collection = $this->_objectManager->get('Magento\Cms\Model\Resource\Page\Collection');
        $collection->addFieldToFilter('page_id', ['in' => $selected]);
        $count = 0;
        foreach ($collection->getAllIds() as $id) {
            /** @var \Magento\Cms\Model\Page $model */
            $model = $this->_objectManager->get('Magento\Cms\Model\Page');
            $model->load($id);
            $model->delete();
            ++$count;
        }
        $this->setSuccessResponses($count);
    }

    /**
     * Set error message and redirect back
     *
     * @return void
     */
    protected function setErrorResponses()
    {
        $this->messageManager->addError(__('Please select item(s).'));
        $this->_redirect('cms/*/index');
    }

    /**
     * Set error messages
     *
     * @param int
     * @return void
     */
    protected function setSuccessResponses($count)
    {
        $this->messageManager->addSuccess(__('A total of %1 record(s) have been deleted.', $count));
    }
}
