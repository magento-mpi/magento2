...
use Magento\CatalogUrlRewrite\Model\Product\UrlGenerator;
    ...
    public function getCollection($storeId)
    {
        ...
        $this->_select = $this->_getWriteAdapter()->select()
            ...
            ->joinLeft(
                ...
                'url_rewrite.entity_id = e.entity_id',
                ['url' => 'request_path']
            )
            ->where('url_rewrite.is_autogenerated = 1')
            ->where('url_rewrite.store_id = ' . (int)$store->getId())
            ->where('url_rewrite.entity_type = ?', UrlGenerator::ENTITY_TYPE_PRODUCT)
            ->where(
                'w.website_id = ?',
                $store->getWebsiteId()
            );
