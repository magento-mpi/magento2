...
class Magento_Checkout_Model_Cart extends Magento_Object implements Magento_Checkout_Model_Cart_Interface
{
    ...
    protected function _getProduct($productInfo)
    {
        ...
        if (!$product
            ...
            Mage::throwException(Mage::helper('Magento_Checkout_Helper_Data')->__('We can\'t find the product.'));
        }
        return $product;
    }
        ...
        if (!empty($productIds)) {
            ...
            if (!$allAvailable) {
                $this->getCheckoutSession()->addError(
                    Mage::helper('Magento_Checkout_Helper_Data')->__("We don't have some of the products you want.")
                );
            }
            if (!$allAdded) {
                $this->getCheckoutSession()->addError(
                    Mage::helper('Magento_Checkout_Helper_Data')->__("We don't have as many of some products as you want.")
                );
            }
        }
        return $this;
    ...
    public function updateItems($data)
    {
        ...
        foreach ($data as $itemId => $itemInfo) {
            ...
            if ($qty > 0) {
                ...
                if (isset($itemInfo['before_suggest_qty']) && ($itemInfo['before_suggest_qty'] != $qty)) {
                    ...
                    $message = $messageFactory->notice(Mage::helper('Magento_Checkout_Helper_Data')->__('Quantity was recalculated from %d to %d', $itemInfo['before_suggest_qty'], $qty));
        ...
        if ($qtyRecalculatedFlag) {
            $session->addNotice(
                Mage::helper('Magento_Checkout_Helper_Data')->__('Some products quantities were recalculated because of quantity increment mismatch.')
            );
        }
    ...
    public function updateItem($itemId, $requestInfo = null, $updatingParams = null)
    {
        try {
            $item = $this->getQuote()->getItemById($itemId);
            if (!$item) {
                Mage::throwException(Mage::helper('Magento_Checkout_Helper_Data')->__('This quote item does not exist.'));
            }
