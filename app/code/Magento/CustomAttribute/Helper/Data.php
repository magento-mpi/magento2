...
class Magento_CustomAttribute_Helper_Data extends Magento_Core_Helper_Abstract
{
    ...
    {
        Mage::throwException(Mage::helper('Magento_CustomAttribute_Helper_Data')->__('Use helper with defined EAV entity.'));
    }
    ...
    public function getAttributeFormOptions()
    {
        return array(
            array(
                'label' => Mage::helper('Magento_CustomAttribute_Helper_Data')->__('Default EAV Form'),
                'value' => 'default'
            )
        );
    }
            ...
            case 'multiline':
                if (isset($validateRules['min_text_length']) && isset($validateRules['max_text_length'])) {
                    ...
                    if ($minTextLength > $maxTextLength) {
                        $errors[] = Mage::helper('Magento_CustomAttribute_Helper_Data')->__('Please correct the values for minimum and maximum text length validation rules.');
                    }
                }
                break;
            case 'date':
                if (isset($validateRules['date_range_min']) && isset($validateRules['date_range_max'])) {
                    ...
                    if ($minValue > $maxValue) {
                        $errors[] = Mage::helper('Magento_CustomAttribute_Helper_Data')->__('Please correct the values for minimum and maximum date validation rules.');
                    }
                }
                break;
            default:
    ...
    public function getAttributeInputTypes($inputType = null)
    {
        $inputTypes = array(
            'text'          => array(
                'label'             => Mage::helper('Magento_CustomAttribute_Helper_Data')->__('Text Field'),
            ...
            'textarea'      => array(
                'label'             => Mage::helper('Magento_CustomAttribute_Helper_Data')->__('Text Area'),
                'manage_options'    => false,
                'validate_types'    => array(
                    'min_text_length',
                    'max_text_length',
            ...
            'multiline'     => array(
                'label'             => Mage::helper('Magento_CustomAttribute_Helper_Data')->__('Multiple Line'),
                'manage_options'    => false,
                'validate_types'    => array(
                    'min_text_length',
                    'max_text_length',
            ...
            'date'          => array(
                'label'             => Mage::helper('Magento_CustomAttribute_Helper_Data')->__('Date'),
                'manage_options'    => false,
                'validate_types'    => array(
                    'date_range_min',
                    'date_range_max'
            ...
            'select'        => array(
                'label'             => Mage::helper('Magento_CustomAttribute_Helper_Data')->__('Dropdown'),
                'manage_options'    => true,
            ...
            'multiselect'   => array(
                'label'             => Mage::helper('Magento_CustomAttribute_Helper_Data')->__('Multiple Select'),
                'manage_options'    => true,
            ...
            'boolean'       => array(
                'label'             => Mage::helper('Magento_CustomAttribute_Helper_Data')->__('Yes/No'),
                'manage_options'    => false,
                'validate_types'    => array(),
                'validate_filters'  => array(),
                'filter_types'      => array(),
            ...
            'file'          => array(
                'label'             => Mage::helper('Magento_CustomAttribute_Helper_Data')->__('File (attachment)'),
                'manage_options'    => false,
                'validate_types'    => array(
                    'max_file_size',
                    'file_extensions'
            ...
            'image'         => array(
                'label'             => Mage::helper('Magento_CustomAttribute_Helper_Data')->__('Image File'),
                'manage_options'    => false,
                'validate_types'    => array(
                    'max_file_size',
                    'max_image_width',
    ...
    {
        return array(
            'alphanumeric'  => Mage::helper('Magento_CustomAttribute_Helper_Data')->__('Alphanumeric'),
            'numeric'       => Mage::helper('Magento_CustomAttribute_Helper_Data')->__('Numeric Only'),
            'alpha'         => Mage::helper('Magento_CustomAttribute_Helper_Data')->__('Alpha Only'),
            'url'           => Mage::helper('Magento_CustomAttribute_Helper_Data')->__('URL'),
            'email'         => Mage::helper('Magento_CustomAttribute_Helper_Data')->__('Email'),
            'date'          => Mage::helper('Magento_CustomAttribute_Helper_Data')->__('Date'),
        );
    }
    ...
    {
        return array(
            'striptags'     => Mage::helper('Magento_CustomAttribute_Helper_Data')->__('Strip HTML Tags'),
            'escapehtml'    => Mage::helper('Magento_CustomAttribute_Helper_Data')->__('Escape HTML Entities'),
            'date'          => Mage::helper('Magento_CustomAttribute_Helper_Data')->__('Normalize Date')
        );
    }
        ...
        if ($data) {
            ...
            if (isset($data['attribute_code'])) {
                ...
                if (!$validatorAttrCode->isValid($data['attribute_code'])) {
                    Mage::throwException(Mage::helper('Magento_CustomAttribute_Helper_Data')->__('The attribute code is invalid. Please use only letters (a-z), numbers (0-9) or underscores (_) in this field. The first character should be a letter.'));
                }
            }
        }
        return $data;
