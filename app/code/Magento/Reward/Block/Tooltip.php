...
class Magento_Reward_Block_Tooltip extends Magento_Core_Block_Template
{
    ...
    {
        $anchorText = null === $anchorText ? Mage::helper('Magento_Reward_Helper_Data')->__('Learn more...') : $anchorText;
        ...
        if ($this->_actionInstance) {
            ...
            if ($this->_rewardInstance->getId()) {
                ...
                if ($this->hasIsGuestNote() && !$this->hasGuestNote()) {
                    $this->setGuestNote(Mage::helper('Magento_Reward_Helper_Data')->__('This applies only to registered users and may vary when a user is logged in.'));
                }
            }
        }
    }
