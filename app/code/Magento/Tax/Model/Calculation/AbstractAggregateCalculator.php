...
abstract class AbstractAggregateCalculator extends AbstractCalculator
{
    ...
    protected function calculateWithTaxInPrice(QuoteDetailsItem $item, $quantity)
    {
        ...
        if ($discountAmount && $applyTaxAfterDiscount) {
            $rowTax = $this->roundAmount($rowTaxExact, $rate, true, self::KEY_TAX_BEFORE_DISCOUNT_DELTA_ROUNDING);
        } else {
    ...
    protected function calculateWithTaxNotInPrice(QuoteDetailsItem $item, $quantity)
    {
        ...
        foreach ($appliedRates as $appliedRate) {
            ...
            if ($discountAmount && $applyTaxAfterDiscount) {
                $rowTaxPerRate = $this->roundAmount(
                    $rowTaxPerRate,
                    $rate,
                    false,
                    self::KEY_TAX_BEFORE_DISCOUNT_DELTA_ROUNDING
                );
            } else {
                $rowTaxPerRate = $this->roundAmount($rowTaxPerRate, $rate, false);
            }
            ...
            if ($discountAmount && $applyTaxAfterDiscount) {
                ...
                $rowTaxAfterDiscount = $this->roundAmount($rowTaxAfterDiscount, $taxRate, false);
            }
