...
class Magento_CustomerBalance_Model_Balance extends Magento_Core_Model_Abstract
{
    ...
    public function loadByCustomer()
    {
        ...
        if ($this->hasWebsiteId()) {
            ...
            if (Mage::app()->getStore()->isAdmin()) {
                Mage::throwException(Mage::helper('Magento_CustomerBalance_Helper_Data')->__('A website ID must be set.'));
            }
    ...
    public function setNotifyByEmail($shouldNotify, $storeId = null)
    {
        ...
        if ($shouldNotify) {
            if (null === $storeId) {
                Mage::throwException(Mage::helper('Magento_CustomerBalance_Helper_Data')->__('Please also set the Store ID.'));
            }
    ...
    protected function _beforeSave()
    {
        ...
        if ($this->getNotifyByEmail() && !$this->hasStoreId()) {
            Mage::throwException(Mage::helper('Magento_CustomerBalance_Helper_Data')->__('The Store ID must be set to send email notifications.'));
        }
    ...
    protected function _ensureCustomer()
    {
        ...
        if (!$this->getCustomerId()) {
            Mage::throwException(Mage::helper('Magento_CustomerBalance_Helper_Data')->__('A customer ID must be specified.'));
        }
        if (!$this->getCustomer()) {
            $this->setCustomer(Mage::getModel('Magento_Customer_Model_Customer')->load($this->getCustomerId()));
        }
        if (!$this->getCustomer()->getId()) {
            Mage::throwException(Mage::helper('Magento_CustomerBalance_Helper_Data')->__('This customer is not set or does not exist.'));
        }
    }
