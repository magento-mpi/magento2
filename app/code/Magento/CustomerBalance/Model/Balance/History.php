...
class Magento_CustomerBalance_Model_Balance_History extends Magento_Core_Model_Abstract
{
    ...
    {
        return array(
            self::ACTION_CREATED  => Mage::helper('Magento_CustomerBalance_Helper_Data')->__('Created'),
            self::ACTION_UPDATED  => Mage::helper('Magento_CustomerBalance_Helper_Data')->__('Updated'),
            self::ACTION_USED     => Mage::helper('Magento_CustomerBalance_Helper_Data')->__('Used'),
            self::ACTION_REFUNDED => Mage::helper('Magento_CustomerBalance_Helper_Data')->__('Refunded'),
            self::ACTION_REVERTED => Mage::helper('Magento_CustomerBalance_Helper_Data')->__('Reverted'),
        );
    }
    ...
    protected function _beforeSave()
    {
        ...
        if ((!$balance) || !$balance->getId()) {
            Mage::throwException(Mage::helper('Magento_CustomerBalance_Helper_Data')->__('You need a balance to save your balance history.'));
        }
        ...
        switch ((int)$balance->getHistoryAction())
        {
                    ...
                    if (Mage::getSingleton('Magento_Core_Model_StoreManagerInterface')->getStore()->isAdmin()
                        ...
                        if ($user->getUsername()) {
                            if (!trim($balance->getComment())){
                                $this->setAdditionalInfo(Mage::helper('Magento_CustomerBalance_Helper_Data')->__('By admin: %s.', $user->getUsername()));
                            }else{
                                $this->setAdditionalInfo(Mage::helper('Magento_CustomerBalance_Helper_Data')->__('By admin: %1$s. (%2$s)', $user->getUsername(), $balance->getComment()));
                            }
                        }
                    }
                } else {
            ...
            case self::ACTION_USED:
                ...
                $this->setAdditionalInfo(Mage::helper('Magento_CustomerBalance_Helper_Data')->__('Order #%s', $balance->getOrder()->getIncrementId()));
                break;
            case self::ACTION_REFUNDED:
                $this->_checkBalanceModelOrder($balance);
                if ((!$balance->getCreditMemo()) || !$balance->getCreditMemo()->getIncrementId()) {
                    Mage::throwException(Mage::helper('Magento_CustomerBalance_Helper_Data')->__('There is no credit memo set to balance model.'));
                }
                $this->setAdditionalInfo(
                    Mage::helper('Magento_CustomerBalance_Helper_Data')->__('Order #%s, creditmemo #%s', $balance->getOrder()->getIncrementId(), $balance->getCreditMemo()->getIncrementId())
                );
                break;
            case self::ACTION_REVERTED:
                ...
                break;
            default:
                Mage::throwException(Mage::helper('Magento_CustomerBalance_Helper_Data')->__('Unknown balance history action code'));
                // break intentionally omitted
        }
    ...
    protected function _checkBalanceModelOrder($model)
    {
        if ((!$model->getOrder()) || !$model->getOrder()->getIncrementId()) {
            Mage::throwException(Mage::helper('Magento_CustomerBalance_Helper_Data')->__('There is no order set to balance model.'));
        }
    }
