...
class Magento_TargetRule_Controller_Adminhtml_Targetrule extends Magento_Adminhtml_Controller_Action
{
    ...
    public function editAction()
    {
        ...
        if ($ruleId) {
            $model->load($ruleId);
            if (!$model->getId()) {
                $this->_getSession()->addError(Mage::helper('Magento_TargetRule_Helper_Data')->__('This rule no longer exists.'));
                $this->_redirect('*/*');
                return;
            }
        }
            ...
            try {
                ...
                if ($ruleId) {
                    ...
                    if ($ruleId != $model->getId()) {
                        Mage::throwException(Mage::helper('Magento_TargetRule_Helper_Data')->__('Please specify a correct rule.'));
                    }
                }
                ...
                $this->_getSession()->addSuccess(
                    Mage::helper('Magento_TargetRule_Helper_Data')->__('You saved the rule.')
                );
                ...
                $this->_getSession()->addError(Mage::helper('Magento_TargetRule_Helper_Data')->__('Invalid date.'));
                $hasError = true;
            } catch (Exception $e) {
                $this->_getSession()->addException($e,
                    Mage::helper('Magento_TargetRule_Helper_Data')->__('An error occurred while saving Product Rule.')
                );
    ...
    public function deleteAction()
    {
        if ($id = $this->getRequest()->getParam('id')) {
            try {
                ...
                Mage::getSingleton('Magento_Adminhtml_Model_Session')
                    ->addSuccess(Mage::helper('Magento_TargetRule_Helper_Data')->__('You deleted the rule.'));
                $this->_redirect('*/*/');
                return;
            }
            catch (Exception $e) {
        ...
        Mage::getSingleton('Magento_Adminhtml_Model_Session')
            ->addError(Mage::helper('Magento_TargetRule_Helper_Data')->__("We can't find a page to delete."));
        $this->_redirect('*/*/');
    }
