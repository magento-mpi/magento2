...
class Magento_Adminhtml_Controller_System_Variable extends Magento_Adminhtml_Controller_Action
{
    ...
    protected function _initLayout()
    {
        $this->loadLayout()
            ...
            ->_addBreadcrumb(Mage::helper('Magento_Adminhtml_Helper_Data')->__('Custom Variables'), Mage::helper('Magento_Adminhtml_Helper_Data')->__('Custom Variables'));
        return $this;
    }
        ...
        if ($data) {
            ...
            try {
                ...
                $this->_getSession()->addSuccess(
                    Mage::helper('Magento_Adminhtml_Helper_Data')->__('You saved the custom variable.')
                );
                if ($back) {
    ...
    public function deleteAction()
    {
        ...
        if ($variable->getId()) {
            try {
                $variable->delete();
                $this->_getSession()->addSuccess(
                    Mage::helper('Magento_Adminhtml_Helper_Data')->__('You deleted the customer.')
                );
            } catch (Exception $e) {
                $this->_getSession()->addError($e->getMessage());
                $this->_redirect('*/*/edit', array('_current' => true, ));
