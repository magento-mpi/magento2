...
class Magento_Adminhtml_Block_Rating_Edit_Tab_Options extends Magento_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        $fieldset = $form->addFieldset('options_form', array('legend'=>Mage::helper('Magento_Rating_Helper_Data')->__('Assigned Options')));
        ...
        if( Mage::registry('rating_data') ) {
            ...
            foreach( $collection->getItems() as $item ) {
                $fieldset->addField('option_code_' . $item->getId() , 'text', array(
                                        'label'     => Mage::helper('Magento_Rating_Helper_Data')->__('Option Label'),
                                        'required'  => true,
            ...
            for( $i=1;$i<=5;$i++ ) {
                $fieldset->addField('option_code_' . $i, 'text', array(
                                        'label'     => Mage::helper('Magento_Rating_Helper_Data')->__('Option Title'),
                                        'required'  => true,
