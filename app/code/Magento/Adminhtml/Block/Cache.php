...
class Magento_Adminhtml_Block_Cache extends Magento_Adminhtml_Block_Widget_Grid_Container
{
    ...
    protected function _construct()
    {
        ...
        $this->_headerText = Mage::helper('Magento_Core_Helper_Data')->__('Cache Storage Management');
        parent::_construct();
        $this->_removeButton('add');
        $this->_addButton('flush_magento', array(
            'label'     => Mage::helper('Magento_Core_Helper_Data')->__('Flush Magento Cache'),
            'onclick'   => 'setLocation(\'' . $this->getFlushSystemUrl() .'\')',
            'class'     => 'delete',
        ));

        $message = Mage::helper('Magento_Core_Helper_Data')->__('Cache storage may contain additional data. Are you sure that you want flush it?');
        $this->_addButton('flush_system', array(
            'label'     => Mage::helper('Magento_Core_Helper_Data')->__('Flush Cache Storage'),
