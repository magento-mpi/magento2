    ...
    public function render(Magento_Data_Form_Element_Abstract $element)
    {
        ...
        foreach ($attributes as $key => $attribute) {
            ...
            $field->setName("groups[watermark][fields][{$key}_size][value]")
                ...
                ->setLabel(Mage::helper('Magento_Adminhtml_Helper_Data')->__('Size for %s', $attribute['title']))
            ...
            $field->setName("groups[watermark][fields][{$key}_image][value]")
                ->setForm( $this->getForm() )
                ->setLabel(Mage::helper('Magento_Adminhtml_Helper_Data')->__('Watermark File for %s', $attribute['title']))
                ->setRenderer($renderer);
            $html.= $field->toHtml();
            ...
            $field->setName("groups[watermark][fields][{$key}_position][value]")
                ->setForm( $this->getForm() )
                ->setLabel(Mage::helper('Magento_Adminhtml_Helper_Data')->__('Position of Watermark for %s', $attribute['title']))
                ->setRenderer($renderer)
