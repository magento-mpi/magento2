...
class Magento_Invitation_Block_Adminhtml_Invitation_View extends Magento_Adminhtml_Block_Widget_Container
{
    ...
    protected function _prepareLayout()
    {
        ...
        $this->_headerText = Mage::helper('Magento_Invitation_Helper_Data')->__('View Invitation for %s (ID: %s)', $invitation->getEmail(), $invitation->getId());
        $this->getLayout()->getBlock('page-title')->setPageTitle($this->_headerText);
        $this->_addButton('back', array(
            'label' => Mage::helper('Magento_Invitation_Helper_Data')->__('Back'),
        ...
        if ($invitation->canBeCanceled()) {
            ...
            $this->_addButton('cancel', array(
                'label' => Mage::helper('Magento_Invitation_Helper_Data')->__('Discard Invitation'),
                'onclick' => 'deleteConfirm(\''. $this->jsQuoteEscape(
                            Mage::helper('Magento_Invitation_Helper_Data')->__('Are you sure you want to discard this invitation?')
                        ) . '\', \'' . $massCancelUrl . '\' )',
                'class' => 'cancel'
            ), -1);
        }
        if ($invitation->canMessageBeUpdated()) {
            $this->_addButton('save_message_button', array(
                'label'   => $this->helper('Magento_Invitation_Helper_Data')->__('Save Invitation'),
        ...
        if ($invitation->canBeSent()) {
            ...
            $this->_addButton('resend', array(
                'label' => Mage::helper('Magento_Invitation_Helper_Data')->__('Send Invitation'),
