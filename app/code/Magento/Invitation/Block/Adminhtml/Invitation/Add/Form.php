...
class Magento_Invitation_Block_Adminhtml_Invitation_Add_Form extends Magento_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        $fieldset = $form->addFieldset('base_fieldset', array(
            'legend' => $this->helper('Magento_Invitation_Helper_Data')->__('Invitations Information'),
            'class' => 'fieldset-wide'
        ));

        $fieldset->addField('email', 'textarea', array(
            'label' => $this->helper('Magento_Invitation_Helper_Data')->__('Enter Each Email on New Line'),
            'required' => true,
        ...
        $fieldset->addField('message', 'textarea', array(
            'label' => $this->helper('Magento_Invitation_Helper_Data')->__('Message'),
            'name' => 'message'
        ));

        if (!Mage::app()->isSingleStoreMode()) {
            $field = $fieldset->addField('store_id', 'select', array(
                'label' => $this->helper('Magento_Invitation_Helper_Data')->__('Send From'),
                'required' => true,
                'name' => 'store_id',
        ...
        $fieldset->addField('group_id', 'select', array(
            'label' => $this->helper('Magento_Invitation_Helper_Data')->__('Invitee Group'),
            'required' => true,
            'name' => 'group_id',
            'values' => $groups
        ));
