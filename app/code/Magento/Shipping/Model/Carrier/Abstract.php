...
abstract class Magento_Shipping_Model_Carrier_Abstract extends Magento_Object
{
     ...
     */
    abstract public function collectRates(Magento_Shipping_Model_Rate_Request $request);
        ...
        if ($speCountriesAllow && $speCountriesAllow == 1){
             ...
             if ($availableCountries && in_array($request->getDestCountryId(), $availableCountries)) {
                 ...
                 && !in_array($request->getDestCountryId(), $availableCountries)))
                   ...
                   $error->setErrorMessage($errorMsg ? $errorMsg : Mage::helper('Magento_Shipping_Helper_Data')->__('Sorry, but we can\'t deliver to the destination country with this shipping module.'));
                   return $error;
             } else {
                 /*
    ...
    protected function _updateFreeMethodQuote($request)
    {
        ...
        if ($request->getFreeMethodWeight() > 0) {
            ...
            if ($result && ($rates = $result->getAllRates()) && count($rates)>0) {
                if ((count($rates) == 1) && ($rates[0] instanceof Magento_Shipping_Model_Rate_Result_Method)) {
                    $price = $rates[0]->getPrice();
                }
                if (count($rates) > 1) {
                    foreach ($rates as $rate) {
                        if ($rate instanceof Magento_Shipping_Model_Rate_Result_Method
