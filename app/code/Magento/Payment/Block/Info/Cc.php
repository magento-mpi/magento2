...
class Magento_Payment_Block_Info_Cc extends Magento_Payment_Block_Info
{
    ...
    public function getCcTypeName()
    {
        ...
        return (empty($ccType)) ? Mage::helper('Magento_Payment_Helper_Data')->__('N/A') : $ccType;
    }
    ...
    protected function _prepareSpecificInformation($transport = null)
    {
        ...
        if ($ccType = $this->getCcTypeName()) {
            $data[Mage::helper('Magento_Payment_Helper_Data')->__('Credit Card Type')] = $ccType;
        }
        if ($this->getInfo()->getCcLast4()) {
            $data[Mage::helper('Magento_Payment_Helper_Data')->__('Credit Card Number')] = sprintf('xxxx-%s', $this->getInfo()->getCcLast4());
        }
        if (!$this->getIsSecureMode()) {
            if ($ccSsIssue = $this->getInfo()->getCcSsIssue()) {
                $data[Mage::helper('Magento_Payment_Helper_Data')->__('Switch/Solo/Maestro Issue Number')] = $ccSsIssue;
            }
            $year = $this->getInfo()->getCcSsStartYear();
            $month = $this->getInfo()->getCcSsStartMonth();
            if ($year && $month) {
                $data[Mage::helper('Magento_Payment_Helper_Data')->__('Switch/Solo/Maestro Start Date')] =  $this->_formatCardDate($year, $month);
            }
        }
