...
class Magento_Payment_Model_Recurring_Profile extends Magento_Core_Model_Abstract
{
    ...
    public function isValid()
    {
        ...
        if (!$this->getStartDatetime()) {
            $this->_errors['start_datetime'][] = Mage::helper('Magento_Payment_Helper_Data')->__('The start date is undefined.');
        } elseif (!Zend_Date::isDate($this->getStartDatetime(), Magento_Date::DATETIME_INTERNAL_FORMAT)) {
            $this->_errors['start_datetime'][] = Mage::helper('Magento_Payment_Helper_Data')->__('The start date has an invalid format.');
        }
        if (!$this->getScheduleDescription()) {
            $this->_errors['schedule_description'][] = Mage::helper('Magento_Payment_Helper_Data')->__('The schedule description must be provided.');
        }

        // period unit and frequency
        if (!$this->getPeriodUnit() || !in_array($this->getPeriodUnit(), $this->getAllPeriodUnits(false), true)) {
            $this->_errors['period_unit'][] = Mage::helper('Magento_Payment_Helper_Data')->__('The billing period unit is not defined or wrong.');
        }
        if ($this->getPeriodFrequency() && !$this->_validatePeriodFrequency('period_unit', 'period_frequency')) {
            $this->_errors['period_frequency'][] = Mage::helper('Magento_Payment_Helper_Data')->__('The period frequency is wrong.');;
        }
        ...
        if ($this->getTrialPeriodUnit()) {
            if (!in_array($this->getTrialPeriodUnit(), $this->getAllPeriodUnits(false), true)) {
                $this->_errors['trial_period_unit'][] = Mage::helper('Magento_Payment_Helper_Data')->__('The trial billing period unit is wrong.');
            }
            if (!$this->getTrialPeriodFrequency() || !$this->_validatePeriodFrequency('trial_period_unit', 'trial_period_frequency')) {
                $this->_errors['trial_period_frequency'][] = Mage::helper('Magento_Payment_Helper_Data')->__('The trial period frequency is wrong.');
            }
            if (!$this->getTrialPeriodMaxCycles()) {
                $this->_errors['trial_period_max_cycles'][] = Mage::helper('Magento_Payment_Helper_Data')->__('The trial period max cycles is wrong.');
            }
            if (!$this->getTrialBillingAmount()) {
                $this->_errors['trial_billing_amount'][] = Mage::helper('Magento_Payment_Helper_Data')->__('The trial billing amount is wrong.');
            }
        }

        // billing and other amounts
        if (!$this->getBillingAmount() || 0 >= $this->getBillingAmount()) {
            $this->_errors['billing_amount'][] = Mage::helper('Magento_Payment_Helper_Data')->__('We found a wrong or empty billing amount specified.');
        }
        foreach (array('trial_billing_abount', 'shipping_amount', 'tax_amount', 'init_amount') as $key) {
            if ($this->hasData($key) && 0 >= $this->getData($key)) {
                $this->_errors[$key][] = Mage::helper('Magento_Payment_Helper_Data')->__('The wrong %s is specified.', $this->getFieldLabel($key));
            }
        }

        // currency code
        if (!$this->getCurrencyCode()) {
            $this->_errors['currency_code'][] = Mage::helper('Magento_Payment_Helper_Data')->__('The currency code is undefined.');
        }

        // payment method
        if (!$this->_methodInstance || !$this->getMethodCode()) {
            $this->_errors['method_code'][] = Mage::helper('Magento_Payment_Helper_Data')->__('The payment method code is undefined.');
        }
        ...
        if ($isGrouped && $this->_errors) {
            ...
            if ($asMessage) {
                return Mage::throwException(
                    Mage::helper('Magento_Payment_Helper_Data')->__("The payment profile is invalid:\n%s.", implode("\n", $result))
                );
            }
            return $result;
        }
    ...
    public function importBuyRequest(Magento_Object $buyRequest)
    {
        ...
        if ($startDate) {
            ...
            if (!Zend_Date::isDate($startDate, $dateFormat, $localeCode)) {
                Mage::throwException(Mage::helper('Magento_Payment_Helper_Data')->__('The recurring profile start date has invalid format.'));
            }
    ...
    public function exportScheduleInfo()
    {
        $result = array(
            new Magento_Object(array(
                'title'    => Mage::helper('Magento_Payment_Helper_Data')->__('Billing Period'),
        ...
        if ($trial) {
            $result[] = new Magento_Object(array(
                'title'    => Mage::helper('Magento_Payment_Helper_Data')->__('Trial Period'),
                'schedule' => $trial,
            ));
        }
        return $result;
    ...
    {
        switch ($unit) {
            case self::PERIOD_UNIT_DAY:  return Mage::helper('Magento_Payment_Helper_Data')->__('Day');
            case self::PERIOD_UNIT_WEEK: return Mage::helper('Magento_Payment_Helper_Data')->__('Week');
            case self::PERIOD_UNIT_SEMI_MONTH: return Mage::helper('Magento_Payment_Helper_Data')->__('Two Weeks');
            case self::PERIOD_UNIT_MONTH: return Mage::helper('Magento_Payment_Helper_Data')->__('Month');
            case self::PERIOD_UNIT_YEAR:  return Mage::helper('Magento_Payment_Helper_Data')->__('Year');
        }
        return $unit;
    }
    ...
    public function getFieldLabel($field)
    {
        switch ($field) {
            case 'subscriber_name':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Subscriber Name');
            case 'start_datetime':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Start Date');
            case 'internal_reference_id':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Internal Reference ID');
            case 'schedule_description':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Schedule Description');
            case 'suspension_threshold':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Maximum Payment Failures');
            case 'bill_failed_later':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Auto Bill on Next Cycle');
            case 'period_unit':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Billing Period Unit');
            case 'period_frequency':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Billing Frequency');
            case 'period_max_cycles':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Maximum Billing Cycles');
            case 'billing_amount':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Billing Amount');
            case 'trial_period_unit':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Trial Billing Period Unit');
            case 'trial_period_frequency':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Trial Billing Frequency');
            case 'trial_period_max_cycles':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Maximum Trial Billing Cycles');
            case 'trial_billing_amount':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Trial Billing Amount');
            case 'currency_code':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Currency');
            case 'shipping_amount':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Shipping Amount');
            case 'tax_amount':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Tax Amount');
            case 'init_amount':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Initial Fee');
            case 'init_may_fail':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Allow Initial Fee Failure');
            case 'method_code':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Payment Method');
            case 'reference_id':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Payment Reference ID');
        }
    }
    ...
    public function getFieldComment($field)
    {
        switch ($field) {
            case 'subscriber_name':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Full name of the person receiving the product or service paid for by the recurring payment.');
            case 'start_datetime':
                return Mage::helper('Magento_Payment_Helper_Data')->__('This is the date when billing for the profile begins.');
            case 'schedule_description':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Enter a short description of the recurring payment. By default, this description will match the product name.');
            case 'suspension_threshold':
                return Mage::helper('Magento_Payment_Helper_Data')->__('This is the number of scheduled payments that can fail before the profile is automatically suspended.');
            case 'bill_failed_later':
                return Mage::helper('Magento_Payment_Helper_Data')->__('Use this to automatically bill the outstanding balance amount in the next billing cycle (if there were failed payments).');
            case 'period_unit':
                return Mage::helper('Magento_Payment_Helper_Data')->__('This is the unit for billing during the subscription period.');
            case 'period_frequency':
                return Mage::helper('Magento_Payment_Helper_Data')->__('This is the number of billing periods that make up one billing cycle.');
            case 'period_max_cycles':
                return Mage::helper('Magento_Payment_Helper_Data')->__('This is the number of billing cycles for the payment period.');
            case 'init_amount':
                return Mage::helper('Magento_Payment_Helper_Data')->__('The initial, non-recurring payment amount is due immediately when the profile is created.');
            case 'init_may_fail':
                return Mage::helper('Magento_Payment_Helper_Data')->__('This sets whether to suspend the payment profile if the initial fee fails or, instead, add it to the outstanding balance.');
        }
    }
    ...
    protected function _validateBeforeSave()
    {
        ...
        if (!$this->getInternalReferenceId()) {
            Mage::throwException(
                Mage::helper('Magento_Payment_Helper_Data')->__('An internal reference ID is required to save the payment profile.')
            );
        }
    }
    ...
    protected function _renderSchedule($periodKey, $frequencyKey, $cyclesKey)
    {
        ...
        $result[] = Mage::helper('Magento_Payment_Helper_Data')->__('%s %s cycle.', $frequency, $this->getPeriodUnitLabel($period));

        $cycles = (int)$this->_getData($cyclesKey);
        if ($cycles) {
            $result[] = Mage::helper('Magento_Payment_Helper_Data')->__('Repeats %s time(s)', $cycles);
        } else {
            $result[] = Mage::helper('Magento_Payment_Helper_Data')->__('Repeats until suspended or canceled.');
        }
        return $result;
    }
}
