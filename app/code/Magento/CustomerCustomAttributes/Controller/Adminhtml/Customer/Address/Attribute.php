    ...
    extends Magento_Adminhtml_Controller_Action
{
    ...
    protected function _initAction()
    {
        $this->loadLayout()
            ...
            ->_addBreadcrumb(
                ...
                Mage::helper('Magento_CustomerCustomAttributes_Helper_Data')->__('Customer'))
            ->_addBreadcrumb(
                Mage::helper('Magento_CustomerCustomAttributes_Helper_Data')->__('Manage Customer Address Attributes'),
                Mage::helper('Magento_CustomerCustomAttributes_Helper_Data')->__('Manage Customer Address Attributes'));
        return $this;
    }
    ...
    public function editAction()
    {
        ...
        if ($attributeId) {
            ...
            if (!$attributeObject->getId()) {
                $this->_getSession()->addError(
                    Mage::helper('Magento_CustomerCustomAttributes_Helper_Data')->__('Attribute is no longer exists.')
                );
                $this->_redirect('*/*/');
                return;
            }
        ...
        $label = $attributeObject->getId()
            ? Mage::helper('Magento_CustomerCustomAttributes_Helper_Data')->__('Edit Customer Address Attribute')
            : Mage::helper('Magento_CustomerCustomAttributes_Helper_Data')->__('New Customer Address Attribute');
        ...
        if ($this->getRequest()->isPost() && $data) {
            ...
            try {
                ...
                $this->_getSession()->addSuccess(
                    Mage::helper('Magento_CustomerCustomAttributes_Helper_Data')->__('You saved the customer address attribute.')
                );
                ...
                $this->_getSession()->addException($e,
                    Mage::helper('Magento_CustomerCustomAttributes_Helper_Data')->__('Something went wrong saving the customer address attribute.')
                );
                $this->_getSession()->setAttributeData($data);
                $this->_redirect('*/*/edit', array('_current' => true));
                return;
    ...
    public function deleteAction()
    {
        ...
        if ($attributeId) {
            ...
            try {
                ...
                $this->_getSession()->addSuccess(
                    Mage::helper('Magento_CustomerCustomAttributes_Helper_Data')->__('You deleted the customer address attribute.')
                );
                $this->_redirect('*/*/');
                return;
            } catch (Magento_Core_Exception $e) {
                ...
                $this->_getSession()->addException($e,
                    Mage::helper('Magento_CustomerCustomAttributes_Helper_Data')->__('Something went wrong deleting the customer address attribute.')
                );
                $this->_redirect('*/*/edit', array('attribute_id' => $attributeId, '_current' => true));
                return;
            }
