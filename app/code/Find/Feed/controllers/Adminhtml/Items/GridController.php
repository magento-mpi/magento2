...
class Find_Feed_Adminhtml_Items_GridController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function massEnableAction()
    {
        ...
        if ($updatedProducts) {
            try {
                ...
                $this->_getSession()->addSuccess(Mage::helper('Find_Feed_Helper_Data')->__("%1 product in feed", $updatedProducts));
            } catch (Exception $e) {
                $this->_getSession()->addError(Mage::helper('Find_Feed_Helper_Data')->__("We are unable to process an import.") . $e->getMessage());
            }
        }
    ...
    public function massDisableAction()
    {
        ...
        if ($updatedProducts) {
            Mage::getModel('Find_Feed_Model_Import')->processImport();
            $this->_getSession()->addSuccess(Mage::helper('Find_Feed_Helper_Data')->__("%1 product not in feed", $updatedProducts));
        }
        $this->_redirect('*/*/index');
    }
