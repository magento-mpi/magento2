...
$size = 0; $files = 0;
@list($url, $size, $files) = preg_split('#\s+#', $latest);

echo "<html><head><style type='text/css'>

* { font: normal 12px sans-serif; }
strong { font-weight:bold; }
.border { border:solid 1px #777; width:100%; }
.progress { height:20px; background:blue; color:white; text-align:right; width:0; }
...
echo "<strong>DOWNLOADING:</strong> ".$url." ";

$filename = basename($url);
$fileext = pathinfo($url, PATHINFO_EXTENSION);

$archiveTypes = array('gz'=>'gzip_file', 'bz2'=>'bzip_file', 'zip'=>'zip_file');

if (!isset($archiveTypes[$fileext])) {
    echo "Unknown file type: $fileext";
    exit;
}
...
echo "<hr/><strong>EXTRACTING</strong>, Please wait, this may take a few minutes... ";

$progress = 0;
createProgress('extract');
// open the downloaded archive in current directory
$class = $archiveTypes[$fileext];
$archive = new $class($filename);
$archive->set_options(array('overwrite' => 1));
$archive->extract_files();

chdir('magento');

if (file_exists('checksum.md5')) {
    echo "<hr/><strong>CHECKING MD5 SUM...</strong> ";

    $progress = 0;
    createProgress('md5sum');
    $cfile = fopen('checksum.md5','r');
    while (!feof($cfile)) {
        $line = fgets($cfile, BUFFER_SIZE);
        if (empty($line)) {
            continue;
        }
        list($md5, $filename) = explode(' ', trim($line), 2);
        ...
        if (md5_file($filename)!=$md5) {
            echo "INVALID MD5 CHECKSUM FOR ".$filename."<br/>";
            continue;
        }

        $progress++;
        updateProgress('md5sum', round($progress/$files*100));
    }
    fclose($cfile);
}
...
if (isset($_SERVER['HTTP_HOST'])) {
    ...
    echo "Try this link for install wizard: <a href='$link'>$link</a>";
}
...
{
    echo '<div class="border"><div id="progress-'.$name.'" class="progress">0%</div></div>';
}
...
function updateProgress($name, $percent)
{
    ...
    if (!isset($current[$name]) || $current[$name]!=$percent) {
        echo '<script type="text/javascript">updateProgress("'.$name.'", "'.$percent.'")</script>'."\n";
    ...
    function extract_files()
    {
        ...
        if ($fp = $this->open_archive())
        {
            ...
            while ($block = fread($fp, 512))
            {
                ...
                {
                    if (!is_dir($file['name']))
                        mkdir($file['name']/*, $file['stat'][2]*/); //UPDATED
                }
                ...
                else if ($file['type'] == 2)
                {
                    ...
                    #chmod($file['name'], $file['stat'][2]); //UPDATED
                }
                else if ($new = @fopen($file['name'], "wb"))
                {
                    $GLOBALS['progress']++; //UPDATED
                    updateProgress('extract', round($GLOBALS['progress']/$GLOBALS['files']*100)); //UPDATED
                    // UPDATED to account for 0 file size
                    $size = $file['stat'][7];
                    if ($size>0) {
                        fwrite($new, fread($fp, $size));
                        $adj = (512 - $file['stat'][7] % 512) == 512 ? 0 : (512 - $file['stat'][7] % 512);
                        if ($adj>0) {
                            fread($fp, $adj);
                        } else {
                            #echo "ADJ0: ".$file['name']."<br/>";
                        }
                    } else {
                        #echo "SIZE0: ".$file['name']."<br/>";
                    }
                    fclose($new);
                    #chmod($file['name'], $file['stat'][2]); //UPDATED
                }
                ...
                #chown($file['name'], $file['stat'][4]); //UPDATED
                #chgrp($file['name'], $file['stat'][5]); //UPDATED
                #touch($file['name'], $file['stat'][9]); //UPDATED
                unset ($file);
            }
        }
        else
