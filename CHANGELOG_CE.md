* Framework improvements:
  * Completely revisited subsystem of static view files preprocessing (MAGETWO-18001):
    ...
    * The concept of "view asset" is used in client code of the View library across the board
    * Refactored and simplified LESS preprocessing library, mechanisms of merging and minifying static view files
  * Revisited the way how links to static view files are generated and served (MAGETWO-18001):
    * Links are generated by a uniform pattern, such as http://example.com/pub/static/frontend/Magento/blank/en_US/Namespace_Module/logo.gif
    * During page generation time, no file operations will be invoked when a link to static view file is generated
    * The actual file will be requested directly via HTTP-server. If it doesn't exist, it will be materialized automatically via separate entry point (pub/static.php)
    * In the "production mode", the (pub/static.php) is not going to work. Instead, it is expected that all files have been pre-materialized in advance (using a view files deployment tool)
  ...
  * Revisited structure of all view files for security and reliability (MAGETWO-21196/MAGETWO-21067):
    * Inside modules, the view files are segregated into directories: "web", "templates" and "layout"
    * Similar for themes, but the structure is recursively repeated for any modular files
    * The `pub/lib` is renamed into `lib/web`. Which means there are no static files that are publicly accessible by default. All static view files may be subject of preprocessing (MAGETWO-21067)
